...
static void fm_cell_renderer_text_get_property(GObject* object, guint param_id, GValue* value, GParamSpec* psec);
static void fm_cell_renderer_text_set_property(GObject* object, guint param_id, const GValue* value, GParamSpec* psec);

enum { PROP_HEIGHT = 1, N_PROPS };
...
static void fm_cell_renderer_text_class_init(FmCellRendererTextClass* klass) {
    ...
    g_object_class_install_property(
        ...
        g_param_spec_int("max-height", "Maximum_height", "Maximum height", -1, 2048, -1, G_PARAM_READWRITE));
}
...
static void _get_size(GtkCellRenderer* cell,
                      GtkWidget* widget,
                      PangoLayout* layout,
                      gchar* text,
                      const GdkRectangle* cell_area,
    ...
    g_object_get(G_OBJECT(cell), "wrap-mode", &wrap_mode, "wrap-width", &wrap_width, "alignment", &alignment, NULL);
...
#endif
{
    ...
    _get_size(cell, widget, layout, text, cell_area, &text_width, &text_height, &xpad, &ypad, &x_offset, &y_offset,
              &x_align_offset);
...
#if GTK_CHECK_VERSION(3, 0, 0)
    gtk_render_layout(style, cr, cell_area->x + x_offset + xpad - x_align_offset, cell_area->y + y_offset + ypad,
                      layout);
#else
    gtk_paint_layout(style, window, state, TRUE, expose_area, widget, "cellrenderertext",
                     cell_area->x + x_offset + xpad - x_align_offset, cell_area->y + y_offset + ypad, layout);
#endif
...
#else
        gtk_paint_focus(style, window, state, background_area, widget, "cellrenderertext", rect.x, rect.y, rect.width,
                        rect.height);
#endif
    }
...
static void fm_cell_renderer_text_get_size(GtkCellRenderer* cell,
    ...
    g_object_get(G_OBJECT(cell), "text", &text, NULL);
    _get_size(cell, widget, NULL, text, rectangle, width, height, NULL, NULL, x_offset, y_offset, NULL);
    g_free(text);
}
...
static void fm_cell_renderer_text_get_preferred_width(GtkCellRenderer* cell,
    ...
    if (wrap_width > 0) {
        ...
        GTK_CELL_RENDERER_CLASS(fm_cell_renderer_text_parent_class)
