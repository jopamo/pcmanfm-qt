...
static GAppInfo* app_info_create_from_commandline(const char* commandline,
    ...
    if (g_mkdir_with_parents(dirname, 0700) == 0) {
        ...
        if (mime_type && application_name[0]) {
            ...
            app = g_app_info_create_from_commandline(commandline, app_basename, 0, NULL);
            if (app) {
        ...
        if (fd != -1) {
            ...
            g_string_printf(content,
                            ...
                            application_name, commandline);
            if (mime_type)
                g_string_append_printf(content, G_KEY_FILE_DESKTOP_KEY_MIME_TYPE "=%s\n", mime_type);
            g_string_append_printf(content, G_KEY_FILE_DESKTOP_KEY_TERMINAL "=%s\n", terminal ? "true" : "false");
            if (terminal)
            ...
            if (g_file_set_contents(filename, content->str, content->len, NULL)) {
                ...
                /* if there is mime_type set then created application will be
                   ...
                   below) but if not then we should remove this temp. file */
                    ...
                    g_object_weak_ref(G_OBJECT(app), on_temp_appinfo_destroy, g_strdup(filename));
            }
            else
                g_unlink(filename);
...
static void on_browse_btn_clicked(GtkButton* btn, AppChooserData* data) {
    ...
    char* binary;
    gtk_file_filter_add_custom(filter, GTK_FILE_FILTER_FILENAME | GTK_FILE_FILTER_MIME_TYPE, exec_filter_func, NULL,
                               NULL);
    ...
    if (g_str_has_suffix(fm_path_get_basename(file), ".desktop")) {
        ...
        if (g_key_file_load_from_file(kf, binary, 0, NULL) && (info = g_desktop_app_info_new_from_keyfile(kf)) != NULL)
        /* it is a valid desktop entry */
        {
            /* FIXME: it will duplicate the file, how to avoid that? */
            gtk_entry_set_text(data->cmdline, g_app_info_get_commandline(G_APP_INFO(info)));
            gtk_entry_set_text(data->app_name, g_app_info_get_name(G_APP_INFO(info)));
            gtk_toggle_button_set_active(
                data->use_terminal,
                g_key_file_get_boolean(kf, G_KEY_FILE_DESKTOP_GROUP, G_KEY_FILE_DESKTOP_KEY_TERMINAL, NULL));
            gtk_toggle_button_set_active(data->keep_open,
                                         g_key_file_get_boolean(kf, G_KEY_FILE_DESKTOP_GROUP, "X-KeepTerminal", NULL));
            g_object_unref(info);
            g_key_file_free(kf);
            fm_path_unref(file);
            return;
...
static void on_use_terminal_changed(GtkToggleButton* btn, AppChooserData* data) {
    if (data->keep_open)
        gtk_widget_set_sensitive(GTK_WIDGET(data->keep_open), gtk_toggle_button_get_active(btn));
}
    ...
    switch (gtk_notebook_get_current_page(data->notebook)) {
        ...
        case 1: /* custom cmd line */
        {
            ...
            if (cmdline && cmdline[0]) {
                    ...
                    if (menu_cache) {
...
#endif
                        {
                            ...
                            for (l = all_apps; l; l = l->next) {
                                ...
                                if (g_strcmp0(bin1, bin2) == 0) {
                                    app =
                                        G_APP_INFO(g_desktop_app_info_new(menu_cache_item_get_id(MENU_CACHE_ITEM(ma))));
                ...
                app =
                    app_info_create_from_commandline(cmdline, app_name ? app_name : "", bin1,
...
GAppInfo* fm_choose_app_for_mime_type(GtkWindow* parent, FmMimeType* mime_type, gboolean can_set_default) {
    ...
    if (gtk_dialog_run(dlg) == GTK_RESPONSE_OK) {
            ...
            g_app_info_get_name(app)[0]) /* don't add empty name */
        {
            ...
            if (set_default)
                g_app_info_set_as_default_for_type(app, fm_mime_type_get_type(mime_type), NULL);
        }
    }
