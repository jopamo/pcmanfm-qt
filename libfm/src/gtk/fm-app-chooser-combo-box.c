    ...
    if (it.user_data == data->other_apps_iter.user_data) {
        ...
        if (app) {
            ...
            else /* if it's not found, add it to the list */
            {
                ...
                gtk_list_store_set(GTK_LIST_STORE(model), &it, 0, g_app_info_get_icon(app), 1, g_app_info_get_name(app),
                                   2, app, -1);
...
void fm_app_chooser_combo_box_setup(GtkComboBox* combo, FmMimeType* mime_type, GList* apps, GAppInfo* sel) {
    ...
    for (l = apps; l; l = l->next) {
        ...
        gtk_list_store_insert_with_values(store, &it, -1, 0, g_app_info_get_icon(app), 1, g_app_info_get_name(app), 2,
                                          app, -1);
    ...
    /* other applications */
    gtk_list_store_insert_with_values(store, &it, -1, 0, NULL, 1, _("Customize"),  // FIXME: should be "Customize..."
                                      2, NULL, -1);
...
GAppInfo* fm_app_chooser_combo_box_dup_selected_app(GtkComboBox* combo, gboolean* is_sel_changed) {
    ...
    if (gtk_combo_box_get_active_iter(combo, &it)) {
        ...
        if (is_sel_changed) {
            FmAppChooserComboBoxData* data =
                (FmAppChooserComboBoxData*)g_object_get_qdata(G_OBJECT(combo), fm_qdata_id);
