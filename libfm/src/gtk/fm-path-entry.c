...
/* properties */
enum { PROP_0, PROP_HIGHLIGHT_COMPLETION_MATCH };
...
static void fm_path_entry_completion_render_func(GtkCellLayout* cell_layout,
                                                 ...
                                                 GtkTreeIter* iter,
                                                 gpointer data);
static void fm_path_entry_set_property(GObject* object, guint prop_id, const GValue* value, GParamSpec* pspec);
static void fm_path_entry_get_property(GObject* object, guint prop_id, GValue* value, GParamSpec* pspec);
...
enum { COL_BASENAME, COL_FULL_PATH, N_COLS };
...
G_DEFINE_TYPE_EXTENDED(FmPathEntryModel,
                       fm_path_entry_model,
                       GTK_TYPE_LIST_STORE,
                       0,
                       G_IMPLEMENT_INTERFACE(GTK_TYPE_TREE_MODEL, fm_path_entry_model_iface_init));
...
static gboolean fm_path_entry_key_press(GtkWidget* widget, GdkEventKey* event, gpointer user_data) {
    ...
    if (gtk_get_current_event_state(&state) && (state & GDK_CONTROL_MASK) == GDK_CONTROL_MASK)
        return FALSE;
...
static void fm_path_entry_class_init(FmPathEntryClass* klass) {
    ...
    g_object_class_install_property(
        object_class, PROP_HIGHLIGHT_COMPLETION_MATCH,
        g_param_spec_boolean("highlight-completion-match", "Highlight completion match",
                             "Whether to highlight the completion match", TRUE, G_PARAM_READWRITE));
...
static inline void update_inline_completion(FmPathEntryPrivate* priv) {
    gtk_entry_completion_set_inline_completion(priv->completion, priv->folder_loaded);
}
...
static gboolean fm_path_entry_focus_in_event(GtkWidget* widget, GdkEventFocus* event) {
    ...
    priv->id_changed = g_signal_connect(entry, "changed", G_CALLBACK(fm_path_entry_changed), NULL);
...
static void on_dir_list_finished(GObject* source_object, GAsyncResult* res, gpointer user_data)
#else
...
#if GLIB_CHECK_VERSION(2, 36, 0)
static void list_sub_dirs(GTask* task, gpointer source_object, gpointer user_data, GCancellable* cancellable)
#else
...
#endif
{
    ...
    GFileEnumerator* enu =
        g_file_enumerate_children(data->dir,
                                  G_FILE_ATTRIBUTE_STANDARD_DISPLAY_NAME "," G_FILE_ATTRIBUTE_STANDARD_EDIT_NAME
...
static void fm_path_entry_changed(GtkEditable* editable, gpointer user_data) {
    ...
    if (sep) /* we found a parent dir */
    {
        ...
        if (!priv->parent_dir || priv->parent_len != parent_len || strncmp(priv->parent_dir, path_str, parent_len)) {
            ...
            g_object_unref(task);
#else
            g_io_scheduler_push_job(list_sub_dirs, data, list_sub_dir_names_free, G_PRIORITY_LOW, data->cancellable);
#endif
        }
...
static void fm_path_entry_paste_and_go(GtkMenuItem* menuitem, GtkEntry* entry) {
    GtkClipboard* clipboard =
        gtk_clipboard_get_for_display(gtk_widget_get_display(GTK_WIDGET(menuitem)), GDK_SELECTION_CLIPBOARD);
...
static void fm_path_entry_populate_popup(GtkEntry* entry, GtkMenu* menu, gpointer user_data) {
    ...
    GtkClipboard* clipboard =
        gtk_clipboard_get_for_display(gtk_widget_get_display(GTK_WIDGET(entry)), GDK_SELECTION_CLIPBOARD);
    ...
    g_signal_connect(menuitem, "activate", G_CALLBACK(fm_path_entry_paste_and_go), entry);
...
static void fm_path_entry_init(FmPathEntry* entry) {
    ...
    gtk_cell_layout_set_cell_data_func(GTK_CELL_LAYOUT(completion), render, fm_path_entry_completion_render_func, entry,
                                       NULL);
...
static void fm_path_entry_model_get_value(GtkTreeModel* tree_model, GtkTreeIter* iter, gint column, GValue* value) {
