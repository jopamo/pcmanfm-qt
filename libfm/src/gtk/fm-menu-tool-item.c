...
static void fm_menu_tool_item_state_changed(GtkWidget* widget, GtkStateType previous_state) {
...
static void fm_menu_tool_item_set_property(GObject* object, guint prop_id, const GValue* value, GParamSpec* pspec) {
...
static void fm_menu_tool_item_get_property(GObject* object, guint prop_id, GValue* value, GParamSpec* pspec) {
    FmMenuToolItem* button = FM_MENU_TOOL_ITEM(object);
...
static void fm_menu_tool_item_class_init(FmMenuToolItemClass* klass) {
    ...
    signals[SHOW_MENU] = g_signal_new("show-menu", G_OBJECT_CLASS_TYPE(klass), G_SIGNAL_RUN_FIRST,
                                      G_STRUCT_OFFSET(FmMenuToolItemClass, show_menu), NULL, NULL,
                                      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

    g_object_class_install_property(
        object_class, PROP_MENU,
        g_param_spec_object("menu", "Menu", "The dropdown menu", GTK_TYPE_MENU, G_PARAM_READWRITE));

    g_type_class_add_private(object_class, sizeof(FmMenuToolItemPrivate));
}

static void menu_position_func(GtkMenu* menu, int* x, int* y, gboolean* push_in, FmMenuToolItem* button) {
    FmMenuToolItemPrivate* priv = button->priv;
...
static void arrow_button_toggled_cb(GtkToggleButton* togglebutton, FmMenuToolItem* button) {
    FmMenuToolItemPrivate* priv = button->priv;
...
static gboolean arrow_button_button_press_event_cb(GtkWidget* widget, GdkEventButton* event, FmMenuToolItem* button) {
    if (event->button == 1) {
...
#endif
{
    ...
    if (button->priv->menu) {
        g_signal_handlers_disconnect_by_func(button->priv->menu, menu_deactivate_cb, button);
        gtk_menu_detach(button->priv->menu);

        g_signal_handlers_disconnect_by_func(button->priv->arrow_button, arrow_button_toggled_cb, button);
        g_signal_handlers_disconnect_by_func(button->priv->arrow_button, arrow_button_button_press_event_cb, button);
    }
...
void fm_menu_tool_item_set_menu(FmMenuToolItem* button, GtkWidget* menu) {
    ...
    if (priv->menu != GTK_MENU(menu)) {
        ...
        if (priv->menu) {
            g_signal_handlers_disconnect_by_func(priv->menu, menu_deactivate_cb, button);
