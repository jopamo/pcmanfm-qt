...
static gboolean on_auto_scroll(gpointer user_data) {
    ...
    */
    if (y < 0 || y > allocation.height || x < 0 || x > allocation.width) {
        as->timeout = 0;
        return FALSE;
    }
...
static void on_drag_leave(GtkWidget* widget, GdkDragContext* drag_context, guint time, gpointer user_data) {
    FmDndAutoScroll* as = (FmDndAutoScroll*)user_data;
    if (as->timeout) {
 ...
 */
void fm_dnd_set_dest_auto_scroll(GtkWidget* drag_dest_widget, GtkAdjustment* hadj, GtkAdjustment* vadj) {
    FmDndAutoScroll* as;
    ...
    g_object_set_qdata_full(G_OBJECT(drag_dest_widget), data_id, as, fm_dnd_auto_scroll_free);

    g_signal_connect(drag_dest_widget, "drag-motion", G_CALLBACK(on_drag_motion), as);
    g_signal_connect(drag_dest_widget, "drag-leave", G_CALLBACK(on_drag_leave), as);
}
