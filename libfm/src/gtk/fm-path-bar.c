...
static void fm_path_bar_class_init(FmPathBarClass* klass) {
     ...
     */
    signals[CHDIR] =
        g_signal_new("chdir", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST, G_STRUCT_OFFSET(FmPathBarClass, chdir),
...
static GtkRadioButton* create_btn(FmPathBar* bar, GSList* grp, FmPath* path_element) {
    ...
    if (!fm_path_get_parent(path_element)) /* this element is root */
    {
        ...
        gtk_box_pack_start(GTK_BOX(hbox), gtk_image_new_from_icon_name("drive-harddisk", GTK_ICON_SIZE_BUTTON), FALSE,
                           FALSE, 0);
