...
enum { URI_LIST = 1, GNOME_COPIED_FILES, KDE_CUT_SEL, UTF8_STRING, N_CLIPBOARD_TARGETS };
...
gboolean fm_clipboard_cut_or_copy_files(GtkWidget* src_widget, FmPathList* files, gboolean _is_cut) {
    ...
    ret = gtk_clipboard_set_with_data(clip, targets, G_N_ELEMENTS(targets), get_data, clear_data,
                                      fm_path_list_ref(files));
    is_cut = _is_cut;
    return ret;
}
...
gboolean fm_clipboard_have_files(GtkWidget* dest_widget) {
    ...
    for (i = 1; i < N_CLIPBOARD_TARGETS; i++)
        if (target_atom[i] != GDK_NONE && gtk_clipboard_wait_is_target_available(clipboard, target_atom[i]))
            return TRUE;
    return FALSE;
}
