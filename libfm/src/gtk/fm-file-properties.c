...
/* for 'Read' combo box */
enum { NO_CHANGE = 0, READ_USER, READ_GROUP, READ_ALL };
...
static GdkPixbuf* vfs_load_icon(GtkIconTheme* theme, const char* icon_name, int size) {
    GdkPixbuf* icon = NULL;
    const char* file;
    GtkIconInfo* inf = gtk_icon_theme_lookup_icon(theme, icon_name, size, GTK_ICON_LOOKUP_USE_BUILTIN);
    if (G_UNLIKELY(!inf))
        return NULL;
...
static void _change_icon(GtkWidget* dlg, FmFilePropData* data) {
    ...
    g_signal_connect(chooser, "update-preview", G_CALLBACK(on_update_preview), GTK_IMAGE(preview));
    ...
    gdk_window_set_cursor(gtk_widget_get_window(GTK_WIDGET(thread_data.view)), gdk_cursor_new(GDK_WATCH));
    ...
    if (gtk_dialog_run(GTK_DIALOG(chooser_dlg)) == GTK_RESPONSE_OK) {
        ...
        if (gtk_notebook_get_current_page(GTK_NOTEBOOK(notebook)) == 0) {
            ...
            if (icon_name) {
                GdkPixbuf* pix = gdk_pixbuf_new_from_file_at_scale(icon_name, 48, 48, TRUE, NULL);
                if (pix) {
        ...
        if (icon_name) {
            g_object_set_qdata_full(G_OBJECT(data->icon), fm_qdata_id, icon_name, g_free);
        }
    }
...
static gboolean _icon_click_event(GtkWidget* widget, GdkEventButton* event, FmFilePropData* data) {
...
static gboolean _icon_press_event(GtkWidget* widget, GdkEventKey* event, FmFilePropData* data) {
...
static gboolean _on_timeout(gpointer user_data) {
    ...
    if (G_LIKELY(dc && !fm_job_is_cancelled(FM_JOB(dc)))) {
        char* str;
        fm_file_size_to_str(size_str, sizeof(size_str), dc->total_size, fm_config->si_unit);
        str = g_strdup_printf("%s (%'llu %s)", size_str, (long long unsigned int)dc->total_size,
                              dngettext(GETTEXT_PACKAGE, "byte", "bytes", (gulong)dc->total_size));
        gtk_label_set_text(data->total_size, str);
        g_free(str);

        fm_file_size_to_str(size_str, sizeof(size_str), dc->total_ondisk_size, fm_config->si_unit);
        str = g_strdup_printf("%s (%'llu %s)", size_str, (long long unsigned int)dc->total_ondisk_size,
                              dngettext(GETTEXT_PACKAGE, "byte", "bytes", (gulong)dc->total_ondisk_size));
...
static gboolean ensure_valid_owner(FmFilePropData* data) {
    ...
    if (tmp && *tmp) {
        if (data->all_native &&
            !isdigit(tmp[0])) /* entering names instead of numbers is only allowed for local files. */
        {
            struct passwd* pw;
            if (!(pw = getpwnam(tmp)))
                ret = FALSE;
    ...
    if (response == GTK_RESPONSE_OK) {
        ...
        if (gtk_widget_get_visible(data->permissions_tab)) {
            ...
            if (new_mode_mask || data->uid != -1 || data->gid != -1) {
                ...
                if (data->has_dir && data->exec_perm_sel == NO_CHANGE && data->flags_set_sel == NO_CHANGE) {
                    ...
                    if (fm_yes_no(GTK_WINDOW(data->dlg), NULL,
...
static void update_permissions(FmFilePropData* data) {
    ...
    if (data->hidden == NULL)
        ...
        /* FIXME: use markup for bold text? */
            ...
            gtk_widget_set_tooltip_text(data->hidden, _("This file is hidden because its "
                                                        "name starts with a dot ('.')."));
        else
            gtk_widget_set_tooltip_text(data->hidden, _("Files on this system are hidden "
                                                        "if their name starts with a dot "
                                                        "('.'). Hit <Ctrl+H> to toggle "
                                                        "displaying hidden files."));
        gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(data->hidden), fm_file_info_is_hidden(fi));
...
static gboolean on_icon_enter_notify(GtkWidget* widget, GdkEvent* event, FmFilePropData* data) {
...
static void update_ui(FmFilePropData* data) {
    ...
    if (data->single_type) /* all files are of the same mime-type */
    {
        ...
        if (data->mime_type) {
            ...
            char* text = g_strdup_printf("%s (%s)", fm_mime_type_get_desc(data->mime_type),
        ...
        if (data->single_file && (fm_file_info_is_symlink(data->fi) || fm_file_info_is_shortcut(data->fi))) {
    ...
    if (data->single_file) {
        ...
        if (!fm_file_info_can_set_name(data->fi) || fm_file_info_is_shortcut(data->fi)) {
...
GtkDialog* fm_file_properties_widget_new(FmFileInfoList* files, gboolean toplevel) {
    ...
    g_signal_connect(data->icon_eventbox, "button-press-event", G_CALLBACK(_icon_click_event), data);
    g_signal_connect(data->icon_eventbox, "key-press-event", G_CALLBACK(_icon_press_event), data);
    ...
    if (gtk_widget_get_can_focus(data->icon_eventbox)) {
        ...
        g_signal_connect(data->icon_eventbox, "enter-notify-event", G_CALLBACK(on_icon_enter_notify), data);
    }
 ...
 * Since: 1.2.0
 */
gboolean fm_file_properties_add_for_mime_type(const char* mime_type, FmFilePropertiesExtensionInit* callbacks) {
    FmMimeType* type;
    FmFilePropExt* ext;
