    ...
    if (fm_file_info_list_get_length(job->file_infos) == 1) {
        ...
        do {
            ...
            if (item && item->fi && (path = fm_file_info_get_path(item->fi)) &&
                fm_path_equal(path, fm_file_info_get_path(fi))) {
                fm_file_info_unref(item->fi);
...
static inline FmPlacesItem* add_new_item(GtkListStore* model, FmPlacesType type, GtkTreeIter* it, GtkTreePath* at) {
    GtkTreeIter next_it;
...
static FmPlacesItem* new_path_item(GtkListStore* model,
                                   GtkTreeIter* it,
                                   FmPath* path,
                                   FmPlacesOrder id,
                                   const char* label,
    ...
    if (gtk_tree_model_get_iter_first(GTK_TREE_MODEL(model), &next_it))
        do {
            tst = NULL;
...
_added:
    pix = fm_pixbuf_from_icon(item->icon, fm_config->pane_icon_size);
    gtk_list_store_set(model, it, FM_PLACES_MODEL_COL_INFO, item, FM_PLACES_MODEL_COL_LABEL, label,
...
static void add_bookmarks(FmPlacesModel* model, FmFileInfoJob* job) {
    ...
    for (l = bms; l; l = l->next) {
        ...
        gtk_list_store_set(GTK_LIST_STORE(model), &it, FM_PLACES_MODEL_COL_ICON, pix, FM_PLACES_MODEL_COL_LABEL,
                           bm->name, -1);
    }
    g_list_free(bms);
...
static gboolean update_trash_item(gpointer user_data) {
    FmPlacesModel* model = FM_PLACES_MODEL(user_data);
    if (!g_source_is_destroyed(g_main_current_source()) && fm_config->use_trash && model->trash) {
...
static void on_trash_changed(GFileMonitor* monitor,
                             GFile* gf,
                             GFile* other,
...
static void on_places_home_changed(FmConfig* cfg, gpointer user_data) {
    ...
    if (cfg->places_home) {
        ...
        new_path_item(model, &it, path, FM_PLACES_ID_HOME, _("Home Folder"), "user-home", NULL);
    }
    else {
...
static void on_places_desktop_changed(FmConfig* cfg, gpointer user_data) {
    GtkListStore* model = GTK_LIST_STORE(user_data);
    GtkTreeIter it;
    if (cfg->places_desktop && g_file_test(g_get_user_special_dir(G_USER_DIRECTORY_DESKTOP), G_FILE_TEST_IS_DIR)) {
        new_path_item(model, &it, fm_path_get_desktop(), FM_PLACES_ID_DESKTOP, _("Desktop"), "user-desktop", NULL);
    }
    else {
...
static void on_places_applications_changed(FmConfig* cfg, gpointer user_data) {
    ...
    if (!fm_module_is_in_use("vfs", "menu"))
        ...
        new_path_item(model, &it, fm_path_get_apps_menu(), FM_PLACES_ID_APPLICATIONS, _("Applications"),
...
static void on_places_root_changed(FmConfig* cfg, gpointer user_data) {
    ...
    GtkTreeIter it;
    if (cfg->places_root) {
        new_path_item(model, &it, fm_path_get_root(), FM_PLACES_ID_ROOT, _("Filesystem Root"), "drive-harddisk", NULL);
    }
    else {
...
static void fm_places_model_init(FmPlacesModel* self) {
    ...
    self->theme_change_handler =
        g_signal_connect_swapped(gtk_icon_theme_get_default(), "changed", G_CALLBACK(update_icons), self);
    self->use_trash_change_handler =
        g_signal_connect(fm_config, "changed::use_trash", G_CALLBACK(on_use_trash_changed), self);
    self->places_home_change_handler =
        g_signal_connect(fm_config, "changed::places_home", G_CALLBACK(on_places_home_changed), self);
    self->places_desktop_change_handler =
        g_signal_connect(fm_config, "changed::places_desktop", G_CALLBACK(on_places_desktop_changed), self);
    self->places_root_change_handler =
        g_signal_connect(fm_config, "changed::places_root", G_CALLBACK(on_places_root_changed), self);
    self->places_computer_change_handler =
        g_signal_connect(fm_config, "changed::places_computer", G_CALLBACK(on_places_computer_changed), self);
    self->places_trash_change_handler =
        g_signal_connect(fm_config, "changed::places_trash", G_CALLBACK(on_use_trash_changed), self);
    self->places_applications_change_handler =
        g_signal_connect(fm_config, "changed::places_applications", G_CALLBACK(on_places_applications_changed), self);
    self->places_network_change_handler =
        g_signal_connect(fm_config, "changed::places_network", G_CALLBACK(on_places_network_changed), self);
    self->places_unmounted_change_handler =
        g_signal_connect(fm_config, "changed::places_unmounted", G_CALLBACK(on_places_unmounted_changed), self);
    self->pane_icon_size_change_handler =
        g_signal_connect(fm_config, "changed::pane_icon_size", G_CALLBACK(on_pane_icon_size_changed), self);
    icon = fm_icon_from_name("media-eject");
    self->eject_icon = fm_pixbuf_from_icon(icon, fm_config->pane_icon_size);
    g_object_unref(icon);
    if (fm_config->places_home) {
        path = fm_path_get_home();
        new_path_item(model, &it, path, FM_PLACES_ID_HOME, _("Home Folder"), "user-home", job);
    }
    ...
    g_list_free(vols);
    self->bookmarks = fm_bookmarks_dup();
    if (self->bookmarks)
...
G_DEFINE_TYPE_WITH_CODE(FmPlacesModel,
                        fm_places_model,
                        GTK_TYPE_LIST_STORE,
                        G_IMPLEMENT_INTERFACE(GTK_TYPE_TREE_DRAG_SOURCE, fm_places_model_drag_source_init))
