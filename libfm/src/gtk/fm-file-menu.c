...
FmFileMenu* fm_file_menu_new_for_files(GtkWindow* parent, FmFileInfoList* files, FmPath* cwd, gboolean auto_destroy) {
    ...
    if (apps) /* add specific menu items for those files */
    {
        ...
        for (l = apps; l; l = l->next) {
            ...
            act =
                gtk_action_new(g_app_info_get_id(app), g_app_info_get_name(app), g_app_info_get_description(app), NULL);
        ...
        for (l = extensions; l; l = l->next) {
            ...
            if (ext->mime_type == mime_types->data) /* the same mime type */
                if (ext->cb.update_file_menu_for_mime_type != NULL)
                    ext->cb.update_file_menu_for_mime_type(parent, ui, xml, act_grp, data, files, (items_num == 1));
        }
    }
    ...
    if (auto_destroy) {
        g_signal_connect_swapped(data->menu, "selection-done", G_CALLBACK(fm_file_menu_destroy), data);
    }
...
static void on_paste(GtkAction* action, gpointer user_data) {
    ...
    if (fi) {
        fm_clipboard_paste_files(gtk_menu_get_attach_widget(data->menu), fm_file_info_get_path(fi));
    }
}
