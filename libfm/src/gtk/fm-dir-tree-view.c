...
static void fm_dir_tree_view_set_property(GObject* object, guint prop_id, const GValue* value, GParamSpec* pspec) {
...
static void fm_dir_tree_view_get_property(GObject* object, guint prop_id, GValue* value, GParamSpec* pspec) {
    GtkTreeView* tree_view;
    FmDirTreeModel* model;
...
static void fm_dir_tree_view_item_popup(GtkWidget* widget, GtkTreeModel* model, GtkTreeIter* it, guint32 time) {
    FmFileInfo* file;
...
static gboolean on_button_press_event(GtkWidget* widget, GdkEventButton* evt) {
    ...
    if (evt->button == 3) /* we handle only right-click here */
    {
        ...
        if (gtk_tree_view_get_path_at_pos(tree_view, (gint)evt->x, (gint)evt->y, &path, NULL, NULL, NULL)) {
...
static gboolean on_drag_motion(GtkWidget* widget, GdkDragContext* drag_context, gint x, gint y, guint time) {
    ...
    if (tp && (pos == GTK_TREE_VIEW_DROP_INTO_OR_BEFORE || pos == GTK_TREE_VIEW_DROP_INTO_OR_AFTER) &&
        gtk_tree_model_get_iter(model, &it, tp)) /* dragged into item */
        gtk_tree_model_get(model, &it, FM_DIR_TREE_MODEL_COL_INFO, &file_info, -1);
...
static void fm_dir_tree_view_class_init(FmDirTreeViewClass* klass) {
    ...
    g_object_class_install_property(
        g_object_class, PROP_SHOW_HIDDEN,
        g_param_spec_boolean("show-hidden", "Show hidden", "Should hidden files be shown or not", FALSE,
                             G_PARAM_READWRITE));
     ...
     */
    signals[CHDIR] =
        g_signal_new("chdir", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(FmDirTreeViewClass, chdir),
     ...
     * Since: 1.2.0
     */
    signals[ITEM_POPUP] = g_signal_new("item-popup", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                                       G_STRUCT_OFFSET(FmDirTreeViewClass, item_popup), NULL, NULL,
                                       fm_marshal_VOID__OBJECT_OBJECT_POINTER, G_TYPE_NONE, 3, G_TYPE_OBJECT,
...
static void fm_dir_tree_view_init(FmDirTreeView* view) {
    ...
    gtk_tree_view_column_set_attributes(col, render, "pixbuf", FM_DIR_TREE_MODEL_COL_ICON, "info",
                                        FM_DIR_TREE_MODEL_COL_INFO, NULL);
