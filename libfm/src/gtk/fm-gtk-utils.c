...
void fm_show_error(GtkWindow* parent, const char* title, const char* msg) {
    GtkWidget* dlg = gtk_message_dialog_new(parent, 0, GTK_MESSAGE_ERROR, GTK_BUTTONS_OK, "%s", msg);
    /* g_message("(!) %s", msg); */
...
gboolean fm_yes_no(GtkWindow* parent, const char* title, const char* question, gboolean default_yes) {
    ...
    GtkDialog* dlg = GTK_DIALOG(
        gtk_message_dialog_new_with_markup(parent, 0, GTK_MESSAGE_QUESTION, GTK_BUTTONS_YES_NO, "%s", question));
...
gboolean fm_ok_cancel(GtkWindow* parent, const char* title, const char* question, gboolean default_ok) {
    int ret;
    GtkDialog* dlg = GTK_DIALOG(
        gtk_message_dialog_new_with_markup(parent, 0, GTK_MESSAGE_QUESTION, GTK_BUTTONS_OK_CANCEL, "%s", question));
    gtk_window_set_title(GTK_WINDOW(dlg), title ? title : _("Confirm"));
...
int fm_askv(GtkWindow* parent, const char* title, const char* question, char* const* options) {
    int ret;
    guint id = 1;
    GtkDialog* dlg = GTK_DIALOG(gtk_message_dialog_new_with_markup(parent, 0, GTK_MESSAGE_QUESTION, 0, "%s", question));
 ...
 * Since: 1.2.0
 */
gchar* fm_get_user_input_n(GtkWindow* parent,
                           const char* title,
                           const char* msg,
...
static gchar* fm_get_user_input_rename(GtkWindow* parent,
                                       const char* title,
                                       const char* msg,
                                       const char* default_text) {
    GtkDialog* dlg = _fm_get_user_input_dialog(parent, title, msg);
...
FmPath* fm_select_file(GtkWindow* parent,
    ...
    chooser =
        (GtkFileChooser*)gtk_file_chooser_dialog_new(title, parent, GTK_FILE_CHOOSER_ACTION_OPEN, GTK_STOCK_CANCEL,
                                                     GTK_RESPONSE_CANCEL, GTK_STOCK_OK, GTK_RESPONSE_OK, NULL);
    gtk_dialog_set_alternative_button_order(GTK_DIALOG(chooser), GTK_RESPONSE_CANCEL, GTK_RESPONSE_OK, NULL);
    if (local_only)
...
FmPath* fm_select_folder(GtkWindow* parent, const char* title) {
    FmPath* path;
    GtkFileChooser* chooser;
    chooser = (GtkFileChooser*)gtk_file_chooser_dialog_new(title ? title : _("Select Folder"), parent,
...
typedef enum { MOUNT_VOLUME, MOUNT_GFILE, UMOUNT_MOUNT, EJECT_MOUNT, EJECT_VOLUME } MountAction;
...
static gboolean fm_do_mount(GtkWindow* parent, GObject* obj, MountAction action, gboolean interactive) {
    ...
    switch (data->action) {
        ...
        case UMOUNT_MOUNT:
            ...
            g_mount_unmount_with_operation(G_MOUNT(obj), G_MOUNT_UNMOUNT_NONE, op, cancellable,
                                           on_mount_action_finished, data);
            break;
        case EJECT_MOUNT:
            prepare_unmount(G_MOUNT(obj));
            g_mount_eject_with_operation(G_MOUNT(obj), G_MOUNT_UNMOUNT_NONE, op, cancellable, on_mount_action_finished,
                                         data);
            break;
        case EJECT_VOLUME: {
            ...
            g_volume_eject_with_operation(G_VOLUME(obj), G_MOUNT_UNMOUNT_NONE, op, cancellable,
                                          on_mount_action_finished, data);
        } break;
    }
...
void fm_trash_files(GtkWindow* parent, FmPathList* files) {
    ...
    if (fm_config->confirm_trash) {
        ...
        if (len == 1) {
            ...
            msg = g_strdup_printf(dngettext(GETTEXT_PACKAGE, "Do you want to move the %d selected file to trash can?",
                                            "Do you want to move the %d selected files to trash can?", (gulong)len),
                                  len);
...
void fm_delete_files(GtkWindow* parent, FmPathList* files) {
    ...
    if (fm_config->confirm_del) {
        ...
        if (len == 1) {
            ...
            msg = g_strdup_printf(dngettext(GETTEXT_PACKAGE, "Do you want to delete the %d selected file?",
                                            "Do you want to delete the %d selected files?", (gulong)len),
                                  len);
        if (!fm_yes_no(parent, NULL, msg, TRUE)) {
            g_free(msg);
            return;
...
void fm_rename_file(GtkWindow* parent, FmPath* file) {
    ...
    new_name = fm_get_user_input_rename(parent, _("Rename File"), _("Please enter a new name:"), old_name);
...
static void assign_tooltips_from_actions(GtkWidget* widget) {
    if (G_LIKELY(GTK_IS_MENU_ITEM(widget))) {
        ...
        gtk_container_forall((GtkContainer*)widget, (GtkCallback)assign_tooltips_from_actions, NULL);
    }
}
...
void fm_widget_menu_fix_tooltips(GtkMenu* menu) {
    ...
    settings = parent ? gtk_settings_get_for_screen(gtk_widget_get_screen(parent)) : gtk_settings_get_default();
