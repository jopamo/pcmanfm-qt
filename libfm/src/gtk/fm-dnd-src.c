...
static void on_drag_begin(GtkWidget* src_widget, GdkDragContext* drag_context, FmDndSrc* ds);

static void on_drag_end(GtkWidget* src_widget, GdkDragContext* drag_context, FmDndSrc* ds);
...
static void fm_dnd_src_class_init(FmDndSrcClass* klass) {
     ...
     */
    signals[DATA_GET] =
        g_signal_new("data-get", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST, G_STRUCT_OFFSET(FmDndSrcClass, data_get),
...
void fm_dnd_src_set_widget(FmDndSrc* ds, GtkWidget* w) {
    ...
    if (w) {
        ...
        gtk_drag_source_set(w, GDK_BUTTON1_MASK, fm_default_dnd_src_targets, G_N_ELEMENTS(fm_default_dnd_src_targets),
...
static void on_drag_data_get(GtkWidget* src_widget,
    ...
    switch (info) {
        case FM_DND_SRC_TARGET_FM_LIST:
             ...
             * within the same app. */
            gtk_selection_data_set(sel_data, type, 8, (guchar*)&ds->files, sizeof(gpointer));
            break;
        ...
        case FM_DND_SRC_TARGET_TEXT: {
            ...
            if (info == FM_DND_SRC_TARGET_URI_LIST)
                gtk_selection_data_set(sel_data, type, 8, (guchar*)uri_list->str, uri_list->len);
