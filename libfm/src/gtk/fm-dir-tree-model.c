...
static void fm_dir_tree_model_class_init(FmDirTreeModelClass* klass) {
     ...
     * Since: 1.0.0
     */
    signals[ROW_LOADED] = g_signal_new("row-loaded", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                                       G_STRUCT_OFFSET(FmDirTreeModelClass, row_loaded), NULL, NULL,
                                       g_cclosure_marshal_VOID__BOXED, G_TYPE_NONE, 1, GTK_TYPE_TREE_PATH);
}
...
static void add_place_holder_child_item(FmDirTreeModel* model,
                                        GList* parent_l,
                                        GtkTreePath* parent_tp,
...
static GList* children_by_name(FmDirTreeModel* model, GList* children, const char* name, int* idx) {
    ...
    for (l = children; l; l = l->next, ++i) {
        ...
        FmPath* path;
        if (G_LIKELY(item->fi) && G_LIKELY((path = fm_file_info_get_path(item->fi))) &&
...
static void on_folder_files_removed(FmFolder* folder, GSList* files, GList* item_l) {
    ...
    for (l = files; l; l = l->next) {
        ...
        GList* rm_item_l = children_by_name(model, item->children, fm_path_get_basename(path), NULL);
        if (rm_item_l)
...
static void on_folder_files_changed(FmFolder* folder, GSList* files, GList* item_l) {
    ...
    for (l = files; l; l = l->next) {
        ...
        int idx;
        FmPath* path = fm_file_info_get_path(fi);
        GList* changed_item_l = children_by_name(model, item->children, fm_path_get_basename(path), &idx);
...
static void item_hide_hidden_children(FmDirTreeModel* model, GList* item_l) {
    ...
    if (item)
        for (child_l = item->children; child_l; child_l = next) {
            next = child_l->next;
            child = child_l->data;
...
static void item_show_hidden_children(FmDirTreeModel* model, GList* item_l) {
    ...
    while (item->hidden_children) {
        ...
        item->hidden_children = g_list_delete_link(item->hidden_children, item->hidden_children);
