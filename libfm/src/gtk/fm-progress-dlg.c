...
enum { RESPONSE_OVERWRITE = 1, RESPONSE_RENAME, RESPONSE_SKIP };
...
static FmJobErrorAction on_error(FmFileOpsJob* job, GError* err, FmJobErrorSeverity severity, FmProgressDisplay* data) {
    ...
    */
    ...
    if (data->cur_file || data->old_cur_file) {
        gtk_text_buffer_insert_with_tags(data->error_buf, &it, data->cur_file ? data->cur_file : data->old_cur_file, -1,
                                         data->bold_tag, NULL);
...
static gint on_ask_rename(FmFileOpsJob* job,
                          FmFileInfo* src,
                          FmFileInfo* dest,
                          char** new_name,
    ...
    if (disp_size) {
        tmp = g_strdup_printf(_("Type: %s\nSize: %s\nModified: %s"), fm_file_info_get_desc(src), disp_size,
                              fm_file_info_get_disp_mtime(src));
    }
    else {
        tmp =
            g_strdup_printf(_("Type: %s\nModified: %s"), fm_file_info_get_desc(src), fm_file_info_get_disp_mtime(src));
    }
    ...
    if (disp_size) {
        tmp = g_strdup_printf(_("Type: %s\nSize: %s\nModified: %s"), fm_file_info_get_desc(dest), disp_size,
        ...
        tmp = g_strdup_printf(_("Type: %s\nModified: %s"), fm_file_info_get_desc(dest),
                              fm_file_info_get_disp_mtime(dest));
    }
    ...
    if (data->dlg) {
        ...
        if (data->has_error) {
            ...
            if (fm_job_is_cancelled(FM_JOB(job))) {
                ...
                gtk_window_set_title(GTK_WINDOW(data->dlg), _("Cancelled"));
            }
            else {
                gtk_label_set_markup(data->msg, _("<b>The file operation was completed with errors.</b>"));
                gtk_window_set_title(GTK_WINDOW(data->dlg), _("Finished"));
            }
        }
        else
...
static void on_response(GtkDialog* dlg, gint id, FmProgressDisplay* data) {
    ...
    if (id == GTK_RESPONSE_CANCEL) {
        ...
        if (data->suspended) {
            ...
            gtk_button_set_image(data->suspend, gtk_image_new_from_stock(GTK_STOCK_MEDIA_PAUSE, GTK_ICON_SIZE_BUTTON));
        }
...
static gboolean on_update_dlg(gpointer user_data) {
    ...
    fm_file_size_to_str(trans_size_str, sizeof(trans_size_str), data->data_transferred_size, fm_config->si_unit);
    fm_file_size_to_str(total_size_str, sizeof(total_size_str), data->data_total_size, fm_config->si_unit);
