...
enum { PROP_0, PROP_HOME_DIR };

enum { CHDIR, ITEM_POPUP, N_SIGNALS };
...
static gboolean on_dnd_dest_files_dropped(FmDndDest* dd,
                                          int x,
                                          int y,
...
// static void on_trash_changed(GFileMonitor *monitor, GFile *gf, GFile *other, GFileMonitorEvent evt, gpointer
// user_data); static void on_use_trash_changed(FmConfig* cfg, gpointer unused); static void
// on_pane_icon_size_changed(FmConfig* cfg, gpointer unused);
...
static const char vol_menu_xml[] = PLACES_MENU_XML
    "<popup>"
    "<placeholder name='ph3'>"
...
static const char mount_menu_xml[] = PLACES_MENU_XML
    "<popup>"
    "<placeholder name='ph3'>"
    "<menuitem action='Unmount'/>"
    "</placeholder>"
...
static const char bookmark_menu_xml[] = PLACES_MENU_XML
    "<popup>"
    "<placeholder name='ph3'>"
...
static const char trash_menu_xml[] = PLACES_MENU_XML
    "<popup>"
    "<placeholder name='ph3'>"
...
enum { FM_DND_TARGET_BOOOKMARK = N_FM_DND_DEST_DEFAULT_TARGETS };

/* Target types for dragging items of list */
static const GtkTargetEntry dnd_targets[] = {{"GTK_TREE_MODEL_ROW", GTK_TARGET_SAME_WIDGET, FM_DND_TARGET_BOOOKMARK}};
...
static gboolean on_drag_motion(GtkWidget* dest_widget, GdkDragContext* drag_context, gint x, gint y, guint time) {
...
static void on_drag_leave(GtkWidget* dest_widget, GdkDragContext* drag_context, guint time) {
    gtk_tree_view_set_drag_dest_row(GTK_TREE_VIEW(dest_widget), NULL, 0);
    /* g_debug("drag_leave"); */
}

static gboolean on_drag_drop(GtkWidget* dest_widget, GdkDragContext* drag_context, gint x, gint y, guint time) {
    /* this is to reorder bookmark */
    if (gtk_drag_dest_find_target(dest_widget, drag_context, NULL) == tree_model_row_atom) {
                    ...
                    if (G_UNLIKELY(gtk_tree_path_compare(src_tp, dest_tp) == 0))
                        ...
                        if (item) {
                            ...
                            if (gtk_tree_model_get_iter(GTK_TREE_MODEL(model), &dest_it, dest_tp)) {
                                ...
                                fm_bookmarks_reorder(fm_places_model_get_bookmarks(model),
                                                     fm_places_item_get_bookmark_item(item), new_pos);
...
static gboolean on_dnd_dest_files_dropped(FmDndDest* dd,
    ...
    /* g_debug("action= %d, %d files-dropped!, dest=%p info_type: %d", action, fm_path_list_get_length(files), dest,
     * info_type); */
    ...
    if (!dest && action == GDK_ACTION_LINK) /* add bookmarks */
    {
        ...
        if (get_bookmark_drag_dest(view, &tp, &pos)) {
            ...
            for (l = fm_path_list_peek_head_link(files); l; l = l->next, ++idx) {
                ...
                if (g_file_query_file_type(gf, G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS, NULL) == G_FILE_TYPE_DIRECTORY) {
...
static void fm_places_view_init(FmPlacesView* self) {
    ...
    handler =
        g_signal_connect(fm_config, "changed::pane_icon_size", G_CALLBACK(on_renderer_icon_size_changed), renderer);
    g_object_weak_ref(G_OBJECT(renderer), on_cell_renderer_pixbuf_destroy, GUINT_TO_POINTER(handler));
    fm_cell_renderer_pixbuf_set_fixed_size((FmCellRendererPixbuf*)renderer, fm_config->pane_icon_size,
                                           fm_config->pane_icon_size);

    gtk_tree_view_column_pack_start(col, renderer, FALSE);
    gtk_tree_view_column_set_attributes(col, renderer, "pixbuf", FM_PLACES_MODEL_COL_ICON, NULL);

    renderer = gtk_cell_renderer_text_new();
    gtk_tree_view_column_pack_start(col, renderer, TRUE);
    g_object_set(renderer, "ellipsize", PANGO_ELLIPSIZE_END, NULL);
    gtk_tree_view_column_set_attributes(col, renderer, "text", FM_PLACES_MODEL_COL_LABEL, NULL);

    renderer = gtk_cell_renderer_pixbuf_new();
    self->mount_indicator_renderer = GTK_CELL_RENDERER_PIXBUF(renderer);
    gtk_tree_view_column_pack_start(col, renderer, FALSE);
    gtk_cell_layout_set_cell_data_func(GTK_CELL_LAYOUT(col), renderer, fm_places_model_mount_indicator_cell_data_func,
                                       NULL, NULL);

    gtk_tree_view_append_column(GTK_TREE_VIEW(self), col);

    gtk_tree_view_enable_model_drag_source(GTK_TREE_VIEW(self), GDK_BUTTON1_MASK, dnd_targets,
                                           G_N_ELEMENTS(dnd_targets), GDK_ACTION_MOVE);
        ...
        switch (fm_places_item_get_type(item)) {
            ...
            case FM_PLACES_ITEM_VOLUME: {
                ...
                if (!mnt) {
                    ...
                    if (!mnt) {
                        g_debug(
                            "GMount is invalid after successful g_volume_mount().\nThis is quite possibly a gvfs "
...
static GtkWidget* place_item_get_menu(FmPlacesItem* item, GtkWidget* widget) {
    ...
    if (fm_places_item_get_type(item) == FM_PLACES_ITEM_PATH) {
        if (fm_places_item_get_bookmark_item(item)) {
            ...
            if (fm_places_model_get_iter_by_fm_path(model, &src_it, fm_places_item_get_path(item))) {
                /* get index of the separator */
                sep_tp = fm_places_model_get_separator_path(model);
                sep_pos = gtk_tree_path_get_indices(sep_tp)[0];
                tp = gtk_tree_model_get_path(GTK_TREE_MODEL(model), &src_it);
                if (!gtk_tree_path_prev(tp) || gtk_tree_path_get_indices(tp)[0] - sep_pos - 1 < 0) {
        ...
        else /* not mounted */
        {
            if (fm_config->format_cmd && fm_config->format_cmd[0])
                unix_path = g_volume_get_identifier(vol, G_VOLUME_IDENTIFIER_KIND_UNIX_DEVICE);
...
static void fm_places_item_popup(GtkWidget* widget, GtkTreeIter* it, guint32 time) {
    if (!fm_places_model_iter_is_separator(model, it)) {
        ...
        if (menu) {
            ...
            gtk_menu_popup(menu, NULL, NULL, popup_position_func, GINT_TO_POINTER(indices[0]), 3, time);
            gtk_tree_path_free(path);
        }
    }
}
...
static void on_eject(GtkAction* act, gpointer user_data) {
    FmPlacesItem* item = (FmPlacesItem*)user_data;
    if (fm_places_item_get_type(item) == FM_PLACES_ITEM_VOLUME) {
        fm_eject_volume(_get_gtk_window_from_action(act), fm_places_item_get_volume(item), TRUE);
        /* NOTE: the most probably FmPlacesView is destroyed at this point */
    }
}
...
static void on_format(GtkAction* act, gpointer user_data) {
    ...
    if (fm_config->format_cmd == NULL || fm_config->format_cmd[0] == 0)
        return;
    unix_path = g_volume_get_identifier(fm_places_item_get_volume(item), G_VOLUME_IDENTIFIER_KIND_UNIX_DEVICE);
    if (unix_path) {
        ...
        fm_path_unref(path);
        fm_launch_command_simple(_get_gtk_window_from_action(act), NULL, 0, fm_config->format_cmd, paths);
...
static void on_rename_bm(GtkAction* act, gpointer user_data) {
    FmPlacesItem* item = (FmPlacesItem*)user_data;
    char* new_name = fm_get_user_input(_get_gtk_window_from_action(act), _("Rename Bookmark Item"),
...
static void on_move_bm_up(GtkAction* act, gpointer user_data) {
    ...
    sep_pos = gtk_tree_path_get_indices(sep_tp)[0];
    tp = gtk_tree_model_get_path(GTK_TREE_MODEL(model), &src_it);
    if (!gtk_tree_path_prev(tp) || (new_pos = gtk_tree_path_get_indices(tp)[0] - sep_pos - 1) < 0 ||
...
static void fm_places_view_set_property(GObject* object, guint prop_id, const GValue* value, GParamSpec* pspec) {
...
static void fm_places_view_get_property(GObject* object, guint prop_id, GValue* value, GParamSpec* pspec) {
    FmPlacesView* view = FM_PLACES_VIEW(object);
...
static void fm_places_view_class_init(FmPlacesViewClass* klass) {
    ...
    g_object_class_install_property(
        g_object_class, PROP_HOME_DIR,
        g_param_spec_string("home-dir-path", "Home item directory", "What directory path will be used for Home item",
     ...
     */
    signals[CHDIR] =
        g_signal_new("chdir", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(FmPlacesViewClass, chdir),
     ...
     * Since: 1.2.0
     */
    signals[ITEM_POPUP] = g_signal_new(
        "item-popup", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(FmPlacesViewClass, item_popup), NULL,
        NULL, fm_marshal_VOID__OBJECT_OBJECT_POINTER, G_TYPE_NONE, 3, G_TYPE_OBJECT, G_TYPE_OBJECT, G_TYPE_POINTER);
