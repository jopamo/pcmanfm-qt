...
G_DEFINE_TYPE_WITH_CODE(FmStandardView,
                        fm_standard_view,
...
static void on_row_changed(GtkTreeModel* tree_model, GtkTreePath* path, GtkTreeIter* iter, FmStandardView* fv) {
    if (fv->mode == FM_FV_LIST_VIEW)
        _reset_columns_widths(GTK_TREE_VIEW(fv->view));
}
static void on_row_deleted(GtkTreeModel* tree_model, GtkTreePath* path, FmStandardView* fv) {
    if (fv->mode == FM_FV_LIST_VIEW)
        _reset_columns_widths(GTK_TREE_VIEW(fv->view));
    g_object_set(G_OBJECT(fv->view), "tooltip-text", NULL, NULL);
}
static void on_row_inserted(GtkTreeModel* tree_model, GtkTreePath* path, GtkTreeIter* iter, FmStandardView* fv) {
    if (fv->mode == FM_FV_LIST_VIEW)
        _reset_columns_widths(GTK_TREE_VIEW(fv->view));
}
...
static void on_show_full_names_changed(FmConfig* cfg, FmStandardView* fv) {
    ...
    if (fm_config->show_full_names)
        ...
        font_height =
            (pango_font_metrics_get_ascent(metrics) + pango_font_metrics_get_descent(metrics)) / PANGO_SCALE + 1;
    ...
    gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(fv->view), GTK_CELL_RENDERER(fv->renderer_text), "text",
                                   FM_FOLDER_MODEL_COL_NAME, NULL);
}
...
static inline void create_icon_view(FmStandardView* fv, GList* sels) {
    ...
    if (fv->mode == FM_FV_COMPACT_VIEW) {
        fv->icon_size_changed_handler =
            g_signal_connect(fm_config, "changed::small_icon_size", G_CALLBACK(on_small_icon_size_changed), fv);
        ...
        g_object_set((GObject*)render, "xalign", 1.0, "yalign", 0.5, NULL);
        ...
        if (fv->show_full_names_handler == 0)
            fv->show_full_names_handler =
                g_signal_connect(fm_config, "changed::show_full_names", G_CALLBACK(on_show_full_names_changed), fv);
        if (fm_config->show_full_names)
            font_height = 0;
        else {
            PangoContext* pc = gtk_widget_get_pango_context((GtkWidget*)fv);
        ...
        if (fv->mode == FM_FV_ICON_VIEW) {
            fv->icon_size_changed_handler =
                g_signal_connect(fm_config, "changed::big_icon_size", G_CALLBACK(on_big_icon_size_changed), fv);
            ...
            g_object_set((GObject*)render, "wrap-mode", PANGO_WRAP_WORD_CHAR, "wrap-width", item_width, "max-height",
                         font_height * 3, "alignment", PANGO_ALIGN_CENTER, "xalign", 0.5, "yalign", 0.0, NULL);
            exo_icon_view_set_column_spacing((ExoIconView*)fv->view, 4);
        }
        else {
            fv->icon_size_changed_handler =
                g_signal_connect(fm_config, "changed::thumbnail_size", G_CALLBACK(on_thumbnail_size_changed), fv);
            ...
            g_object_set((GObject*)render, "wrap-mode", PANGO_WRAP_WORD_CHAR, "wrap-width", item_width, "max-height",
                         font_height * 5, "alignment", PANGO_ALIGN_CENTER, "xalign", 0.5, "yalign", 0.0, NULL);
            exo_icon_view_set_column_spacing((ExoIconView*)fv->view, 8);
        }
    }
    gtk_cell_layout_pack_start(GTK_CELL_LAYOUT(fv->view), render, TRUE);
    gtk_cell_layout_add_attribute(GTK_CELL_LAYOUT(fv->view), render, "text", FM_FOLDER_MODEL_COL_NAME);
    if (fv->renderer_text)
    ...
    exo_icon_view_set_single_click_timeout((ExoIconView*)fv->view, fm_config->auto_selection_delay);
...
static void on_column_width_changed(GtkTreeViewColumn* col, GParamSpec* pspec, FmStandardView* view) {
...
static void on_column_move_left(GtkMenuItem* menu_item, GtkTreeViewColumn* col) {
    ...
    if (l && l->prev) {
        gtk_tree_view_move_column_after(view, col, l->prev->prev ? l->prev->prev->data : NULL);
...
static gboolean on_column_button_press_event(GtkWidget* button, GdkEventButton* event, GtkTreeViewColumn* col) {
    if (event->button == 3) {
        GtkWidget* view = gtk_tree_view_column_get_tree_view(col);
        GtkWidget* fv = gtk_widget_get_parent(view);
        GList *columns, *l;
...
static gboolean on_column_button_released_event(GtkWidget* button, GdkEventButton* event, GtkTreeViewColumn* col) {
    if (event->button == 1) {
        GtkWidget* view = gtk_tree_view_column_get_tree_view(col);
        GtkWidget* fv = gtk_widget_get_parent(view);
        FmFolderViewColumnInfo* info = g_object_get_qdata(G_OBJECT(col), fm_qdata_id);
...
static GtkTreeViewColumn* create_list_view_column(FmStandardView* fv, FmFolderViewColumnInfo* set) {
    ...
    switch (col_id) {
        case FM_FOLDER_MODEL_COL_NAME:
            ...
            gtk_tree_view_column_set_attributes(col, GTK_CELL_RENDERER(fv->renderer_pixbuf), "pixbuf",
                                                FM_FOLDER_MODEL_COL_ICON, "info", FM_FOLDER_MODEL_COL_INFO, NULL);
...
#endif
    if (label) {
        g_signal_connect(label, "button-press-event", G_CALLBACK(on_column_button_press_event), col);
        g_signal_connect(label, "button-release-event", G_CALLBACK(on_column_button_released_event), col);
    }
    return col;
}
static void _check_tree_columns_defaults(FmStandardView* fv) {
    const FmFolderViewColumnInfo cols[] = {{FM_FOLDER_MODEL_COL_NAME},
...
static inline void create_list_view(FmStandardView* fv, GList* sels) {
    ...
    exo_tree_view_set_single_click_timeout((ExoTreeView*)fv->view, fm_config->auto_selection_delay);
...
void fm_standard_view_set_mode(FmStandardView* fv, FmStandardViewMode mode) {
    if (mode != fv->mode) {
        ...
        fm_dnd_set_dest_auto_scroll(fv->view, gtk_scrolled_window_get_hadjustment((GtkScrolledWindow*)fv),
...
static gboolean on_btn_pressed(GtkWidget* view, GdkEventButton* evt, FmStandardView* fv) {
    ...
    if (evt->type == GDK_BUTTON_PRESS) {
                ...
                case FM_FV_THUMBNAIL_VIEW:
                    if (exo_icon_view_get_item_at_pos(EXO_ICON_VIEW(view), evt->x, evt->y, &tp, NULL)) {
                        if (!exo_icon_view_path_is_selected(EXO_ICON_VIEW(view), tp)) {
                            ...
                            if (sels) {
                                exo_icon_view_unselect_all(EXO_ICON_VIEW(view));
                                g_list_foreach(sels, (GFunc)gtk_tree_path_free, NULL);
                                g_list_free(sels);
                            }
                            exo_icon_view_select_path(EXO_ICON_VIEW(view), tp);
        ...
        if (evt->button == 2 && !fm_config->middle_click)
...
static void on_sel_changed(GObject* obj, FmStandardView* fv) {
    if (!fv->sel_changed_idle) {
        fv->sel_changed_idle = gdk_threads_add_timeout_full(G_PRIORITY_HIGH_IDLE, 200, on_sel_changed_idle, fv, NULL);
...
static void fm_standard_view_select_file_path(FmFolderView* ffv, FmPath* path) {
    ...
    if (cwd && fm_path_equal(fm_path_get_parent(path), cwd)) {
        FmFolderModel* model = fv->model;
        GtkTreeIter it;
        if (fv->select_path && fm_folder_model_find_iter_by_filename(model, &it, fm_path_get_basename(path)))
...
static gboolean _fm_standard_view_set_columns(FmFolderView* fv, const GSList* cols) {
    ...
    for (i = 0; i < n_cols; i++)
        if (old_cols[i].col != NULL)
            gtk_tree_view_remove_column(GTK_TREE_VIEW(view->view), old_cols[i].col);
    if (n_cols > 0)
        g_free(old_cols);
    return TRUE;
}
...
static void _fm_standard_view_scroll_to_path(FmFolderView* fv, FmPath* path, gboolean focus) {
    ...
    if (!FM_IS_STANDARD_VIEW(fv) || path == NULL)
        return;
    view = (FmStandardView*)fv;
    if (!fm_folder_model_find_iter_by_filename(view->model, &it, fm_path_get_basename(path)))
        return;
