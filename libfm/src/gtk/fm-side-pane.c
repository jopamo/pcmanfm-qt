...
enum { CHDIR, MODE_CHANGED, N_SIGNALS };
...
static void fm_side_pane_class_init(FmSidePaneClass* klass) {
     ...
     */
    signals[CHDIR] =
        g_signal_new("chdir", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(FmSidePaneClass, chdir),
     ...
     * Since: 0.1.12
     */
    signals[MODE_CHANGED] = g_signal_new("mode-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                                         G_STRUCT_OFFSET(FmSidePaneClass, mode_changed), NULL, NULL,
...
static void on_menu_btn_clicked(GtkButton* btn, FmSidePane* sp) {
    gtk_menu_popup((GtkMenu*)sp->menu, NULL, NULL, menu_position_func, btn, 1, gtk_get_current_event_time());
}
...
static void fm_side_pane_init(FmSidePane* sp) {
    ...
    gtk_box_pack_start(GTK_BOX(hbox), gtk_arrow_new(GTK_ARROW_DOWN, GTK_SHADOW_NONE), FALSE, TRUE, 0);
    ...
    gtk_action_group_add_radio_actions(act_grp, menu_actions, G_N_ELEMENTS(menu_actions), -1,
                                       G_CALLBACK(on_mode_changed), sp);
...
static void on_item_popup(GtkWidget* view,
                          GtkUIManager* ui,
...
void fm_side_pane_set_mode(FmSidePane* sp, FmSidePaneMode mode) {
    ...
    switch (mode) {
        case FM_SP_PLACES:
            ...
            gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(sp->scroll), GTK_POLICY_NEVER, GTK_POLICY_AUTOMATIC);
        ...
        case FM_SP_DIR_TREE:
            ...
            gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(sp->scroll), GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC);
    ...
    gtk_radio_action_set_current_value((GtkRadioAction*)gtk_ui_manager_get_action(sp->ui, "/popup/Places"), sp->mode);
}
 ...
 * Since: 1.2.0
 */
void fm_side_pane_set_popup_updater(FmSidePane* sp, FmSidePaneUpdatePopup update_popup, gpointer user_data) {
