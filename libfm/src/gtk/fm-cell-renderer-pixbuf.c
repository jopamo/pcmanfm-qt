...
static void fm_cell_renderer_pixbuf_get_property(GObject* object, guint param_id, GValue* value, GParamSpec* pspec);
...
static void fm_cell_renderer_pixbuf_class_init(FmCellRendererPixbufClass* klass) {
     ...
     */
    g_object_class_install_property(g_object_class, PROP_INFO,
...
static void fm_cell_renderer_pixbuf_get_size(GtkCellRenderer* cell,
    ...
    if (render->fixed_w > 0 && render->fixed_h > 0) {
        ...
        *height = render->fixed_h;
        ...
        GTK_CELL_RENDERER_CLASS(fm_cell_renderer_pixbuf_parent_class)
            ->get_size(cell, widget, rectangle, x_offset, y_offset, width, height);
    }
}
...
#endif
{
    ...
    if (render->fi && G_UNLIKELY(fm_file_info_is_symlink(render->fi) || fm_file_info_is_shortcut(render->fi))) {
        GdkPixbuf* pix;
        ...
        if (pix) {
            ...
            if (cell_area->width >= 20 && render->fixed_w >= 20)
                ...
                GdkPixbuf* scaled = gdk_pixbuf_scale_simple(link_icon, scale_width, scale_width, GDK_INTERP_TILES);
