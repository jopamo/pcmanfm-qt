...
#define fm_drag_context_has_target(ctx, target) (g_list_find(gdk_drag_context_list_targets(ctx), target) != NULL)

#define fm_drag_context_has_target_name(ctx, name) fm_drag_context_has_target(ctx, gdk_atom_intern_static_string(name))
...
#define fm_dnd_dest_add_targets(widget, targets, n) \
    gtk_target_list_add_table(gtk_drag_dest_get_target_list(widget), targets, n)
