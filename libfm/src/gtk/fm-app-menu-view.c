...
enum { COL_ICON, COL_TITLE, COL_ITEM, N_COLS };
...
#endif
    {
        ...
        switch (menu_cache_item_get_type(item)) {
            ...
            case MENU_CACHE_TYPE_DIR:
                ...
                gtk_tree_store_set(store, &it, COL_ICON, gicon, COL_TITLE, menu_cache_item_get_name(item), COL_ITEM,
                                   item, -1);
                if (gicon)
...
GtkTreeView* fm_app_menu_view_new(void) {
    ...
    if (!store) {
        ...
        if (G_UNLIKELY(!menu_cache_item_type))
            menu_cache_item_type = g_boxed_type_register_static("MenuCacheItem", (GBoxedCopyFunc)menu_cache_item_ref,
