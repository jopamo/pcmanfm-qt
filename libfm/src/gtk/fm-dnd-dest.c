...
static gboolean fm_dnd_dest_files_dropped(FmDndDest* dd,
                                          int x,
                                          int y,
                                          guint action,
                                          guint info_type,
...
static void on_drag_leave(GtkWidget* widget, GdkDragContext* drag_context, guint time, FmDndDest* dd);
static gboolean on_drag_drop(GtkWidget* widget,
                             ...
                             guint time,
                             FmDndDest* dd);
static void on_drag_data_received(GtkWidget* w,
                                  GdkDragContext* drag_context,
                                  gint x,
                                  gint y,
...
static void fm_dnd_dest_class_init(FmDndDestClass* klass) {
     ...
     * Since: 0.1.0
     */
    signals[FILES_DROPPED] = g_signal_new(
        "files-dropped", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(FmDndDestClass, files_dropped),
        g_signal_accumulator_true_handled, NULL, fm_marshal_BOOL__INT_INT_UINT_UINT_POINTER, G_TYPE_BOOLEAN, 5,
        G_TYPE_INT, G_TYPE_INT, G_TYPE_UINT, G_TYPE_UINT, G_TYPE_POINTER);
...
void fm_dnd_dest_set_widget(FmDndDest* dd, GtkWidget* w) {
    ...
    if (w) {
        ...
        gtk_drag_dest_set(w, 0, fm_default_dnd_dest_targets, G_N_ELEMENTS(fm_default_dnd_dest_targets),
...
static int fm_dnd_dest_can_receive_drop(FmFileInfo* dest_fi, FmPath* dest, FmPath* src) {
...
static GtkActionEntry drop_menu_actions[] = {{"Cancel", NULL, N_("_Cancel"), NULL, NULL, G_CALLBACK(on_cancel_sel)},
...
static GdkDragAction _ask_action_on_drop(GtkWidget* widget, GdkDragContext* drag_context, gboolean link_only) {
    ...
    unmap_handler = g_signal_connect(ri.menu, "unmap", G_CALLBACK(run_unmap_handler), &ri);
    destroy_handler = g_signal_connect(ri.menu, "destroy", G_CALLBACK(run_destroy_handler), &ri);
...
static gboolean fm_dnd_dest_files_dropped(FmDndDest* dd,
    ...
    can_drop = fm_dnd_dest_can_receive_drop(dd->dest_file, dest, fm_path_list_peek_head(files));
    if (can_drop == 0)
        return FALSE;
    ...
    if (fm_file_info_is_desktop_entry(dd->dest_file)) {
        ...
        return fm_launch_desktop_entry_simple(GTK_WINDOW(parent), NULL, dd->dest_file, files);
    }
 ...
 * Since: 0.1.17
 */
static inline gboolean _on_drag_data_received(FmDndDest* dd,
                                              GdkDragContext* drag_context,
                                              gint x,
                                              gint y,
    ...
    if (info == FM_DND_DEST_TARGET_FM_LIST) {
        ...
        if ((length >= 0) && (format == 8)) {
            ...
            if (files && !fm_path_list_is_empty(files)) {
                ...
                const char* attr =
                    fm_path_is_native(path) ? G_FILE_ATTRIBUTE_UNIX_DEVICE : G_FILE_ATTRIBUTE_ID_FILESYSTEM;
                ...
                if (fm_path_is_native(path)) {
                    ...
                    dd->src_fs_id =
                        g_intern_string(g_file_info_get_attribute_string(inf, G_FILE_ATTRIBUTE_ID_FILESYSTEM));
                else
                    dd->src_fs_id = NULL;
                if (inf)
                    g_object_unref(inf);
    ...
    else if (info == FM_DND_DEST_TARGET_XDS) /* X direct save */
    {
        ...
        if (format == 8 && length == 1 && data[0] == 'F') {
            ...
            gdk_property_change(source_window, dest_target_atom[FM_DND_DEST_TARGET_XDS],
                                gdk_atom_intern_static_string("text/plain"), 8, GDK_PROP_MODE_REPLACE,
                                (const guchar*)"", 0);
        }
        else if (format == 8 && length == 1 && data[0] == 'S') {
            success = TRUE;
        }
        gtk_drag_finish(drag_context, success, gdk_drag_context_get_selected_action(drag_context) == GDK_ACTION_MOVE,
                        time);
        return TRUE;
    }
...
GdkAtom fm_dnd_dest_find_target(FmDndDest* dd, GdkDragContext* drag_context) {
    ...
    for (i = 1; i < N_FM_DND_DEST_DEFAULT_TARGETS; i++) {
        ...
        if (G_LIKELY(target != GDK_NONE) &&
            ...
            && (i != FM_DND_DEST_TARGET_FM_LIST || gtk_drag_get_source_widget(drag_context)))
            return target;
    }
    return GDK_NONE;
}
 ...
 * Since: 0.1.17
 */
static inline gboolean _on_drag_drop(FmDndDest* dd,
                                     GdkDragContext* drag_context,
                                     GdkAtom target,
                                     int x,
                                     int y,
    ...
    if (ret) /* we support this kind of target */
    {
        if (i == FM_DND_DEST_TARGET_XDS) /* if this is XDS */
        {
            ...
            source_window = gdk_drag_context_get_source_window(drag_context);
            if (gdk_property_get(source_window, target, text_atom, 0, 1024, FALSE, NULL, NULL, &len, &data) && data) {
                ...
                if (dest && fm_file_info_is_dir(dest)) {
                    ...
                    /* setup the property */
                    gdk_property_change(source_window, target, text_atom, 8, GDK_PROP_MODE_REPLACE, (const guchar*)uri,
                                        strlen(uri));
                    fm_path_unref(path);
                    g_free(uri);
                }
                ...
                fm_show_error(GTK_WINDOW(gtk_widget_get_toplevel(dest_widget)), NULL, _("XDirectSave failed."));
                gdk_property_change(source_window, target, text_atom, 8, GDK_PROP_MODE_REPLACE, (const guchar*)"", 0);
            }
            g_free(data);
 ...
 * Since: 0.1.17
 */
GdkDragAction fm_dnd_dest_get_default_action(FmDndDest* dd, GdkDragContext* drag_context, GdkAtom target) {
    GdkDragAction action;
    ...
    if (fm_file_info_is_desktop_entry(dest)) {
        ...
        gdk_window_get_device_position(gtk_widget_get_window(dd->widget), gdk_drag_context_get_device(drag_context),
                                       NULL, NULL, &mask);
    ...
    else /* we have got drag source files */
    {
        ...
        if (can_drop < 0 && fm_config->drop_default_action == FM_DND_DEST_DROP_ASK)
            ...
            /* make decision based on config: Auto / Copy / Move / Ask */
                ...
                switch ((FmDndDestDropAction)fm_config->drop_default_action) {
