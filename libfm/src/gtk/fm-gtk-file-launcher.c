...
static gboolean on_launch_error(GAppLaunchContext* ctx, GError* err, FmPath* path, gpointer user_data) {
    LaunchData* data = (LaunchData*)user_data;
...
static FmFileLauncherExecAction on_exec_file(FmFileInfo* file, gpointer user_data) {
    ...
    if (fm_file_info_is_text(file)) {
        msg_str =
            g_strdup_printf(_("This text file '%s' seems to be an executable script.\nWhat do you want to do with it?"),
...
gboolean fm_launch_files_simple(GtkWindow* parent,
                                ...
                                gpointer user_data) {
    FmFileLauncher launcher = {.get_app = choose_app,
    ...
    if (ctx == NULL) {
        ...
        gdk_app_launch_context_set_screen(
            _ctx, parent ? gtk_widget_get_screen(GTK_WIDGET(parent)) : gdk_screen_get_default());
...
gboolean fm_launch_desktop_entry_simple(GtkWindow* parent,
    ...
    FmFileLauncher launcher = {
        .get_app = NULL, .open_folder = NULL, .exec_file = NULL, .error = on_launch_error, .ask = on_launch_ask};
 ...
 * Since: 1.2.0
 */
gboolean fm_launch_command_simple(GtkWindow* parent,
                                  GAppLaunchContext* ctx,
...
static gboolean launch_search(FileSearchUI* ui) {
    ...
    if (gtk_tree_model_get_iter_first(model, &it)) /* we need to have at least one dir path */
    {
        ...
        if (ui->dir_file_checkbutton && gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(ui->dir_file_checkbutton)))
            mime_types = g_slist_prepend(mime_types, (gpointer) "inode/directory");
        if (ui->other_file_checkbutton && gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(ui->other_file_checkbutton)) &&
        ...
        if (ui->ctx == NULL) {
            _ctx = gdk_display_get_app_launch_context(gdk_display_get_default());
            gdk_app_launch_context_set_screen(
                _ctx, ui->parent ? gtk_widget_get_screen(GTK_WIDGET(ui->parent)) : gdk_screen_get_default());
            gdk_app_launch_context_set_timestamp(_ctx, gtk_get_current_event_time());
            /* FIXME: how to handle gdk_app_launch_context_set_icon? */
...
static void on_add_path_button_clicked(GtkButton* btn, gpointer user_data) {
    FileSearchUI* ui = (FileSearchUI*)user_data;
    GtkWidget* dlg =
        gtk_file_chooser_dialog_new(_("Select Folder"), GTK_WINDOW(ui->dlg), GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER,
...
gboolean fm_launch_search_simple(GtkWindow* parent,
    ...
    ui->name_case_insensitive_checkbutton =
        GTK_CHECK_BUTTON(gtk_builder_get_object(builder, "name_case_insensitive_checkbutton"));
    ui->name_regex_checkbutton = GTK_CHECK_BUTTON(gtk_builder_get_object(builder, "name_regex_checkbutton"));
    ui->search_recursive_checkbutton =
        GTK_CHECK_BUTTON(gtk_builder_get_object(builder, "search_recursive_checkbutton"));
    ui->search_hidden_files_checkbutton =
        GTK_CHECK_BUTTON(gtk_builder_get_object(builder, "search_hidden_files_checkbutton"));
    ...
    if (ui->other_file_checkbutton) {
        ...
        g_signal_connect(ui->other_file_checkbutton, "toggled", G_CALLBACK(on_other_file_checkbutton_toggled), ui);
    }

    ui->content_entry = GTK_ENTRY(gtk_builder_get_object(builder, "content_entry"));
    ui->content_case_insensitive_checkbutton =
        GTK_CHECK_BUTTON(gtk_builder_get_object(builder, "content_case_insensitive_checkbutton"));
