...
static void fm_file_info_job_class_init(FmFileInfoJobClass* klass) {
     ...
     * Since: 1.2.0
     */
    signals[GOT_INFO] = g_signal_new("got-info", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                                     G_STRUCT_OFFSET(FmFileInfoJobClass, got_info), NULL, NULL,
...
static void _check_gfile_display_names(FmPath* path, GFile* child) {
    ...
    if (gf == NULL) /* file systems such as search:// don't support this */
        return;
    inf = g_file_query_info(gf, G_FILE_ATTRIBUTE_STANDARD_DISPLAY_NAME "," G_FILE_ATTRIBUTE_STANDARD_EDIT_NAME,
