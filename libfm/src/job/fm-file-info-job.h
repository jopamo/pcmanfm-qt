...
#define FM_FILE_INFO_JOB_CLASS(klass) (G_TYPE_CHECK_CLASS_CAST((klass), FM_TYPE_FILE_INFO_JOB, FmFileInfoJobClass))
#define FM_IS_FILE_INFO_JOB(obj) (G_TYPE_CHECK_INSTANCE_TYPE((obj), FM_TYPE_FILE_INFO_JOB))
#define FM_IS_FILE_INFO_JOB_CLASS(klass) (G_TYPE_CHECK_CLASS_TYPE((klass), FM_TYPE_FILE_INFO_JOB))
...
static inline gboolean _fm_file_info_job_get_info_for_native_file(FmJob* job,
                                                                  FmFileInfo* fi,
                                                                  const char* path,
                                                                  GError** err) {
...
static inline gboolean _fm_file_info_job_update_fs_readonly(GFile* gf,
                                                            GFileInfo* inf,
                                                            GCancellable* cancellable,
                                                            GError** error) {
    /* check if FS is R/O and set attr. into inf */
    GFileInfo* fs_inf = g_file_query_filesystem_info(gf, G_FILE_ATTRIBUTE_FILESYSTEM_READONLY, cancellable, error);
    if (fs_inf) {
        if (g_file_info_has_attribute(fs_inf, G_FILE_ATTRIBUTE_FILESYSTEM_READONLY))
            g_file_info_set_attribute_boolean(
                inf, G_FILE_ATTRIBUTE_FILESYSTEM_READONLY,
...
static inline gboolean _fm_file_info_job_get_info_for_gfile(FmJob* job, FmFileInfo* fi, GFile* gf, GError** err) {
    GFileInfo* inf;
