...
gboolean _fm_file_ops_job_delete_file(FmJob* job, GFile* gf, GFileInfo* inf, FmFolder* folder, gboolean only_empty) {
    GError* err = NULL;
    ...
    while (!inf) {
        _inf = inf =
            g_file_query_info(gf, query, G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS, fm_job_get_cancellable(job), &err);
        if (_inf)
            break;
    ...
    while (!fm_job_is_cancelled(job)) {
        ...
        if (err) {
            ...
            if (is_trash_root || /* FIXME: need to refactor this! */
                (is_dir && !only_empty && err->domain == G_IO_ERROR && err->code == G_IO_ERROR_NOT_EMPTY)) {
                GFileEnumerator* enu;
                FmFolder* sub_folder;

                g_error_free(err);
                err = NULL;
                enu = g_file_enumerate_children(gf, query, G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS,
...
gboolean _fm_file_ops_job_trash_run(FmFileOpsJob* job) {
    ...
    for (; !fm_job_is_cancelled(fmjob) && l; l = l->next) {
        ...
        parent = fm_path_get_parent(path);
    _retry_trash:
        inf = g_file_query_info(gf, G_FILE_ATTRIBUTE_STANDARD_DISPLAY_NAME, 0, fm_job_get_cancellable(fmjob), &err);
        if (inf) {
            /* currently processed file. */
...
static const char trash_query[] = G_FILE_ATTRIBUTE_STANDARD_TYPE
...
gboolean _fm_file_ops_job_untrash_run(FmFileOpsJob* job) {
    ...
    _retry_get_orig_path:
        inf = g_file_query_info(gf, trash_query, G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS, fm_job_get_cancellable(fmjob),
                                &err);
        if (inf) {
            ...
            if (orig_path_str) {
                ...
                g_set_error(&err, G_IO_ERROR, G_IO_ERROR_FAILED, _("Cannot untrash file '%s': original path not known"),
