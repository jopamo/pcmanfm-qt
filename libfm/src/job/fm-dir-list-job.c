...
static void fm_dir_list_job_class_init(FmDirListJobClass* klass) {
     ...
     * Since: 1.0.2
     */
    signals[FILES_FOUND] = g_signal_new("files-found", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                                        G_STRUCT_OFFSET(FmDirListJobClass, files_found), NULL, NULL,
...
static inline FmFileInfo* _new_info_for_native_file(FmDirListJob* job,
                                                    FmPath* path,
                                                    const char* path_str,
                                                    GError** err) {
    FmFileInfo* fi;
...
static gboolean fm_dir_list_job_run_posix(FmDirListJob* job) {
    ...
    if (fi) {
        if (!fm_file_info_is_dir(fi)) {
            err = g_error_new(G_IO_ERROR, G_IO_ERROR_NOT_DIRECTORY, _("The specified directory '%s' is not valid"),
                              path_str);
            fm_file_info_unref(fi);
        ...
        err =
            g_error_new(G_IO_ERROR, G_IO_ERROR_NOT_DIRECTORY, _("The specified directory '%s' is not valid"), path_str);
        fm_job_emit_error(fmjob, err, FM_JOB_ERROR_CRITICAL);
        g_error_free(err);
        g_free(path_str);
        return FALSE;
...
_retry:
    ...
    if (G_UNLIKELY(job->flags & FM_DIR_LIST_JOB_DIR_ONLY)) {
        query = G_FILE_ATTRIBUTE_STANDARD_TYPE
    ...
    if (enu) {
        while (!fm_job_is_cancelled(fmjob)) {
            ...
            if (inf) {
                ...
                child = g_file_get_child(g_file_enumerator_get_container(enu), g_file_info_get_name(inf));
                if (g_file_info_get_file_type(inf) == G_FILE_TYPE_DIRECTORY)
...
static gpointer queue_add_file(FmJob* fmjob, gpointer user_data) {
    ...
    if (job->delay_add_files_handler == 0)
        job->delay_add_files_handler =
            g_timeout_add_seconds_full(G_PRIORITY_LOW, 1, emit_found_files, g_object_ref(job), g_object_unref);
    return NULL;
}
