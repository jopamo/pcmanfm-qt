...
static const char query[] = G_FILE_ATTRIBUTE_STANDARD_TYPE
    "," G_FILE_ATTRIBUTE_STANDARD_DISPLAY_NAME "," G_FILE_ATTRIBUTE_STANDARD_NAME
    "," G_FILE_ATTRIBUTE_STANDARD_IS_VIRTUAL "," G_FILE_ATTRIBUTE_STANDARD_SIZE "," G_FILE_ATTRIBUTE_UNIX_BLOCKS
    "," G_FILE_ATTRIBUTE_UNIX_BLOCK_SIZE "," G_FILE_ATTRIBUTE_ID_FILESYSTEM;
...
static gboolean _fm_file_ops_job_check_paths(FmFileOpsJob* job, GFile* src, GFileInfo* src_inf, GFile* dest) {
    ...
    {
        err = g_error_new_literal(G_IO_ERROR, G_IO_ERROR_FAILED, _("Destination does not exist"));
    }
    else if (job->type == FM_FILE_OP_MOVE && g_file_equal(src, dest)) {
        err = g_error_new_literal(G_IO_ERROR, G_IO_ERROR_FAILED, _("Source and destination are the same."));
    }
    else if (g_file_info_get_file_type(src_inf) == G_FILE_TYPE_DIRECTORY && g_file_has_prefix(dest, src)) {
        const char* msg = NULL;
...
static gboolean _fm_file_ops_job_copy_file(FmFileOpsJob* job,
                                           GFile* src,
                                           GFileInfo* inf,
                                           GFile* dest,
    ...
    switch (type) {
            ...
            if (!fm_job_is_cancelled(fmjob) && !job->skip_dir_content &&
                ...
                if (!fm_job_is_cancelled(fmjob) && !job->skip_dir_content) {
                    ...
                    _retry_chmod_for_dir:
                        ...
                        if (!g_file_set_attribute_uint32(dest, G_FILE_ATTRIBUTE_UNIX_MODE, mode,
                        ...
                        if (inf) {
                            ...
                            if (G_UNLIKELY(job->skip_dir_content)) {
                                ...
                                if (g_file_is_native(src) == g_file_is_native(dest))
                                    ...
                                    tmp_basename = g_filename_to_utf8(g_file_info_get_name(inf), -1, NULL, NULL, NULL);
                                /* gvfs escapes it itself */
                                else /* copy from virtual to native */
                                    /* display name as copying target more prefers, that basename (for example, for
                                    Google Drive) See
                                    https://debarshiray.wordpress.com/2015/09/13/google-drive-and-gnome-what-is-a-volatile-path/
                                    for some explanations*/
                                    tmp_basename =
                                        fm_uri_subpath_to_native_subpath(g_file_info_get_display_name(inf), NULL);
                                sub_dest =
                                    g_file_get_child(dest, tmp_basename ? tmp_basename : g_file_info_get_name(inf));
                                g_free(tmp_basename);
        ...
        case G_FILE_TYPE_SPECIAL:
            ...
            if (!ret) {
                g_set_error(&err, G_IO_ERROR, G_IO_ERROR_FAILED, _("Cannot copy file '%s': not supported"),
        ...
        _retry_copy:
            if (!g_file_copy(src, dest, flags, fm_job_get_cancellable(fmjob), progress_cb, fmjob, &err)) {
                ...
                if (err->domain == G_IO_ERROR &&
                    ...
                    gboolean is_no_space = (err->domain == G_IO_ERROR && err->code == G_IO_ERROR_NO_SPACE);
                    FmJobErrorAction act = fm_job_emit_error(fmjob, err, FM_JOB_ERROR_MODERATE);
                    g_error_free(err);
                    err = NULL;
                    if (act == FM_JOB_RETRY) {
...
gboolean _fm_file_ops_job_move_file(FmFileOpsJob* job,
                                    GFile* src,
                                    GFileInfo* inf,
                                    GFile* dest,
                                    FmPath* src_path,
            ...
            if (err->domain == G_IO_ERROR && err->code == G_IO_ERROR_EXISTS) {
                ...
                switch (opt) {
                    ...
                    case FM_FILE_OP_OVERWRITE:
                        ...
                        {
                            GFileEnumerator* enu = g_file_enumerate_children(
                                src, query, G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS, fm_job_get_cancellable(fmjob), &err);
                            if (enu) {
                                GFileInfo* child_inf;
                                ...
                                while (!fm_job_is_cancelled(fmjob)) {
                                    ...
                                    if (child_inf) {
                                        ...
                                        _fm_file_ops_job_move_file(job, child, child_inf, child_dest, child_path,
                                                                   sub_src_folder, sub_dest_folder);
...
gboolean _fm_file_ops_job_copy_run(FmFileOpsJob* job) {
    ...
    for (l = fm_path_list_peek_head_link(job->srcs); !fm_job_is_cancelled(fmjob) && l; l = l->next) {
        ...
        if (g_file_is_native(src) && g_file_is_native(dest_dir))
            ...
            tmp_basename = g_filename_to_utf8(fm_path_get_basename(path), -1, NULL, NULL, NULL);
        /* gvfs escapes it itself */
        ...
        dest = g_file_get_child(dest_dir, tmp_basename ? tmp_basename : fm_path_get_basename(path));
        g_free(tmp_basename);
...
_retry_query_dest_info:
    inf = g_file_query_info(dest_dir,
                            G_FILE_ATTRIBUTE_STANDARD_IS_VIRTUAL "," G_FILE_ATTRIBUTE_UNIX_DEVICE
    ...
    g_object_unref(dc);
    g_debug("total size to move: %llu, dest_fs: %s", (long long unsigned int)job->total, job->dest_fs_id);
