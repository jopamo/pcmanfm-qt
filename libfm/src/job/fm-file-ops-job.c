...
enum { PREPARED, CUR_FILE, PERCENT, ASK_RENAME, N_SIGNALS };
...
static void fm_file_ops_job_class_init(FmFileOpsJobClass* klass) {
     ...
     * Since: 0.1.10
     */
    signals[PREPARED] = g_signal_new("prepared", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                                     G_STRUCT_OFFSET(FmFileOpsJobClass, prepared), NULL, NULL,
     ...
     * Since: 0.1.0
     */
    signals[CUR_FILE] = g_signal_new("cur-file", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                                     G_STRUCT_OFFSET(FmFileOpsJobClass, cur_file), NULL, NULL,
     ...
     * Since: 0.1.0
     */
    signals[PERCENT] = g_signal_new("percent", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                                    G_STRUCT_OFFSET(FmFileOpsJobClass, percent), NULL, NULL,
     ...
     * Since: 0.1.0
     */
    signals[ASK_RENAME] = g_signal_new("ask-rename", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                                       G_STRUCT_OFFSET(FmFileOpsJobClass, ask_rename), g_signal_accumulator_first_wins,
                                       NULL, fm_marshal_INT__POINTER_POINTER_POINTER, G_TYPE_INT, 3, G_TYPE_POINTER,
...
static gboolean fm_file_ops_job_run(FmJob* fm_job) {
    ...
    err = g_error_new_literal(G_IO_ERROR, G_IO_ERROR_NOT_SUPPORTED, _("Operation not supported"));
...
FmFileOpOption fm_file_ops_job_ask_rename(FmFileOpsJob* job,
    ...
    {
        GError* err = g_error_new_literal(G_IO_ERROR, G_IO_ERROR_FAILED, _("Cannot access destination file"));
        fm_job_emit_error(FM_JOB(job), err, FM_JOB_ERROR_CRITICAL);
        g_error_free(err);
        fm_file_info_unref(src_fi);
...
FmFileOpOption _fm_file_ops_job_ask_new_name(FmFileOpsJob* job,
                                             GFile* src,
                                             GFile* dest,
                                             GFile** new_dest,
...
static gboolean _fm_file_ops_job_link_run(FmFileOpsJob* job) {
    ...
    if (!g_file_is_native(dest_dir)) {
        GError* err =
            g_error_new_literal(G_IO_ERROR, G_IO_ERROR_FAILED, _("Cannot create a link on non-native filesystem"));
        fm_job_emit_error(FM_JOB(job), err, FM_JOB_ERROR_CRITICAL);
        g_error_free(err);
    ...
    _retry_link:
        ...
        else /* create shortcut instead */
        {
            ...
            GFileOutputStream* out = g_file_create(dest, G_FILE_CREATE_NONE, fm_job_get_cancellable(fmjob), &err);
            GFile* src_file;
            GFileInfo* inf;
            ...
            if (out == NULL)
                goto _link_error;
            src_file = fm_path_to_gfile(path);
            inf = g_file_query_info(src_file,
                                    G_FILE_ATTRIBUTE_STANDARD_TARGET_URI "," G_FILE_ATTRIBUTE_STANDARD_ICON
                                                                         "," G_FILE_ATTRIBUTE_STANDARD_DISPLAY_NAME,
                                    G_FILE_QUERY_INFO_NONE, fm_job_get_cancellable(fmjob), NULL);
            g_object_unref(src_file);
            if (inf) {
            ...
            dname = g_strdup_printf(
                "[Desktop Entry]\n"
                "Type=Link\n"
                "Name=%s"
                "%s%s\n"
                "URL=%s\n",
                name, iname ? "\nIcon=" : "", iname ? iname : "", src);
            g_free(name);
            g_free(iname);
            if (!g_output_stream_write_all(G_OUTPUT_STREAM(out), dname, strlen(dname), &out_len,
                                           fm_job_get_cancellable(fmjob), &err) ||
                !g_output_stream_close(G_OUTPUT_STREAM(out), fm_job_get_cancellable(fmjob), &err)) {
                g_object_unref(out);
                g_free(dname);
                goto _link_error;
            }
