...
static const char query[] = G_FILE_ATTRIBUTE_STANDARD_TYPE
    "," G_FILE_ATTRIBUTE_STANDARD_NAME "," G_FILE_ATTRIBUTE_UNIX_GID "," G_FILE_ATTRIBUTE_UNIX_UID
    "," G_FILE_ATTRIBUTE_UNIX_MODE "," G_FILE_ATTRIBUTE_STANDARD_DISPLAY_NAME;

static gboolean _fm_file_ops_job_change_attr_file(FmFileOpsJob* job, GFile* gf, GFileInfo* inf, FmFolder* folder) {
    GError* err = NULL;
    ...
    if (!inf) {
    _retry_query_info:
        inf = g_file_query_info(gf, query, G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS, cancellable, &err);
        if (!inf) {
    ...
    _retry_change_owner:
        if (!g_file_set_attribute_uint32(gf, G_FILE_ATTRIBUTE_UNIX_UID, job->uid, G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS,
                                         cancellable, &err)) {
            FmJobErrorAction act = fm_job_emit_error(fmjob, err, FM_JOB_ERROR_MILD);
            g_error_free(err);
            err = NULL;
    ...
    _retry_change_group:
        if (!g_file_set_attribute_uint32(gf, G_FILE_ATTRIBUTE_UNIX_GID, job->gid, G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS,
                                         cancellable, &err)) {
            FmJobErrorAction act = fm_job_emit_error(fmjob, err, FM_JOB_ERROR_MILD);
            g_error_free(err);
            err = NULL;
    ...
    if (!fm_job_is_cancelled(fmjob) && job->new_mode_mask) {
        ...
        /* new mode */
    _retry_chmod:
        if (!g_file_set_attribute_uint32(gf, G_FILE_ATTRIBUTE_UNIX_MODE, mode, G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS,
                                         cancellable, &err)) {
            FmJobErrorAction act = fm_job_emit_error(fmjob, err, FM_JOB_ERROR_MILD);
            g_error_free(err);
            err = NULL;
    ...
    _retry_change_icon:
        if (!g_file_set_attribute(gf, G_FILE_ATTRIBUTE_STANDARD_ICON, G_FILE_ATTRIBUTE_TYPE_OBJECT, job->icon,
    ...
    _retry_change_hidden:
        if (!g_file_set_attribute(gf, G_FILE_ATTRIBUTE_STANDARD_IS_HIDDEN, G_FILE_ATTRIBUTE_TYPE_BOOLEAN, &hidden,
                                  G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS, cancellable, &err)) {
            FmJobErrorAction act = fm_job_emit_error(fmjob, err, FM_JOB_ERROR_MILD);
            g_clear_error(&err);
            if (act == FM_JOB_RETRY)
    ...
    _retry_change_target:
        if (!g_file_set_attribute_string(gf, G_FILE_ATTRIBUTE_STANDARD_TARGET_URI, job->target,
                                         G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS, cancellable, &err)) {
            FmJobErrorAction act = fm_job_emit_error(fmjob, err, FM_JOB_ERROR_MILD);
            g_clear_error(&err);
            if (act == FM_JOB_RETRY)
    ...
    _retry_enum_children:
        enu = g_file_enumerate_children(gf, query, G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS, cancellable, &err);
        if (!enu) {
            FmJobErrorAction act = fm_job_emit_error(fmjob, err, FM_JOB_ERROR_MILD);
            g_error_free(err);
            err = NULL;
...
gboolean _fm_file_ops_job_change_attr_run(FmFileOpsJob* job) {
    ...
    if (!fm_job_is_cancelled(FM_JOB(job)) && l->next && (job->display_name || job->target)) {
        GError* error;
