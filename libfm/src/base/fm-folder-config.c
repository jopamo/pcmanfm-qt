...
FmFolderConfig* fm_folder_config_open(FmPath* path) {
    ...
    if (g_file_test(fc->filepath, G_FILE_TEST_EXISTS)) {
        ...
        if (g_key_file_load_from_file(fc->kf, fc->filepath, G_KEY_FILE_KEEP_COMMENTS | G_KEY_FILE_KEEP_TRANSLATIONS,
                                      NULL) &&
 ...
 * Since: 1.2.0
 */
gboolean fm_folder_config_get_integer(FmFolderConfig* fc, const char* key, gint* val) {
 ...
 * Since: 1.2.0
 */
gboolean fm_folder_config_get_uint64(FmFolderConfig* fc, const char* key, guint64* val) {
    GError* error = NULL;
 ...
 * Since: 1.2.0
 */
gboolean fm_folder_config_get_double(FmFolderConfig* fc, const char* key, gdouble* val) {
    GError* error = NULL;
 ...
 * Since: 1.2.0
 */
gboolean fm_folder_config_get_boolean(FmFolderConfig* fc, const char* key, gboolean* val) {
 ...
 * Since: 1.2.0
 */
char** fm_folder_config_get_string_list(FmFolderConfig* fc, const char* key, gsize* length) {
 ...
 * Since: 1.2.0
 */
void fm_folder_config_set_boolean(FmFolderConfig* fc, const char* key, gboolean val) {
    fc->changed = TRUE;
 ...
 * Since: 1.2.0
 */
void fm_folder_config_set_string(FmFolderConfig* fc, const char* key, const char* string) {
    fc->changed = TRUE;
 ...
 * Since: 1.2.0
 */
void fm_folder_config_set_string_list(FmFolderConfig* fc, const char* key, const gchar* const list[], gsize length) {
    fc->changed = TRUE;
...
void fm_folder_config_save_cache(void) {
    ...
    if (fc_cache_changed && (out = g_key_file_to_data(fc_cache, &len, NULL))) {
        ...
        if (g_file_set_contents(path2, out, len, &error)) {
            ...
            g_unlink(path3);
            if (!g_file_test(path, G_FILE_TEST_EXISTS) || g_rename(path, path3) == 0) {
                /* rename temp file */
                if (g_rename(path2, path) == 0) {
                    ...
                    g_warning("cannot rename %s to %s: %s", path2, path, g_strerror(errno));
            }
            else
                g_warning("cannot rename %s to %s: %s", path, path3, g_strerror(errno));
        }
        else {
...
void _fm_folder_config_init(void) {
    char* path = g_build_filename(g_get_user_config_dir(), "libfm/dir-settings.conf", NULL);
