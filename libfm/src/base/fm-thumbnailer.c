...
GPid fm_thumbnailer_launch_for_uri_async(FmThumbnailer* thumbnailer,
    ...
    if (cmd_line) {
        ...
        if (g_shell_parse_argv(cmd_line, &argc, &argv, NULL)) {
            g_spawn_async("/", argv, NULL, G_SPAWN_SEARCH_PATH | G_SPAWN_STDOUT_TO_DEV_NULL | G_SPAWN_DO_NOT_REAP_CHILD,
                          NULL, NULL, &pid, error);
            g_strfreev(argv);
        }
...
static void find_thumbnailers_in_data_dir(GHashTable* hash, const char* data_dir) {
    ...
    if (dir) {
        ...
        while ((basename = g_dir_read_name(dir)) != NULL) {
            ...
            if (G_LIKELY(g_str_has_suffix(basename, ".thumbnailer")) && g_hash_table_lookup(hash, basename) == NULL)
