...
static SpecialDirInfo special_dir_info[G_USER_N_DIRECTORIES] = {
    {NULL, NULL, "user-desktop"},     {NULL, NULL, "folder-documents"}, {NULL, NULL, "folder-download"},
    {NULL, NULL, "folder-music"},     {NULL, NULL, "folder-pictures"},  {NULL, NULL, "folder-publicshare"},
    {NULL, NULL, "folder-templates"}, {NULL, NULL, "folder-videos"}};
 ...
 */
gboolean _fm_file_info_set_from_native_file(FmFileInfo* fi, const char* path, GError** err, gboolean get_fast) {
    struct stat st;
    char* dname;
    ...
    if (lstat(path, &st) == 0) {
            ...
            if (g_key_file_load_from_file(kf, path, 0, NULL)) {
                ...
                if (type) {
                    ...
                    if (strcmp(type, G_KEY_FILE_DESKTOP_TYPE_LINK) == 0) {
                        char* uri =
                            g_key_file_get_string(kf, G_KEY_FILE_DESKTOP_GROUP, G_KEY_FILE_DESKTOP_KEY_URL, NULL);
                        ...
                        if (uri) {
                            ...
                            if (strcmp(fm_mime_type_get_type(new_mime_type), "application/octet-stream") == 0 ||
        ...
        if (S_ISDIR(st.st_mode)) {
            inf = g_file_query_filesystem_info(gfile, G_FILE_ATTRIBUTE_FILESYSTEM_READONLY, NULL, NULL);
            if (inf) {
        ...
        g_set_error(err, G_IO_ERROR, g_io_error_from_errno(errno), "%s: %s", path, g_strerror(errno));
        return FALSE;
    }
...
gboolean fm_file_info_is_dir(FmFileInfo* fi) {
    return (S_ISDIR(fi->mode) || (fi->mime_type == _fm_mime_type_get_inode_directory()));
}
...
gboolean fm_file_info_is_executable_type(FmFileInfo* fi) {
    ...
    if (strncmp(type, "text/", 5) == 0) { /* g_content_type_can_be_executable reports text files as executables too */
        /* We don't execute remote files nor files in trash */
        if (fm_path_is_native(fi->path) &&
            (fi->mode & (S_IXOTH | S_IXGRP | S_IXUSR))) { /* it has executable bits so lets check shell-bang */
    ...
    else if (strcmp(type, "application/x-desktop") == 0) { /* treat desktop entries as executables if
        ...
        if (fm_path_is_native(fi->path) && (fi->mode & (S_IRUSR | S_IRGRP | S_IROTH))) {
            if (fi->shortcut && fi->target) {
                ...
                if (is_native) {
                    ...
                    gboolean usr_share =
                        (g_str_has_prefix(path, "/usr/share/") || g_str_has_prefix(path, "/usr/local/share/"));
                    g_free(path);
                    if (!usr_share)
                        return TRUE;
                }
...
gboolean fm_file_info_can_thumbnail(FmFileInfo* fi) {
    ...
    if (fi->size == 0 || /* don't generate thumbnails for empty files */
        !(fi->mode & S_IFREG) || fm_file_info_is_desktop_entry(fi) || fm_file_info_is_unknown_type(fi))
        return FALSE;
    return TRUE;
}
...
const char* fm_file_info_get_disp_mtime(FmFileInfo* fi) {
    ...
    if (fi->mtime > 0) {
        if (!fi->disp_mtime) {
            char buf[128];
            strftime(buf, sizeof(buf), fm_config->date_iso_8601 ? "%F %R" : "%x %R", localtime(&fi->mtime));
...
static FmListFuncs fm_list_funcs = {.item_ref = (gpointer (*)(gpointer))&fm_file_info_ref,
