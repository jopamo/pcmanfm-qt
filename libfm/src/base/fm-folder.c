...
static void fm_folder_class_init(FmFolderClass* klass) {
     ...
     * Since: 0.1.0
     */
    signals[FILES_ADDED] = g_signal_new("files-added", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                                        G_STRUCT_OFFSET(FmFolderClass, files_added), NULL, NULL,
     ...
     * Since: 0.1.0
     */
    signals[FILES_REMOVED] = g_signal_new("files-removed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                                          G_STRUCT_OFFSET(FmFolderClass, files_removed), NULL, NULL,
                                          g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER);
     ...
     * Since: 0.1.0
     */
    signals[FILES_CHANGED] = g_signal_new("files-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                                          G_STRUCT_OFFSET(FmFolderClass, files_changed), NULL, NULL,
                                          g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER);
     ...
     * Since: 1.0.0
     */
    signals[START_LOADING] = g_signal_new("start-loading", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                                          G_STRUCT_OFFSET(FmFolderClass, start_loading), NULL, NULL,
     ...
     * Since: 1.0.0
     */
    signals[FINISH_LOADING] = g_signal_new("finish-loading", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                                           G_STRUCT_OFFSET(FmFolderClass, finish_loading), NULL, NULL,
                                           g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
     ...
     */
    signals[UNMOUNT] =
        g_signal_new("unmount", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST, G_STRUCT_OFFSET(FmFolderClass, unmount),
     ...
     */
    signals[CHANGED] =
        g_signal_new("changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST, G_STRUCT_OFFSET(FmFolderClass, changed),
                     NULL, NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
     ...
     */
    signals[REMOVED] =
        g_signal_new("removed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST, G_STRUCT_OFFSET(FmFolderClass, removed),
                     NULL, NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
     ...
     * Since: 0.1.16
     */
    signals[CONTENT_CHANGED] = g_signal_new("content-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                                            G_STRUCT_OFFSET(FmFolderClass, content_changed), NULL, NULL,
                                            g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
     ...
     */
    signals[FS_INFO] =
        g_signal_new("fs-info", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST, G_STRUCT_OFFSET(FmFolderClass, fs_info),
                     NULL, NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
    ...
    signals[ERROR] = g_signal_new("error", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                                  G_STRUCT_OFFSET(FmFolderClass, error), NULL, NULL,
#if GLIB_CHECK_VERSION(2, 26, 0)
                                  fm_marshal_UINT__BOXED_UINT, G_TYPE_UINT, 2, G_TYPE_ERROR, G_TYPE_UINT);
#else
                                  fm_marshal_INT__POINTER_INT, G_TYPE_INT, 2, G_TYPE_POINTER, G_TYPE_INT);
#endif
}
...
static void queue_reload(FmFolder* folder) {
    ...
    if (!folder->idle_reload_handler)
        folder->idle_reload_handler =
            g_idle_add_full(G_PRIORITY_LOW, (GSourceFunc)on_idle_reload, g_object_ref(folder), NULL);
    G_UNLOCK(query);
}
...
gboolean _fm_folder_event_file_changed(FmFolder* folder, FmPath* path) {
    ...
    if (!g_slist_find(folder->files_to_update, path) && !g_slist_find(folder->files_to_add, path) &&
        ...
        if (G_LIKELY(files)) {
            ...
            if (folder->defer_content_test && fm_path_is_native(folder->dir_path))
                ...
                for (l = files; l; l = l->next)
                    folder->files_to_update =
                        g_slist_prepend(folder->files_to_update, fm_path_ref(fm_file_info_get_path(l->data)));
            G_UNLOCK(lists);
...
static FmJobErrorAction on_dirlist_job_error(FmDirListJob* job,
                                             GError* err,
...
void fm_folder_reload(FmFolder* folder) {
    ...
    folder->dirlist_job = fm_dir_list_job_new2(
        folder->dir_path, folder->defer_content_test ? FM_DIR_LIST_JOB_FAST : FM_DIR_LIST_JOB_DETAILED);
...
void fm_folder_query_filesystem_info(FmFolder* folder) {
    ...
    if (!folder->fs_size_cancellable && !folder->fs_info_not_avail) {
        ...
        g_file_query_filesystem_info_async(
            folder->gf, G_FILE_ATTRIBUTE_FILESYSTEM_SIZE "," G_FILE_ATTRIBUTE_FILESYSTEM_FREE, G_PRIORITY_LOW,
            folder->fs_size_cancellable, (GAsyncReadyCallback)on_query_filesystem_info_finished, g_object_ref(folder));
    }
    G_UNLOCK(query);
}
