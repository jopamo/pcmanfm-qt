...
typedef enum { EXEC_MODE_NORMAL, EXEC_MODE_TERMINAL, EXEC_MODE_EMBEDDED, EXEC_MODE_DISPLAY_OUTPUT } FmActionExecMode;
...
static gboolean _expand_params(GString* str, const char* line, FmActionMenu* at, gboolean do_quote, GError** error) {
    FmActionMenu* menu;
    ...
    if (line == NULL || line[0] == '\0') {
        g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_FAILED, _("Empty value"));
        return FALSE;
    }

    menu = _get_top_menu(at);
    if (menu == NULL) {
        g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_FAILED, _("Invalid selection"));
        return FALSE;
    }
    files = menu->files;
...
G_DEFINE_TYPE_WITH_CODE(FmAction,
                        fm_action,
                        G_TYPE_OBJECT,
                        G_IMPLEMENT_INTERFACE(G_TYPE_APP_INFO, fm_action_g_app_info_init))
...
static gboolean _do_launch(FmAction* action, GAppLaunchContext* launch_context, GError** error) {
    GString* str = g_string_sized_new(64);
    char** argv;
    int argc;
    gboolean ok = FALSE;
    ...
    if (g_shell_parse_argv(str->str, &argc, &argv, error)) {
        ...
        if (launch_context) {
            ...
            for (l = fm_file_info_list_peek_head_link(menu->files); l; l = l->next)
                launched_files = g_list_prepend(launched_files, fm_path_to_gfile(fm_file_info_get_path(l->data)));
            launched_files = g_list_reverse(launched_files);
            data.display = g_app_launch_context_get_display(launch_context, G_APP_INFO(action), launched_files);
            if (action->use_sn)
                data.sn_id =
                    g_app_launch_context_get_startup_notify_id(launch_context, G_APP_INFO(action), launched_files);
        ...
        ok = g_spawn_async(str->str, argv, NULL, G_SPAWN_SEARCH_PATH, child_setup, &data, NULL, error);
        if (!ok) {
            if (data.sn_id)
...
static gboolean _fm_action_launch(GAppInfo* appinfo, GList* files, GAppLaunchContext* launch_context, GError** error) {
...
static gboolean _fm_action_return_false_unsupported(GAppInfo* appinfo, const char* content_type, GError** error) {
...
G_DEFINE_TYPE_WITH_CODE(FmActionMenu,
                        fm_action_menu,
                        G_TYPE_OBJECT,
                        G_IMPLEMENT_INTERFACE(G_TYPE_APP_INFO, fm_action_menu_g_app_info_init))
...
/* ---- FmActionCache class ---- */
static void _action_cache_monitor_event(GFileMonitor* mon,
                                        GFile* gf,
                                        GFile* other,
...
static void _fm_action_cache_finalize(GObject* object) {
    ...
    while (l_dirs != NULL) {
        ...
        l_dirs = dir->next;
        g_signal_handlers_disconnect_by_func(dir->mon, _action_cache_monitor_event, cache);
...
/* ---- internal API ---- */
static inline FmActionCondition* _add_condition_int(FmActionCondition* clist, FmActionConditionType type, gint val) {
...
static inline FmActionCondition* _add_condition_str(FmActionCondition* clist, FmActionConditionType type, gchar* val) {
    FmActionCondition* cond = g_slice_new(FmActionCondition);
    cond->next = clist;
    cond->type = type;
    cond->str = val;
...
static void fm_actions_add_action_from_keyfile(FmActionCache* cache,
    ...
    info->toolbar_label = g_key_file_get_locale_string(kf, "Desktop Entry", "ToolbarLabel", NULL, NULL);
...
/* lock is on, don't do any unref! */
static void fm_actions_add_for_keyfile(FmActionCache* cache, GKeyFile* kf, GFile* parent, const char* id) {
...
static gboolean fm_action_file_may_update(FmActionCache* cache,
                                          const char* file,
                                          GFile** gfp,
                                          char** idp,
                                          GList** to_drop) {
...
static void fm_action_cache_ensure_updates(FmActionCache* cache) {
    ...
    while (to_update != NULL) {
        ...
        if (fm_action_file_may_update(cache, to_update->data, &parent, &id, &to_drop) &&
            g_key_file_load_from_file(kf, to_update->data, G_KEY_FILE_KEEP_TRANSLATIONS,
                                      NULL)) /* NOTE: ignoring errors */
...
static gboolean fm_actions_update_idle(gpointer data) {
    ...
    if (fm_action_file_may_update(cache, cache_to_update->data, &parent, &id, &to_drop)) {
        kf = g_key_file_new();
        if (g_key_file_load_from_file(kf, cache_to_update->data, G_KEY_FILE_KEEP_TRANSLATIONS,
                                      NULL)) /* NOTE: ignoring errors */
            fm_actions_add_for_keyfile(cache, kf, parent, id);
        g_key_file_free(kf);
    }
...
static void fm_actions_schedule_update(FmActionCache* cache, char* filename) {
    if (cache_to_update == NULL) /* no updater running now */
    {
        ...
        *cache_updater_data = cache;
        g_idle_add_full(G_PRIORITY_LOW, fm_actions_update_idle, cache_updater_data, g_free);
    }
...
static void _action_cache_monitor_event(GFileMonitor* mon,
    ...
    switch (evt) {
        ...
        case G_FILE_MONITOR_EVENT_DELETED:
            ...
            if (filename)
                for (sl = cache_to_update; sl; sl = sn) {
                    sn = sl->next;
...
static void fm_action_cache_add_directory(FmActionCache* cache, const char* path) {
    ...
    g_signal_connect(fmdir->mon, "changed", G_CALLBACK(_action_cache_monitor_event), cache);
}

/* ---- matching conditions (arggh!) ---- */
static gboolean _matches_cond(FmFileInfoList* files,
                              FmFileInfo* location,
                              FmActionCondition* cond,
                              FmActionMenu* root) {
    GList *flist = NULL, *l;
    char **x, *tst, *s;
    const char* c;
    ...
    for (; match && cond; cond = cond->next) {
        ...
        switch (cond->type) {
            ...
            case CONDITION_TYPE_DBUS: /* ShowIfRegistered */
                ...
                result = g_dbus_connection_call_sync(conn, "org.freedesktop.DBus", /* bus name */
                                                     "/org/freedesktop/DBus",      /* object path */
                                                     "org.freedesktop.DBus",       /* interface name */
                                                     "NameHasOwner",               /* method name */
                                                     g_variant_new("(s)", str->str), G_VARIANT_TYPE("(b)"),
                    ...
                    else if (tst[0] == '!') /* negate condition */
                    {
                        if (strncmp(&tst[1], "all/", 4) == 0) {
                            ...
                            for (l = flist; match && l; l = l->next) {
                                ...
                                match = (strncmp(c, tst, len) != 0 || c[len] != tst[len]);
                            }
                        }
                    }
                    else if (!found) {
...
static gboolean _menu_matches(FmActionMenu* menu, FmFileInfo* location, FmFileInfoList* files, FmActionMenu* root) {
    return _matches_cond(files, location, menu->conditions, root);
}

static FmAction* _action_matches(FmAction* action,
                                 FmFileInfo* location,
                                 FmFileInfoList* files,
                                 FmActionTarget target,
                                 FmActionMenu* root) {
...
static void _insert_menu(FmActionMenu* root, GList* elem, GList** menus, GList** actions, GList** zero_level_menus) {
 ...
 * Since: 1.3.0
 */
FmActionMenu* fm_action_get_for_context(FmActionCache* cache, FmFileInfo* location, FmFileInfoList* files) {
