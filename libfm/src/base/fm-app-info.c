...
static void append_file_to_cmd(GFile* gf, GString* cmd) {
    ...
    if (file == NULL) /* trash:// gvfs is incomplete in resolving it */
    {
        ...
        if (g_file_has_uri_scheme(gf, "trash")) {
            ...
            inf = g_file_query_info(gf, G_FILE_ATTRIBUTE_STANDARD_TARGET_URI, G_FILE_QUERY_INFO_NONE, NULL, NULL);
            if (inf == NULL) /* failed */
                return;
...
static char* expand_exec_macros(GAppInfo* app,
                                const char* full_desktop_path,
                                GKeyFile* kf,
                                GList** gfiles,
                                GList** launching) {
    GString* cmd;
...
static char* expand_terminal(char* cmd, gboolean keep_open, GError** error) {
    ...
    if (keep_open && term->noclose_arg)
        ...
        opts = "";
    if (term->custom_args)
        ret = g_strdup_printf("%s %s %s %s", term->program, term->custom_args, opts, cmd);
    else
...
static gboolean do_launch(GAppInfo* appinfo,
                          const char* full_desktop_path,
                          GKeyFile* kf,
                          GList** inp,
    ...
    if (cmd == NULL || cmd[0] == '\0') {
        ...
        g_set_error_literal(err, G_IO_ERROR, G_IO_ERROR_FAILED, "Desktop entry contains no valid Exec line");
        return FALSE;
    }
    ...
    if (use_terminal) {
        ...
        if (G_LIKELY(kf))
            keep_open = g_key_file_get_boolean(kf, "Desktop Entry", "X-KeepTerminal", NULL);
    ...
    if (g_shell_parse_argv(cmd, &argc, &argv, err)) {
        ...
        ret = g_spawn_async((path && path[0] == '/') ? path : NULL, argv, NULL,
                            G_SPAWN_SEARCH_PATH | G_SPAWN_DO_NOT_REAP_CHILD, child_setup, &data, &pid, err);
        if (ret)
            /* Ensure that we don't double fork and break pkexec */
...
gboolean fm_app_info_launch(GAppInfo* appinfo, GList* files, GAppLaunchContext* launch_context, GError** error) {
    ...
    if (G_IS_DESKTOP_APP_INFO(appinfo)) {
        ...
        if (id) /* this is an installed application */
        {
            ...
            if (supported) {
                do {
                    ret = do_launch(appinfo, full_desktop_path, kf, &launch_list, launch_context, error);
                } while (launch_list && ret);
...
#endif
            {
                ...
                if (g_object_get_data(G_OBJECT(appinfo), "flags")) {
                    supported = TRUE;
                    do {
                        ret = do_launch(appinfo, NULL, NULL, &launch_list, launch_context, error);
                    } while (launch_list && ret);
                }
            }
        }
