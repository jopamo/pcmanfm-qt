...
static void fm_config_class_init(FmConfigClass* klass) {
     ...
     * Since: 0.1.0
     */
    signals[CHANGED] = g_signal_new("changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST | G_SIGNAL_DETAILED,
                                    G_STRUCT_OFFSET(FmConfigClass, changed), NULL, NULL, g_cclosure_marshal_VOID__VOID,
                                    G_TYPE_NONE, 0);
}

static void _on_cfg_file_changed(GFileMonitor* mon, GFile* gf, GFile* other, GFileMonitorEvent evt, FmConfig* cfg);
...
#define _save_config_strv(_str_, _cfg_, _name_)                  \
    do {                                                         \
...
#define _save_drop_action(_str_, _cfg_, _name_)            \
    do {                                                   \
        switch (_cfg_->_name_) {                           \
