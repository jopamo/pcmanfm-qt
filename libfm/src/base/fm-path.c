...
static inline FmPath* _fm_path_new_internal(FmPath* parent, const char* name, int name_len, int flags) {
    ...
    if (parent) {
        ...
        path->iter = g_sequence_insert_sorted(parent->children, path, (GCompareDataFunc)fm_path_compare, NULL);
        G_UNLOCK(roots);
    }
    return path;
}
 ...
 */
static FmPath* _fm_path_new_child_len(FmPath* parent,
                                      const char* basename,
                                      int name_len,
    ...
    if (parent->children) {
        ...
        iter = g_sequence_lookup(parent->children, path, (GCompareDataFunc)fm_path_compare, NULL);
#else
        iter = g_sequence_search(parent->children, path, (GCompareDataFunc)fm_path_compare, NULL);
        if (iter) {
            FmPath* pp = NULL;
...
FmPath* fm_path_new_child_len(FmPath* parent, const char* basename, int name_len) {
    return _fm_path_new_child_len(parent, basename, name_len, parent && fm_path_is_native(parent), FALSE);
}
...
FmPath* fm_path_new_child(FmPath* parent, const char* basename) {
    if (G_LIKELY(basename && *basename)) {
        ...
        return _fm_path_new_child_len(parent, basename, baselen, parent && fm_path_is_native(parent), FALSE);
    }
...
FmPath* fm_path_new_for_uri(const char* uri) {
    ...
    if (*rel_path) {
        if (root == root_path) {
            ...
            path = _fm_path_new_child_len(root, rel_path + 1, strlen(rel_path) - 1, TRUE, TRUE);
        else
...
FmPath* fm_path_new_for_display_name(const char* path_name) {
    ...
    if (path_name[0] != '/') /* it's an URI */
    {
        ...
        c = g_uri_escape_string(
            path_copy, G_URI_RESERVED_CHARS_GENERIC_DELIMITERS G_URI_RESERVED_CHARS_SUBCOMPONENT_DELIMITERS, FALSE);
...
FmPath* fm_path_new_for_str(const char* path_str) {
    ...
    escaped = g_uri_escape_string(
        path_str, G_URI_RESERVED_CHARS_GENERIC_DELIMITERS G_URI_RESERVED_CHARS_SUBCOMPONENT_DELIMITERS, TRUE);
...
void _fm_path_init() {
    ...
    trash_root_path =
        _fm_path_new_internal(NULL, "trash:///", 9, FM_PATH_IS_TRASH | FM_PATH_IS_VIRTUAL | FM_PATH_IS_LOCAL);
...
static FmListFuncs funcs = {.item_ref = (gpointer (*)(gpointer))&fm_path_ref,
