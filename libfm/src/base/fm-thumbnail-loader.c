...
static void generate_thumbnails(ThumbnailTask* task) {
    if (fm_file_info_is_image(task->fi) &&
        ...
        (fm_config->thumbnail_max == 0 || fm_file_info_get_size(task->fi) <= (fm_config->thumbnail_max << 10))) {
...
static GObject* scale_pix(GObject* ori_pix, int size) {
    ...
    if ((new_width == width && new_height == height) || (size > width && size > height)) /* don't scale up */
    {
            ...
            if (exif_data) {
                ...
                if (exif_data->data) /* if an embedded thumbnail is available */
                {
                    ...
                    GInputStream* mem_stream =
                        g_memory_input_stream_new_from_data(exif_data->data, exif_data->size, NULL);
...
static gboolean run_thumbnailer(FmThumbnailer* thumbnailer, ThumbnailTask* task, const char* output_file, guint size) {
    /* g_print("run_thumbnailer: uri: %s\n", uri); */
    ThumbnailerStatus status = {FALSE, 0};
    gboolean timed_out = FALSE;
    GPid _pid = fm_thumbnailer_launch_for_uri_async(thumbnailer, task->uri, output_file, size, NULL);
    ...
    thumbnailer_timeout_id = g_timeout_add_seconds(THUMBNAILER_TIMEOUT_SEC, on_thumbnailer_timeout, &timed_out);
    g_child_watch_add(_pid, _pid_watcher, &status);
    /* g_print("pid: %d\n", thumbnailer_pid); */
    while (!timed_out && !status.finished && !g_cancellable_is_cancelled(task->cancellable))
        g_cond_wait(cond_ptr, lock_ptr);
    if (thumbnailer_timeout_id)
