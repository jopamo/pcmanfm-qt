...
gboolean fm_launch_desktop_entry(GAppLaunchContext* ctx,
    ...
    if (app) {
        ...
        g_set_error(&err, G_IO_ERROR, G_IO_ERROR_FAILED, _("Invalid desktop entry file: '%s'"), file_or_id);
...
static FmJobErrorAction on_query_target_info_error(FmJob* job,
                                                   GError* err,
                                                   FmJobErrorSeverity severity,
                                                   QueryErrorData* data) {
    if (data->launcher->error &&
        !data->launcher->error(data->ctx, err, fm_file_info_job_get_current(FM_FILE_INFO_JOB(job)), data->user_data))
        return FM_JOB_RETRY;
...
gboolean fm_launch_files(GAppLaunchContext* ctx, GList* file_infos, FmFileLauncher* launcher, gpointer user_data) {
    ...
    for (l = file_infos; l; l = l->next) {
        ...
        if (launcher->open_folder && (fm_file_info_is_dir(fi) || fm_file_info_is_mountable(fi))) {
            /* special handling for shortcuts */
            if (target) {
                fi = _fetch_file_info_for_shortcut(fm_file_info_get_target(fi), ctx, launcher, user_data);
                if (fi == NULL)
        ...
        _launch_desktop_entry:
            ...
            if (fm_file_info_is_executable_type(fi)) {
                switch (launcher->exec_file(fi, user_data)) {
                    ...
                    case FM_FILE_LAUNCHER_EXEC: {
                        ...
                        fm_launch_desktop_entry(ctx, target ? target : filename, NULL, launcher, user_data);
                        if (!target)
                            g_free(filename);
                        continue;
                    }
            ...
            else if (fm_file_info_is_native(fi) /* an exception */ || fm_path_is_xdg_menu(fm_file_info_get_path(fi))) {
                if (!target)
                    filename = fm_path_to_str(fm_file_info_get_path(fi));
                fm_launch_desktop_entry(ctx, target ? target : filename, NULL, launcher, user_data);
                if (!target)
            ...
            if (fm_path_is_native(path)) {
                ...
                if (target) {
                    ...
                    /* if we already know MIME type then use it instead */
                    {
                        ...
                        if (scheme) {
                            /* FIXME: this is rough! */
                            if (strcmp(scheme, "file") != 0 && strcmp(scheme, "trash") != 0 &&
                                strcmp(scheme, "network") != 0 && strcmp(scheme, "computer") != 0 &&
                                ...
                                if (app) {
                                    ...
                                    g_set_error(&err, G_IO_ERROR, G_IO_ERROR_FAILED,
                                                _("No default application is set to launch URIs %s://"), scheme);
                                if (err) {
                                    launcher->error(ctx, err, NULL, user_data);
                                    g_clear_error(&err);
                                }
                                            ...
                                            if (chdir(run_path) != 0) {
                                                ...
                                                if (launcher->error) {
                                                    g_set_error(&err, G_IO_ERROR, g_io_error_from_errno(errno),
                                                                _("Cannot set working directory to '%s': %s"), run_path,
                                                                g_strerror(errno));
                                                    launcher->error(ctx, err, NULL, user_data);
                                                    g_clear_error(&err);
                                                }
    ...
    if (g_hash_table_size(hash) > 0) {
        ...
        while (g_hash_table_iter_next(&it, (void**)&type, (void**)&fis)) {
            ...
            if (app) {
                ...
                g_set_error(&err, G_IO_ERROR, G_IO_ERROR_FAILED, _("No default application is set for MIME type %s"),
                            type);
            if (err) {
                launcher->error(ctx, err, NULL, user_data);
                g_clear_error(&err);
