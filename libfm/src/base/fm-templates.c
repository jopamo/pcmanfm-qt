...
static FmMimeType* _fm_template_guess_mime_type(FmPath* path, FmMimeType* mime_type, FmPath** tpath) {
    ...
    if (g_key_file_load_from_file(kf, filename, G_KEY_FILE_NONE, NULL)) {
        type = g_key_file_get_string(kf, G_KEY_FILE_DESKTOP_GROUP, G_KEY_FILE_DESKTOP_KEY_TYPE, NULL);
        ...
        type = g_key_file_get_string(kf, G_KEY_FILE_DESKTOP_GROUP, G_KEY_FILE_DESKTOP_KEY_MIME_TYPE, NULL);
        if (type) {
            if (tpath)
                *tpath = subpath;
            else
...
static FmTemplate* _fm_template_find_for_file(FmPath* path, FmMimeType* mime_type) {
    ...
    for (l = templates; l; l = l->next) {
        templ = l->data;
        if (!template_type_once) {
            if (strcmp(fm_path_get_basename(templ->template_file), fm_path_get_basename(tpath)) == 0) {
                g_object_ref(templ);
                fm_mime_type_unref(mime_type);
                fm_path_unref(tpath);
                return templ;
...
static void _fm_template_update_from_file(FmTemplate* templ, FmTemplateFile* file) {
    ...
    if (file->is_desktop_entry) /* desktop entry */
    {
        ...
        if (g_key_file_load_from_file(kf, filename, G_KEY_FILE_NONE, &error)) {
            hidden = g_key_file_get_boolean(kf, G_KEY_FILE_DESKTOP_GROUP, G_KEY_FILE_DESKTOP_KEY_HIDDEN, NULL);
            file->inactive = hidden;
            /* g_debug("%s hidden: %d", filename, hidden); */
            /* FIXME: test for G_KEY_FILE_DESKTOP_KEY_ONLY_SHOW_IN ? */
            if (!hidden) {
                tmp = g_key_file_get_string(kf, G_KEY_FILE_DESKTOP_GROUP, G_KEY_FILE_DESKTOP_KEY_TYPE, NULL);
                ...
                g_free(tmp);
                tmp = g_key_file_get_string(kf, G_KEY_FILE_DESKTOP_GROUP, G_KEY_FILE_DESKTOP_KEY_URL, NULL);
                if (tmp) {
                    if (templ->template_file)
                ...
                tmp = g_key_file_get_string(kf, G_KEY_FILE_DESKTOP_GROUP, G_KEY_FILE_DESKTOP_KEY_ICON, NULL);
                if (tmp) {
                    if (templ->icon)
                ...
                tmp = g_key_file_get_string(kf, G_KEY_FILE_DESKTOP_GROUP, G_KEY_FILE_DESKTOP_KEY_EXEC, NULL);
                if (tmp) {
                    g_free(templ->command);
                    templ->command = tmp;
                }
                tmp =
                    g_key_file_get_locale_string(kf, G_KEY_FILE_DESKTOP_GROUP, G_KEY_FILE_DESKTOP_KEY_NAME, NULL, NULL);
                if (tmp) {
                    g_free(templ->label);
                    templ->label = tmp;
                }
                tmp = g_key_file_get_locale_string(kf, G_KEY_FILE_DESKTOP_GROUP, G_KEY_FILE_DESKTOP_KEY_COMMENT, NULL,
                                                   NULL);
                if (tmp) {
                    g_free(templ->prompt);
                    templ->prompt = tmp;
...
/* requires lock held */
static void _fm_template_file_free(FmTemplate* templ, FmTemplateFile* file, gboolean do_update) {
...
static void on_dir_changed(GFileMonitor* mon, GFile* gf, GFile* other, GFileMonitorEvent evt, FmTemplateDir* dir) {
        ...
        case G_FILE_MONITOR_EVENT_CREATED:
            ...
            if (!file && basename[0] != '.' && !g_str_has_suffix(basename, "~")) {
                ...
                if (templ) {
                    ...
                    G_UNLOCK(templates);
                    fm_path_unref(path);
                    g_warning("could not guess type of template %s, ignoring it", basename);
                }
...
void _fm_templates_init(void) {
    ...
    if (!g_file_query_exists(gfile, NULL)) {
        g_warning("The directory '%s' doesn't exist, ignoring it", dir_name ? dir_name : "~/Templates");
    ...
    g_signal_connect(fm_config, "changed::template_type_once", G_CALLBACK(on_once_type_changed), NULL);
}
...
GList* fm_template_list_all(gboolean user_only) {
    ...
    G_LOCK(templates);
    for (l = templates; l; l = l->next)
        if (!((FmTemplate*)l->data)->files->inactive && (!user_only || ((FmTemplate*)l->data)->files->dir->user_dir))
 ...
 * Since: 1.2.0
 */
gboolean fm_template_create_file(FmTemplate* templ, GFile* path, GError** error, gboolean run_default) {
    char* command;
    GAppInfo* app;
    GFile* tfile;
    GList* list;
    ...
    if ((templ && !FM_IS_TEMPLATE(templ)) || !G_IS_FILE(path)) {
        g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_FAILED, _("fm_template_create_file: invalid argument"));
        return FALSE;
    }
    tfile = NULL;
    if (!templ)
    ...
    else if (!g_file_copy(tfile, path, G_FILE_COPY_TARGET_DEFAULT_PERMS, NULL, NULL, NULL, error)) {
    ...
    if (!run_default || !templ)
        return TRUE;
    if (templ->command) {
        app = g_app_info_create_from_commandline(templ->command, NULL, G_APP_INFO_CREATE_NONE, error);
    }
    else {
        app = g_app_info_get_default_for_type(fm_mime_type_get_type(templ->mime_type), FALSE);
        if (!app && error)
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_FAILED, _("No default application is set for MIME type %s"),
