...
static void fm_bookmarks_class_init(FmBookmarksClass* klass) {
     ...
     * Since: 0.1.0
     */
    signals[CHANGED] = g_signal_new("changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                                    G_STRUCT_OFFSET(FmBookmarksClass, changed), NULL, NULL,
...
static void on_changed(GFileMonitor* mon, GFile* gf, GFile* other, GFileMonitorEvent evt, FmBookmarks* bookmarks) {
    G_LOCK(bookmarks);
    /* reload bookmarks */
...
static gboolean save_bookmarks(FmBookmarks* bookmarks) {
    ...
    if (!g_file_replace_contents(bookmarks->file, buf->str, buf->len, NULL, FALSE, 0, NULL, NULL, &err)) {
