...
static void _dentry_browse_exec_event(GtkButton* button, FmFilePropertiesDEntryData* data) {
    ...
    chooser = gtk_file_chooser_dialog_new(_("Choose Executable File"), NULL, GTK_FILE_CHOOSER_ACTION_OPEN,
    ...
    gtk_file_filter_add_custom(GTK_FILE_FILTER(filter), GTK_FILE_FILTER_FILENAME, exe_filter, NULL, NULL);
...
static void _dentry_name_changed(GtkEditable* editable, FmFilePropertiesDEntryData* data) {
    ...
    if (data->lang)
        g_key_file_set_locale_string(data->kf, GRP_NAME, "Name", data->lang, gtk_entry_get_text(GTK_ENTRY(editable)));
    else
        g_key_file_set_string(data->kf, GRP_NAME, "Name", gtk_entry_get_text(GTK_ENTRY(editable)));
    data->changed = TRUE;
}
...
static void _dentry_tooltip_changed(GtkEditable* editable, FmFilePropertiesDEntryData* data) {
    ...
    if (data->lang)
        ...
        g_key_file_set_string(data->kf, GRP_NAME, "Comment", gtk_entry_get_text(GTK_ENTRY(editable)));
    data->changed = TRUE;
}

static void _dentry_exec_changed(GtkEditable* editable, FmFilePropertiesDEntryData* data) {
    /* g_debug("entry content changed"); */
    g_key_file_set_string(data->kf, GRP_NAME, "Exec", gtk_entry_get_text(GTK_ENTRY(editable)));
    data->changed = TRUE;
}

static void _dentry_genname_changed(GtkEditable* editable, FmFilePropertiesDEntryData* data) {
    /* g_debug("entry content changed"); */
    g_key_file_set_string(data->kf, GRP_NAME, "GenericName", gtk_entry_get_text(GTK_ENTRY(editable)));
    data->changed = TRUE;
}

static void _dentry_path_changed(GtkEditable* editable, FmFilePropertiesDEntryData* data) {
    /* g_debug("entry content changed"); */
    g_key_file_set_string(data->kf, GRP_NAME, "Path", gtk_entry_get_text(GTK_ENTRY(editable)));
    data->changed = TRUE;
}

static void _dentry_terminal_toggled(GtkToggleButton* togglebutton, FmFilePropertiesDEntryData* data) {
...
static void _dentry_keepterm_toggled(GtkToggleButton* togglebutton, FmFilePropertiesDEntryData* data) {
    /* g_debug("keep terminal open toggled"); */
    g_key_file_set_boolean(data->kf, GRP_NAME, "X-KeepTerminal", gtk_toggle_button_get_active(togglebutton));
    data->changed = TRUE;
}

static void _dentry_notification_toggled(GtkToggleButton* togglebutton, FmFilePropertiesDEntryData* data) {
    /* g_debug("startup notification toggled"); */
    g_key_file_set_boolean(data->kf, GRP_NAME, "StartupNotify", gtk_toggle_button_get_active(togglebutton));
    data->changed = TRUE;
}

static void _dentry_hidden_toggled(GtkToggleButton* togglebutton, FmFilePropertiesDEntryData* data) {
    /* g_debug("no display toggled"); */
    g_key_file_set_boolean(data->kf, GRP_NAME, "Hidden", gtk_toggle_button_get_active(togglebutton));
    data->changed = TRUE;
}
...
#define HIDE_WIDGET(x)                      \
    ...
    g_key_file_load_from_data(data->kf, txt, length, G_KEY_FILE_KEEP_COMMENTS | G_KEY_FILE_KEEP_TRANSLATIONS, NULL);
    g_free(txt);
...
#undef HIDE_WIDGET
    ...
    g_signal_connect(new_widget, "clicked", G_CALLBACK(_dentry_browse_exec_event), data);
    ...
    gtk_widget_set_tooltip_text(new_widget, _("Command to execute when the application icon is activated"));
    ...
    gtk_widget_set_tooltip_text(new_widget, _("The working directory to run the program in"));
...
static void _dentry_ui_finish(gpointer pdata, gboolean cancelled) {
    ...
    if (!cancelled && data->changed) {
        ...
        g_file_replace_contents(data->file, text, len, NULL, FALSE, 0, NULL, NULL, NULL);
        /* FIXME: handle errors */
        g_free(text);
    }
    ...
    if (data->hidden) {
        ...
        g_signal_handlers_disconnect_by_func(data->hidden, _dentry_hidden_toggled, data);
...
FmFilePropertiesExtensionInit fm_module_init_gtk_file_prop = {&_dentry_ui_init, &_dentry_ui_finish};
