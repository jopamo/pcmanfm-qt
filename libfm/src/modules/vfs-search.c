...
static void parse_search_uri(FmVfsSearchEnumerator* priv, const char* uri_str) {
    ...
    if (g_ascii_strncasecmp(uri_str, scheme, sizeof(scheme) - 1) == 0) {
        ...
        while (p) {
            ...
            priv->target_folders = g_slist_prepend(priv->target_folders, fm_file_new_for_commandline_arg(path));
            g_free(path);
        ...
        if (g_file_info_get_file_type(info) == G_FILE_TYPE_REGULAR && g_file_info_get_size(info) > 0) {
            ...
            if (stream) {
                if (priv->content_pattern && !priv->content_case_insensitive) {
                     ...
                     * are line-based. */
                    ret =
                        fm_search_job_match_content_line_based(priv, info, G_INPUT_STREAM(stream), cancellable, error);
                }
...
FmFileInitTable fm_module_init_vfs = {.new_for_uri = &_fm_vfs_search_new_for_uri};
