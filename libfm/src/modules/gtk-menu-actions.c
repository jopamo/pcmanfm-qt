...
static void add_custom_action_item(GString* xml,
...
#endif
                         g_app_info_get_description(item), NULL);
    ...
    g_object_set_qdata_full(G_OBJECT(act), _fm_actions_qdata_id, g_object_ref(root_menu), g_object_unref);
    ...
    if (FM_IS_ACTION_MENU(item)) {
        ...
        const GList* l;
        g_string_append_printf(xml, "<menu action='%s'>", g_app_info_get_id(item));
        ...
        g_string_append_printf(xml, "<menuitem action='%s'/>", g_app_info_get_id(item));
    }
}
...
static void _fm_actions_update_file_menu_for_scheme(GtkWindow* window,
                                                    GtkUIManager* ui,
                                                    GString* xml,
                                                    GtkActionGroup* act_grp,
                                                    FmFileMenu* menu,
    ...
    if (items) {
        ...
        for (l = items; l; l = l->next) {
            ...
            add_custom_action_item(xml, root_menu, item, act_grp, G_CALLBACK(on_custom_action_file), menu);
        }
...
static void _fm_actions_update_folder_menu_for_scheme(FmFolderView* fv,
                                                      GtkWindow* window,
                                                      GtkUIManager* ui,
                                                      GtkActionGroup* act_grp,
                                                      FmFileInfoList* files) {
    ...
    if (items) {
        ...
        for (l = items; l; l = l->next) {
            GAppInfo* item = l->data;
            add_custom_action_item(xml, root_menu, item, act_grp, G_CALLBACK(on_custom_action_folder), fv);
        }
        g_string_append(xml, "</placeholder></popup>");
