...
static void _update_file_menu_for_trash(GtkWindow* window,
                                        GtkUIManager* ui,
                                        GString* xml,
    ...
    for (l = fm_file_info_list_peek_head_link(files); l; l = l->next) {
        ...
        if (!fm_path_get_parent(trash_path) || !fm_path_is_trash_root(fm_path_get_parent(trash_path))) {
            can_restore = FALSE;
            break;
        }
    }
    if (can_restore) {
        act = gtk_action_new("UnTrash", _("_Restore"), _("Restore trashed files to original paths"), NULL);
        g_signal_connect(act, "activate", G_CALLBACK(on_untrash), menu);
        gtk_action_group_add_action(act_grp, act);
        g_object_unref(act);
        g_string_append(xml,
                        "<popup><placeholder name='ph1'>"
                        "<menuitem action='UnTrash'/>"
                        "</placeholder></popup>");
    }
    else if (is_trash_root) {
        act = gtk_action_new("EmptyTrash", _("_Empty Trash Can"), NULL, NULL);
...
static void _update_folder_menu_for_trash(FmFolderView* fv,
                                          GtkWindow* window,
                                          GtkUIManager* ui,
                                          GtkActionGroup* act_grp,
                                          FmFileInfoList* files) {
...
FmContextMenuSchemeAddonInit fm_module_init_gtk_menu_scheme = {NULL, NULL, _update_file_menu_for_trash,
