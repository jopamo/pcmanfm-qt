...
static void _shortcut_name_changed(GtkEditable* editable, FmFilePropertiesShortcutData* data) {
    ...
    if (data->lang)
        ...
        g_key_file_set_string(data->kf, G_KEY_FILE_DESKTOP_GROUP, "Name", gtk_entry_get_text(GTK_ENTRY(editable)));
    data->changed = TRUE;
}

static void _shortcut_hidden_toggled(GtkToggleButton* togglebutton, FmFilePropertiesShortcutData* data) {
...
static gpointer _shortcut_ui_init(GtkBuilder* ui, gpointer uidata, FmFileInfoList* files) {
    ...
    kf = g_key_file_new();
    ok = g_key_file_load_from_data(kf, txt, length, G_KEY_FILE_KEEP_COMMENTS | G_KEY_FILE_KEEP_TRANSLATIONS, NULL);
    g_free(txt);
    if (!ok || !g_key_file_has_group(kf, G_KEY_FILE_DESKTOP_GROUP))
        goto _not_the_file;
    txt = g_key_file_get_string(kf, G_KEY_FILE_DESKTOP_GROUP, G_KEY_FILE_DESKTOP_KEY_TYPE, NULL);
    if (txt == NULL)
        goto _not_the_file;
...
static void _shortcut_ui_finish(gpointer pdata, gboolean cancelled) {
    ...
    if (data->hidden) {
        ...
        g_signal_handlers_disconnect_by_func(data->hidden, _shortcut_hidden_toggled, data);
        /* disable default handler returning previous value */
        gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(data->hidden), data->was_hidden);
    }
...
FmFilePropertiesExtensionInit fm_module_init_gtk_file_prop = {.init = &_shortcut_ui_init,
