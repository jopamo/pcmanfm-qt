...
static gboolean _menu_xml_handler_pass(FmXmlFileItem* item,
                                       GList* children,
...
static FmXmlFileItem* _create_path_in_tree(FmXmlFileItem* parent, const char* path) {
    ...
    item = fm_xml_file_item_new(menuTag_Menu);
    if (!fm_xml_file_item_append_child(parent, item) || !fm_xml_file_item_append_child(item, name)) {
...
static FmXmlFileItem* _prepare_contents(FmMenuMenuTree* data, GCancellable* cancellable, GError** error, GFile** gf) {
    ...
    contents = g_strdup_printf("%sapplications.menu", xdg_menu_prefix ? xdg_menu_prefix : "lxde-");
    ...
    *gf = g_file_new_for_path(data->file_path);
    ...
    menuTag_Menu = fm_xml_file_set_handler(data->menu, "Menu", &_menu_xml_handler_pass, FALSE, NULL);
    menuTag_Name = fm_xml_file_set_handler(data->menu, "Name", &_menu_xml_handler_pass, FALSE, NULL);
    menuTag_Deleted = fm_xml_file_set_handler(data->menu, "Deleted", &_menu_xml_handler_pass, FALSE, NULL);
    menuTag_NotDeleted = fm_xml_file_set_handler(data->menu, "NotDeleted", &_menu_xml_handler_pass, FALSE, NULL);
    menuTag_Directory = fm_xml_file_set_handler(data->menu, "Directory", &_menu_xml_handler_pass, FALSE, NULL);
    menuTag_Include = fm_xml_file_set_handler(data->menu, "Include", &_menu_xml_handler_pass, FALSE, NULL);
    menuTag_Exclude = fm_xml_file_set_handler(data->menu, "Exclude", &_menu_xml_handler_pass, FALSE, NULL);
    menuTag_Filename = fm_xml_file_set_handler(data->menu, "Filename", &_menu_xml_handler_pass, FALSE, NULL);
    menuTag_MergeFile = fm_xml_file_set_handler(data->menu, "MergeFile", &_menu_xml_handler_pass, FALSE, NULL);
    menuTag_Category = fm_xml_file_set_handler(data->menu, "Category", &_menu_xml_handler_pass, FALSE, NULL);
    ...
    if (xml == NULL) /* error is set by failed function */
    {
        ...
        g_prefix_error(error, _("XML file '%s' error (%d:%d): "), data->file_path, data->line, data->pos);
    }
    else {
        ...
        if (apps)
            return apps;
        g_set_error_literal(error, G_FILE_ERROR, G_FILE_ERROR_NOENT, _("XML file doesn't contain Applications root"));
    }
    return NULL;
}
...
static gboolean _save_new_menu_file(GFile* gf, FmXmlFile* file, GCancellable* cancellable, GError** error) {
    gsize len;
    ...
    result = g_file_replace_contents(gf, contents, len, NULL, FALSE, G_FILE_CREATE_REPLACE_DESTINATION, NULL,
                                     cancellable, error);
    g_free(contents);
    return result;
}
...
static gboolean _remove_directory(const char* path, GCancellable* cancellable, GError** error) {
    GList *xml = NULL, *it;
    GFile* gf;
    ...
    if (apps == NULL) {
    ...
    else if ((xml = fm_xml_file_item_get_children(apps)) != NULL && (item = _find_in_children(xml, path)) != NULL) {
        ...
        if (item == NULL) {
            g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED, _("Cannot create XML definition for '%s'"), path);
            ok = FALSE;
        }
        else {
            FmXmlFileItem* item2;
...
/* changes .menu XML file */
static gboolean _add_directory(const char* path, GCancellable* cancellable, GError** error) {
    GList *xml = NULL, *it;
    GFile* gf;
    ...
    if (apps == NULL) {
        ...
        if (!ok) /* see FIXME above */
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_EXISTS, _("Menu path '%s' already exists"), path);
        else {
        ...
        item = _create_path_in_tree(apps, path);
        if (item == NULL) {
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_EXISTS, _("Cannot create XML definition for '%s'"), path);
            ok = FALSE;
        }
        else {
            ...
            dir = g_build_filename(g_get_user_data_dir(), "desktop-directories", pathtag, NULL);
...
/* changes .menu XML file */
static gboolean _add_application(const char* path, GCancellable* cancellable, GError** error) {
    const char* id;
    char* dir;
    GList *xml = NULL, *it;
    GFile* gf;
    ...
    else if ((xml = fm_xml_file_item_get_children(apps)) != NULL && (item = _find_in_children(xml, dir)) != NULL) {
...
/* changes .menu XML file */
static gboolean _remove_application(const char* path, GCancellable* cancellable, GError** error) {
    const char* id;
    char* dir;
    GList *xml = NULL, *it;
    GFile* gf;
...
#define FM_MENU_VFILE(o) (G_TYPE_CHECK_INSTANCE_CAST((o), FM_TYPE_MENU_VFILE, FmMenuVFile))
...
G_DEFINE_TYPE_WITH_CODE(FmMenuVFile,
                        fm_vfs_menu_file,
                        G_TYPE_OBJECT,
...
#define FM_VFS_MENU_ENUMERATOR(o) (G_TYPE_CHECK_INSTANCE_CAST((o), FM_TYPE_VFS_MENU_ENUMERATOR, FmVfsMenuEnumerator))
...
static GFileInfo* _g_file_info_from_menu_cache_item(
    MenuCacheItem* item,
...
#if MENU_CACHE_CHECK_VERSION(0, 5, 0)
        g_file_info_set_is_hidden(fileinfo, !menu_cache_dir_is_visible(MENU_CACHE_DIR(item)));
#else
    ...
    else /* MENU_CACHE_TYPE_APP */
    {
        ...
        g_file_info_set_attribute_string(fileinfo, G_FILE_ATTRIBUTE_STANDARD_TARGET_URI, path);
        g_free(path);
        g_file_info_set_content_type(fileinfo, "application/x-desktop");
        g_file_info_set_is_hidden(fileinfo, !menu_cache_app_get_is_visible(MENU_CACHE_APP(item), de_flag));
    }
    // FIXME: use attribute_matcher and set G_FILE_ATTRIBUTE_ACCESS_CAN_{WRITE,READ,EXECUTE,DELETE}
    g_file_info_set_attribute_string(fileinfo, G_FILE_ATTRIBUTE_ID_FILESYSTEM, "menu-Applications");
    g_file_info_set_attribute_boolean(fileinfo, G_FILE_ATTRIBUTE_ACCESS_CAN_RENAME, TRUE);
    g_file_info_set_attribute_boolean(fileinfo, G_FILE_ATTRIBUTE_ACCESS_CAN_TRASH, FALSE);
    return fileinfo;
}
...
static gboolean _fm_vfs_menu_enumerator_close(GFileEnumerator* enumerator, GCancellable* cancellable, GError** error) {
...
static gboolean _fm_vfs_menu_enumerator_new_real(gpointer data) {
    ...
    enumerator = g_object_new(FM_TYPE_VFS_MENU_ENUMERATOR, "container", init->file, NULL);
    enumerator->mc = mc;
...
#define ERROR_UNSUPPORTED(err) \
    g_set_error_literal(err, G_IO_ERROR, G_IO_ERROR_NOT_SUPPORTED, _("Operation not supported"))
...
static GFile* _fm_vfs_menu_resolve_relative_path(GFile* file, const char* relative_path) {
    ...
    if (relative_path == NULL || *relative_path == '\0')
        ...
        char* escaped = g_uri_escape_string(relative_path, G_URI_RESERVED_CHARS_ALLOWED_IN_PATH, TRUE);
...
#endif
    if (is_invalid)
        g_set_error_literal(init->error, G_IO_ERROR, G_IO_ERROR_NOT_FOUND, _("Invalid menu directory"));
    else if (dir) {
#if MENU_CACHE_CHECK_VERSION(0, 5, 0)
        MenuCacheItem* item = menu_cache_find_child_by_name(MENU_CACHE_DIR(dir), init->display_name);
        g_debug("searched for child '%s' found '%s'", init->display_name,
                item ? menu_cache_item_get_id(item) : "(nil)");
        if (item == NULL)
            init->result = _fm_vfs_menu_resolve_relative_path(init->file, init->display_name);
        else {
            init->result = _fm_vfs_menu_resolve_relative_path(init->file, menu_cache_item_get_id(item));
        ...
        if (l == NULL) /* not found */
            ...
            init->result = _fm_vfs_menu_resolve_relative_path(init->file, menu_cache_item_get_id(l->data));
#if MENU_CACHE_CHECK_VERSION(0, 4, 0)
        ...
        g_set_error_literal(init->error, G_IO_ERROR, G_IO_ERROR_FAILED, _("Menu cache error"));
...
static GFile* _fm_vfs_menu_get_child_for_display_name(GFile* file, const char* display_name, GError** error) {
    FmMenuVFile* item = FM_MENU_VFILE(file);
    FmVfsMenuMainThreadData enu;

    /* g_debug("_fm_vfs_menu_get_child_for_display_name: '%s' '%s'", item->path, display_name); */
    if (display_name == NULL || *display_name == '\0') {
        g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_FAILED, _("Menu item name cannot be empty"));
        return NULL;
    }
...
#endif
    if (is_invalid)
        g_set_error(init->error, G_IO_ERROR, G_IO_ERROR_NOT_FOUND, _("Invalid menu directory '%s'"), init->path_str);
    else if (dir) {
        const char* de_name = g_getenv("XDG_CURRENT_DESKTOP");

        if (de_name)
            init->result = _g_file_info_from_menu_cache_item(dir, menu_cache_get_desktop_env_flag(mc, de_name));
        else
...
static GFileInfo* _fm_vfs_menu_query_info(GFile* file,
    ...
    if (item->path == NULL) /* menu root */
    {
        ...
        if (g_file_attribute_matcher_matches(matcher, G_FILE_ATTRIBUTE_ID_FILESYSTEM))
            g_file_info_set_attribute_string(info, G_FILE_ATTRIBUTE_ID_FILESYSTEM, "menu-Applications");
...
static GMount* _fm_vfs_menu_find_enclosing_mount(GFile* file, GCancellable* cancellable, GError** error) {
    ERROR_UNSUPPORTED(error);
    return NULL;
}
...
static gboolean _fm_vfs_menu_set_display_name_real(gpointer data) {
    ...
    dir = _vfile_path_to_menu_cache_item(mc, init->path_str);
    if (dir == NULL)
        g_set_error_literal(init->error, G_IO_ERROR, G_IO_ERROR_NOT_FOUND, _("Invalid menu item"));
    else if (menu_cache_item_get_file_basename(dir) == NULL || menu_cache_item_get_file_dirname(dir) == NULL)
        g_set_error(init->error, G_IO_ERROR, G_IO_ERROR_NOT_SUPPORTED,
                    _("The menu item '%s' doesn't have appropriate entry file"), menu_cache_item_get_id(dir));
    else if (!g_cancellable_set_error_if_cancelled(init->cancellable, init->error)) {
        char* path = menu_cache_item_get_file_path(dir);
        GKeyFile* kf = g_key_file_new();

        ok = g_key_file_load_from_file(kf, path, G_KEY_FILE_KEEP_COMMENTS | G_KEY_FILE_KEEP_TRANSLATIONS, init->error);
        g_free(path);
        if (ok) {
            /* get locale name */
            ...
            if (strcmp(langs[0], "C") != 0) {
                ...
                g_key_file_set_locale_string(kf, G_KEY_FILE_DESKTOP_GROUP, G_KEY_FILE_DESKTOP_KEY_NAME, lang,
                                             init->display_name);
                g_free(lang);
            }
            else
                g_key_file_set_string(kf, G_KEY_FILE_DESKTOP_GROUP, G_KEY_FILE_DESKTOP_KEY_NAME, init->display_name);
...
static gboolean _fm_vfs_menu_set_attributes_from_info_real(gpointer data) {
    ...
    if (g_file_info_get_attribute_data(init->info, G_FILE_ATTRIBUTE_STANDARD_DISPLAY_NAME, NULL, &value, NULL))
        display_name = value;
    if (g_file_info_get_attribute_data(init->info, G_FILE_ATTRIBUTE_STANDARD_ICON, NULL, &value, NULL))
        icon = value;
    if (g_file_info_get_attribute_data(init->info, G_FILE_ATTRIBUTE_STANDARD_IS_HIDDEN, NULL, &value, NULL))
    ...
    if (item == NULL)
        g_set_error_literal(init->error, G_IO_ERROR, G_IO_ERROR_NOT_FOUND, _("Invalid menu item"));
    else if (menu_cache_item_get_file_basename(item) == NULL || menu_cache_item_get_file_dirname(item) == NULL)
        g_set_error(init->error, G_IO_ERROR, G_IO_ERROR_NOT_SUPPORTED,
                    _("The menu item '%s' doesn't have appropriate entry file"), menu_cache_item_get_id(item));
    else if (!g_cancellable_set_error_if_cancelled(init->cancellable, init->error)) {
        char* path;
        GKeyFile* kf;
        GError* err = NULL;
        ...
        kf = g_key_file_new();
        ok = g_key_file_load_from_file(kf, path, G_KEY_FILE_KEEP_COMMENTS | G_KEY_FILE_KEEP_TRANSLATIONS, &err);
        g_free(path);
        if (ok) /* otherwise there is no reason to continue */
        {
            ...
            if (display_name) {
                ...
                if (strcmp(langs[0], "C") != 0) {
                    ...
                    g_key_file_set_string(kf, G_KEY_FILE_DESKTOP_GROUP, G_KEY_FILE_DESKTOP_KEY_NAME, display_name);
            }
            if (icon) {
                char* icon_str = g_icon_to_string(icon);
                /* FIXME: need to change encoding in some cases? */
                g_key_file_set_string(kf, G_KEY_FILE_DESKTOP_GROUP, G_KEY_FILE_DESKTOP_KEY_ICON, icon_str);
                g_free(icon_str);
            }
            if (set_hidden >= 0) {
                g_key_file_set_boolean(kf, G_KEY_FILE_DESKTOP_GROUP, G_KEY_FILE_DESKTOP_KEY_NO_DISPLAY,
                                       (set_hidden > 0));
            }
...
static gboolean _fm_vfs_menu_set_attribute(GFile* file,
    ...
    result = _fm_vfs_menu_set_attributes_from_info(file, info, flags, cancellable, error);
    g_object_unref(info);
    return result;

_invalid_arg:
    g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, _("Invalid value for attribute '%s'"), attribute);
    return FALSE;
}
...
static gboolean _fm_vfs_menu_read_fn_real(gpointer data) {
    ...
    if (item != NULL && menu_cache_item_get_type(item) == MENU_CACHE_TYPE_DIR)
        g_set_error(init->error, G_IO_ERROR, G_IO_ERROR_IS_DIRECTORY, _("The '%s' is a menu directory"),
                    init->path_str);
    else if (item == NULL || menu_cache_item_get_type(item) != MENU_CACHE_TYPE_APP)
        g_set_error(init->error, G_IO_ERROR, G_IO_ERROR_NOT_FOUND, _("The '%s' isn't a menu item"),
...
static GFileInputStream* _fm_vfs_menu_read_fn(GFile* file, GCancellable* cancellable, GError** error) {
    FmMenuVFile* item = FM_MENU_VFILE(file);
    FmVfsMenuMainThreadData enu;
...
#define FM_MENU_VFILE_OUTPUT_STREAM(o) \
    (G_TYPE_CHECK_INSTANCE_CAST((o), FM_TYPE_MENU_VFILE_OUTPUT_STREAM, FmMenuVFileOutputStream))
...
static gboolean fm_vfs_menu_file_output_stream_close(GOutputStream* gos, GCancellable* cancellable, GError** error) {
    ...
    if (!g_key_file_has_key(kf, G_KEY_FILE_DESKTOP_GROUP, G_KEY_FILE_DESKTOP_KEY_NAME, NULL))
        g_key_file_set_string(kf, G_KEY_FILE_DESKTOP_GROUP, G_KEY_FILE_DESKTOP_KEY_NAME, "");
    if (!g_key_file_has_key(kf, G_KEY_FILE_DESKTOP_GROUP, G_KEY_FILE_DESKTOP_KEY_EXEC, NULL))
        g_key_file_set_string(kf, G_KEY_FILE_DESKTOP_GROUP, G_KEY_FILE_DESKTOP_KEY_EXEC, "");
    ...
    if (!content)
        return FALSE;
    ok = g_output_stream_write_all(stream->real_stream, content, len, &len, cancellable, error);
    g_free(content);
...
static GFileOutputStream* _vfile_menu_create(GFile* file,
    ...
    if (ostream == NULL) {
        if (g_cancellable_is_cancelled(cancellable) || err->domain != G_IO_ERROR || err->code != G_IO_ERROR_NOT_FOUND) {
...
static gboolean _fm_vfs_menu_create_real(gpointer data) {
    ...
    if (is_invalid)
        g_set_error(init->error, G_IO_ERROR, G_IO_ERROR_EXISTS, _("Cannot create menu item '%s'"),
                    init->path_str ? init->path_str : "/");
    else {
        GFile* gf = _g_file_new_for_id(id);

        if (gf) {
            init->result = _vfile_menu_create(gf, G_FILE_CREATE_NONE, init->cancellable, init->error, unescaped);
            g_object_unref(gf);
        }
    }
    g_free(unescaped);
...
static gboolean _fm_vfs_menu_replace_real(gpointer data) {
    ...
    if (is_invalid)
        ...
        if (gf) {
            ...
            init->result =
                _vfile_menu_replace(gf, NULL, FALSE, G_FILE_CREATE_REPLACE_DESTINATION, init->cancellable, init->error,
...
static GKeyFile* _g_key_file_from_item(GFile* file, GCancellable* cancellable, GError** error) {
    char* contents;
    gsize length;
    GKeyFile* kf;
    ...
    if (!g_file_load_contents(file, cancellable, &contents, &length, NULL, error))
        return NULL;
    kf = g_key_file_new();
    if (!g_key_file_load_from_data(kf, contents, length, G_KEY_FILE_KEEP_COMMENTS | G_KEY_FILE_KEEP_TRANSLATIONS,
                                   error)) {
        g_key_file_free(kf);
        kf = NULL;
    }
...
static gboolean _g_key_file_into_item(GFile* file, GKeyFile* kf, GCancellable* cancellable, GError** error) {
    char* contents;
    gsize length;
    gboolean result = FALSE;
    ...
    if (contents == NULL)
        return FALSE;
    result = g_file_replace_contents(file, contents, length, NULL, FALSE, G_FILE_CREATE_REPLACE_DESTINATION, NULL,
                                     cancellable, error);
    g_free(contents);
    return result;
}

static gboolean _fm_vfs_menu_delete_file(GFile* file, GCancellable* cancellable, GError** error) {
    FmMenuVFile* item = FM_MENU_VFILE(file);
    GKeyFile* kf;
    GError* err = NULL;
    ...
    g_key_file_set_boolean(kf, G_KEY_FILE_DESKTOP_GROUP, G_KEY_FILE_DESKTOP_KEY_NO_DISPLAY, TRUE);
    return _g_key_file_into_item(file, kf, cancellable, error);
}

static gboolean _fm_vfs_menu_trash(GFile* file, GCancellable* cancellable, GError** error) {
    ERROR_UNSUPPORTED(error);
    return FALSE;
}

static gboolean _fm_vfs_menu_make_directory(GFile* file, GCancellable* cancellable, GError** error) {
#if !MENU_CACHE_CHECK_VERSION(0, 5, 0)
...
static gboolean _fm_vfs_menu_move_real(gpointer data) {
    ...
    if (init->path_str == NULL || dst_path == NULL) {
        g_set_error_literal(init->error, G_IO_ERROR, G_IO_ERROR_FAILED, _("Invalid operation with menu root"));
        return FALSE;
    }
    /* make path strings */
    ...
    if (item == NULL || menu_cache_item_get_type(item) != MENU_CACHE_TYPE_APP) {
        ...
        g_set_error(init->error, G_IO_ERROR, G_IO_ERROR_NOT_FOUND, _("The '%s' isn't a menu item"), init->path_str);
        goto _failed;
    }
    item2 = _vfile_path_to_menu_cache_item(mc, init->destination->path);
    if (item2) {
        g_set_error(init->error, G_IO_ERROR, G_IO_ERROR_EXISTS, _("Menu path '%s' already exists"), dst_path);
#if MENU_CACHE_CHECK_VERSION(0, 4, 0)
        menu_cache_item_unref(item2);
#endif
        goto _failed;
...
static gboolean _fm_vfs_menu_move(GFile* source,
    ...
    if (!FM_IS_FILE(destination)) {
        g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, _("Invalid destination"));
        return FALSE;
    }
    enu.path_str = item->path;
    enu.cancellable = cancellable;
...
#define FM_MENU_VFILE_MONITOR(o) (G_TYPE_CHECK_INSTANCE_CAST((o), FM_TYPE_MENU_VFILE_MONITOR, FmMenuVFileMonitor))
...
#endif
{
    ...
    {
        menu_cache_item_unref(dir);
        g_file_monitor_emit_event(G_FILE_MONITOR(mon), G_FILE(mon->file), NULL, G_FILE_MONITOR_EVENT_DELETED);
        return;
    }
    ...
    g_file_monitor_emit_event(G_FILE_MONITOR(mon), G_FILE(mon->file), NULL, G_FILE_MONITOR_EVENT_CHANGED);
    items = mon->items;
    mon->items =
        g_slist_copy_deep(menu_cache_dir_get_children(MENU_CACHE_DIR(dir)), (GCopyFunc)menu_cache_item_ref, NULL);
    ...
    while (ol) {
        for (nl = new_items; nl; nl = nl->next)
            if (strcmp(menu_cache_item_get_id(ol->data), menu_cache_item_get_id(nl->data)) == 0)
                break; /* the same id found */
        if (nl) {
            /* check if any visible attribute of it was changed */
            if (g_strcmp0(menu_cache_item_get_name(ol->data), menu_cache_item_get_name(nl->data)) == 0 ||
                g_strcmp0(menu_cache_item_get_icon(ol->data), menu_cache_item_get_icon(nl->data)) == 0 ||
                menu_cache_app_get_is_visible(ol->data, de_flag) != menu_cache_app_get_is_visible(nl->data, de_flag)) {
                file = _fm_vfs_menu_resolve_relative_path(G_FILE(mon->file), menu_cache_item_get_id(nl->data));
                g_file_monitor_emit_event(G_FILE_MONITOR(mon), file, NULL, G_FILE_MONITOR_EVENT_ATTRIBUTE_CHANGED);
                g_object_unref(file);
            }
    ...
    while (items) {
        file = _fm_vfs_menu_resolve_relative_path(G_FILE(mon->file), menu_cache_item_get_id(items->data));
        g_file_monitor_emit_event(G_FILE_MONITOR(mon), file, NULL, G_FILE_MONITOR_EVENT_DELETED);
        g_object_unref(file);
    ...
    while (new_items) {
        file = _fm_vfs_menu_resolve_relative_path(G_FILE(mon->file), menu_cache_item_get_id(new_items->data));
        g_file_monitor_emit_event(G_FILE_MONITOR(mon), file, NULL, G_FILE_MONITOR_EVENT_CREATED);
        g_object_unref(file);
...
static gboolean _fm_vfs_menu_monitor_dir_real(gpointer data) {
...
#endif
    {
        g_set_error(init->error, G_IO_ERROR, G_IO_ERROR_FAILED,
                    _("FmMenuVFileMonitor: folder '%s' not found in menu cache"), mon->file->path);
        goto _fail;
    }
#if !MENU_CACHE_CHECK_VERSION(0, 4, 0)
    ...
    mon->notifier = menu_cache_add_reload_notify(mon->cache, &_reload_notify_handler, mon);
    init->result = mon;
    return TRUE;
...
static GFileIOStream* _fm_vfs_menu_open_readwrite(GFile* file, GCancellable* cancellable, GError** error) {
    ERROR_UNSUPPORTED(error);
    return NULL;
}
...
static void fm_menu_g_file_init(GFileIface* iface) {
    ...
    g_file_attribute_info_list_add(list, G_FILE_ATTRIBUTE_STANDARD_IS_HIDDEN, G_FILE_ATTRIBUTE_TYPE_BOOLEAN,
                                   G_FILE_ATTRIBUTE_INFO_COPY_WHEN_MOVED);
    g_file_attribute_info_list_add(list, G_FILE_ATTRIBUTE_STANDARD_ICON, G_FILE_ATTRIBUTE_TYPE_OBJECT,
                                   G_FILE_ATTRIBUTE_INFO_COPY_WHEN_MOVED);
...
FmFileInitTable fm_module_init_vfs = {.new_for_uri = &_fm_vfs_menu_new_for_uri};
