...
static void g_udisks_volume_monitor_init(GUDisksVolumeMonitor* self) {
    ...
    if (self->con) {
        ...
        self->udisks_proxy = dbus_g_proxy_new_for_name(self->con, "org.freedesktop.UDisks", "/org/freedesktop/UDisks",
...
static void add_device(GUDisksVolumeMonitor* mon, DBusGProxy* proxy, const char* obj_path, gboolean emit_signal) {
    if (!find_device(mon, obj_path)) {
        DBusGProxy* dev_proxy =
            dbus_g_proxy_new_for_name(mon->con, "org.freedesktop.UDisks", obj_path, "org.freedesktop.DBus.Properties");
        GError* err = NULL;
