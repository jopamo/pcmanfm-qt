...
static void g_udisks_volume_eject_with_operation(GVolume* base,
...
G_DEFINE_TYPE_EXTENDED(GUDisksVolume,
                       g_udisks_volume,
                       G_TYPE_OBJECT,
                       0,
                       G_IMPLEMENT_INTERFACE(G_TYPE_VOLUME, g_udisks_volume_volume_iface_init))
...
static void g_udisks_volume_eject(GVolume* base,
                                  GMountUnmountFlags flags,
                                  GCancellable* cancellable,
                                  GAsyncReadyCallback callback,
                                  gpointer user_data) {
...
static void g_udisks_volume_eject_with_operation(GVolume* base,
    ...
    if (vol->drive && g_drive_can_eject(G_DRIVE(vol->drive))) {
        ...
        g_drive_eject_with_operation(G_DRIVE(vol->drive), flags, mount_operation, cancellable, on_drive_ejected, data);
    }
}
...
static void mount_callback(DBusGProxy* proxy, char* OUT_mount_path, GError* error, gpointer user_data) {
    ...
    if (error) {
        ...
        res = g_simple_async_result_new_from_error(G_OBJECT(data->vol), data->callback, data->user_data, error);
        g_error_free(error);
    }
    else {
        res = g_simple_async_result_new(G_OBJECT(data->vol), data->callback, data->user_data, NULL);
...
static void g_udisks_volume_mount_fn(GVolume* base,
    ...
    data->call = org_freedesktop_UDisks_Device_filesystem_mount_async(proxy, dev->type ? dev->type : "auto", NULL,
                                                                      mount_callback, data);
    if (cancellable) {
