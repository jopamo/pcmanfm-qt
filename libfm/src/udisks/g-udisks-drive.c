...
G_DEFINE_TYPE_EXTENDED(GUDisksDrive,
                       g_udisks_drive,
                       G_TYPE_OBJECT,
                       0,
                       G_IMPLEMENT_INTERFACE(G_TYPE_DRIVE, g_udisks_drive_drive_iface_init))
...
static void on_ejected(DBusGProxy* proxy, GError* error, gpointer user_data) {
    ...
    if (error) {
        ...
        res = g_simple_async_result_new_from_error(G_OBJECT(data->drv), data->callback, data->user_data, error);
        g_error_free(error);
    }
    else {
        res = g_simple_async_result_new(G_OBJECT(data->drv), data->callback, data->user_data, NULL);
...
static void do_eject(EjectData* data) {
    ...
    data->call = org_freedesktop_UDisks_Device_drive_eject_async(data->proxy, NULL, on_ejected, data);
}
...
#define data ((EjectData*)input_data)
    ...
    if (success) {
        ...
        g_error_free(err);
        res = g_simple_async_result_new_from_error(G_OBJECT(data->drv), data->callback, data->user_data, err);
        finish_eject(res, data);
        g_error_free(error);
    }
#undef data
...
static void unmount_before_eject(EjectData* data) {
    ...
    g_mount_unmount_with_operation(mnt, data->flags, data->op, data->cancellable, on_unmounted, data);
...
static void g_udisks_drive_eject_with_operation(GDrive* base,
                                                GMountUnmountFlags flags,
...
static void g_udisks_drive_eject(GDrive* base,
                                 GMountUnmountFlags flags,
                                 GCancellable* cancellable,
                                 GAsyncReadyCallback callback,
                                 gpointer user_data) {
...
static void g_udisks_drive_poll_for_media(GDrive* base,
                                          GCancellable* cancellable,
                                          GAsyncReadyCallback callback,
                                          gpointer user_data) {
    // GUDisksDrive* drv = G_UDISKS_DRIVE(base);
...
static void g_udisks_drive_start(GDrive* base,
...
static void g_udisks_drive_stop(GDrive* base,
                                GMountUnmountFlags flags,
                                GMountOperation* mount_operation,
                                GCancellable* cancellable,
                                GAsyncReadyCallback callback,
