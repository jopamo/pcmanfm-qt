...
static void g_udisks_mount_unmount_with_operation(GMount* base,
...
G_DEFINE_TYPE_EXTENDED(GUDisksMount,
                       g_udisks_mount,
                       G_TYPE_OBJECT,
                       0,
                       G_IMPLEMENT_INTERFACE(G_TYPE_MOUNT, g_udisks_mount_mount_iface_init))
...
static void g_udisks_mount_eject_with_operation(GMount* base,
                                                GMountUnmountFlags flags,
                                                GMountOperation* mount_operation,
                                                GCancellable* cancellable,
                                                GAsyncReadyCallback callback,
    ...
    if (drv) {
        ...
        g_drive_eject_with_operation(drv, flags, mount_operation, cancellable, on_drive_ejected, data);
        g_object_unref(drv);
    }
}
...
static void g_udisks_mount_eject(GMount* base,
                                 GMountUnmountFlags flags,
                                 GCancellable* cancellable,
                                 GAsyncReadyCallback callback,
                                 gpointer user_data) {
...
static gboolean guess_content_job(GIOSchedulerJob* job, GCancellable* cancellable, gpointer user_data) {
    ...
    res = g_simple_async_result_new(G_OBJECT(data->mnt), data->callback, data->user_data, NULL);
...
static void g_udisks_mount_guess_content_type(GMount* base,
...
static gchar** g_udisks_mount_guess_content_type_sync(GMount* base,
                                                      gboolean force_rescan,
                                                      GCancellable* cancellable,
                                                      GError** error) {
    GUDisksMount* mnt = G_UDISKS_MOUNT(base);
...
// static void g_udisks_mount_remount (GMount* base, GMountMountFlags flags, GMountOperation* mount_operation,
...
static void unmount_callback(DBusGProxy* proxy, GError* error, gpointer user_data) {
    ...
    if (error) {
        ...
        res = g_simple_async_result_new_from_error(G_OBJECT(data->mnt), data->callback, data->user_data, error);
        g_error_free(error);
    }
    else {
        res = g_simple_async_result_new(G_OBJECT(data->mnt), data->callback, data->user_data, NULL);
...
static void g_udisks_mount_unmount_with_operation(GMount* base,
    ...
    if (mnt->vol) {
        ...
        data->call = org_freedesktop_UDisks_Device_filesystem_unmount_async(proxy, NULL, unmount_callback, data);
        if (cancellable)
...
static void g_udisks_mount_unmount(GMount* base,
                                   GMountUnmountFlags flags,
                                   GCancellable* cancellable,
                                   GAsyncReadyCallback callback,
                                   gpointer user_data) {
