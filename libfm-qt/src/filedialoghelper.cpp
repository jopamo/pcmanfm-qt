...
void FileDialogHelper::applyOptions() {
    ...
    if (opt->windowTitle().isEmpty()) {
        dlg_->setWindowTitle(opt->acceptMode() == QFileDialogOptions::AcceptOpen ? tr("Open File") : tr("Save File"));
    }
    else {
...
void FileDialogHelper::loadSettings() {
    ...
    dlg_->sort(sortColumnFromString(settings.value("SortColumn"_L1).toString()),
               sortOrderFromString(settings.value("SortOrder"_L1).toString()));
