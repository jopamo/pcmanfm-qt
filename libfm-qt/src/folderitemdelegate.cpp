...
QSize FolderItemDelegate::iconViewTextSize(const QModelIndex& index) const {
    ...
    QRectF textRect(0, 0, itemSize_.width() - 2 * margins_.width(),
...
void FolderItemDelegate::paint(QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const {
    ...
    if (option.decorationPosition == QStyleOptionViewItem::Top ||
        ...
        QPoint iconPos(opt.rect.x() + (opt.rect.width() - option.decorationSize.width()) / 2,
                       opt.rect.y() + margins_.height());
        ...
        if (untrusted) {
            ...
            untrustedIcon_.paint(painter, iconRect.translated(0, option.decorationSize.height() / 2), Qt::AlignCenter,
                                 iconMode);
        }

        if (file && file->canUnmount()) {
            // emblem for mounted mountable files
            mountedIcon_.paint(painter, iconRect.translated(option.decorationSize.width() / 2, 0), Qt::AlignCenter,
                               iconMode);
        }
        ...
        if (!emblems.empty()) {
            ...
            emblem.paint(painter,
                         iconRect.translated(option.decorationSize.width() / 2, option.decorationSize.height() / 2),
        ...
        if (iv != nullptr
            ...
            iconPos = QPoint(std::max(opt.rect.x(), iconPos.x() - s), std::max(opt.rect.y(), iconPos.y() - s));
            QPoint curPos = iv->viewport()->mapFromGlobal(QCursor::pos());
            if (curPos.x() >= iconPos.x() && curPos.x() <= iconPos.x() + s && curPos.y() >= iconPos.y() &&
                curPos.y() <= iconPos.y() + s) {
        ...
        QRectF textRect(opt.rect.x() + (opt.rect.width() - drawAreaSize.width()) / 2,
                        opt.rect.y() + margins_.height() + option.decorationSize.height(), drawAreaSize.width(),
    ...
    if (editor && event->type() == QEvent::KeyPress) {
        ...
        // go to first/last position with Home/End key in the text-edit
            ...
            if (auto textEdit = qobject_cast<QTextEdit*>(editor)) {
                ...
                txtCur.movePosition(
                    k == Qt::Key_Home ? QTextCursor::Start : QTextCursor::End,
                    ke->modifiers() == Qt::ShiftModifier ? QTextCursor::KeepAnchor : QTextCursor::MoveAnchor);
...
void FolderItemDelegate::updateEditorGeometry(QWidget* editor,
                                              const QStyleOptionViewItem& option,
                                              const QModelIndex& index) const {
    if (option.decorationPosition == QStyleOptionViewItem::Top ||
        option.decorationPosition == QStyleOptionViewItem::Bottom) {
        // give all of the available space to the editor
        QRect textRect(option.rect.x(), option.rect.y() + margins_.height() + option.decorationSize.height(),
                       itemSize_.width(), itemSize_.height() - margins_.height() - option.decorationSize.height());
        ...
        opt.showDecorationSelected =
            editor->style()->styleHint(QStyle::SH_ItemView_ShowDecorationSelected, nullptr, editor);
        const QWidget* widget = option.widget;
        QStyle* style = widget ? widget->style() : QApplication::style();
