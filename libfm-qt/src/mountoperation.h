...
class LIBFM_QT_API MountOperation : public QObject {
   ...
   public:
    ...
    FM_QT6_DEPRECATED
    void mount(const Fm::FilePath& path) { mountEnclosingVolume(path); }
    ...
    void mount(GVolume* volume) {
        ...
        g_volume_mount(volume, G_MOUNT_MOUNT_NONE, op, cancellable_, (GAsyncReadyCallback)onMountVolumeFinished,
                       new QPointer<MountOperation>(this));
    }

    void unmount(GMount* mount) {
        prepareUnmount(mount);
        g_mount_unmount_with_operation(mount, G_MOUNT_UNMOUNT_NONE, op, cancellable_,
                                       (GAsyncReadyCallback)onUnmountMountFinished, new QPointer<MountOperation>(this));
    }
    ...
    void unmount(const Fm::FilePath& path) {
        g_file_unmount_mountable_with_operation(path.gfile().get(), G_MOUNT_UNMOUNT_NONE, op, cancellable_,
                                                (GAsyncReadyCallback)onUnmountFileFinished,
                                                new QPointer<MountOperation>(this));
    }

    void eject(GMount* mount) {
        prepareUnmount(mount);
        g_mount_eject_with_operation(mount, G_MOUNT_UNMOUNT_NONE, op, cancellable_,
                                     (GAsyncReadyCallback)onEjectMountFinished, new QPointer<MountOperation>(this));
    }
    ...
    void eject(GVolume* volume) {
        ...
        if (GDrive* drv = g_volume_get_drive(volume)) {
            if (g_drive_can_stop(drv)) {
                g_drive_stop(drv, G_MOUNT_UNMOUNT_NONE, op, cancellable_, (GAsyncReadyCallback)onStopDriveFinished,
                             new QPointer<MountOperation>(this));
                g_object_unref(drv);
                return;
            }
            else if (g_drive_can_eject(drv)) {
                g_drive_eject_with_operation(drv, G_MOUNT_UNMOUNT_NONE, op, cancellable_,
                                             (GAsyncReadyCallback)onEjectDriveFinished,
                                             new QPointer<MountOperation>(this));
                g_object_unref(drv);
                return;
            }
        ...
        g_volume_eject_with_operation(volume, G_MOUNT_UNMOUNT_NONE, op, cancellable_,
                                      (GAsyncReadyCallback)onEjectVolumeFinished, new QPointer<MountOperation>(this));
    }
    ...
    void eject(const Fm::FilePath& path) {
        ...
        g_file_eject_mountable_with_operation(path.gfile().get(), G_MOUNT_UNMOUNT_NONE, op, cancellable_,
                                              (GAsyncReadyCallback)onEjectFileFinished,
                                              new QPointer<MountOperation>(this));
    }
    ...
    GMountOperation* mountOperation() { return tmpOp_ ? tmpOp_ : op; }

    void cancel() { g_cancellable_cancel(cancellable_); }
    ...
    bool autoDestroy() { return autoDestroy_; }

    void setAutoDestroy(bool destroy = true) { autoDestroy_ = destroy; }
   ...
   private:
    ...
    static void onAskPassword(GMountOperation* _op,
                              gchar* message,
                              gchar* default_user,
    ...
    static void onShowProcesses(GMountOperation* _op,
                                gchar* message,
                                GArray* processes,
                                GStrv choices,
                                MountOperation* pThis);
    static void onShowUnmountProgress(GMountOperation* _op,
                                      gchar* message,
                                      gint64 time_left,
                                      gint64 bytes_left,
                                      MountOperation* pThis);

    // it's possible that this object is freed when the callback is called by gio, so guarding with QPointer is needed
    // here.
