...
PlacesModel::PlacesModel(QObject* parent)
    ...
    desktopItem = new PlacesModelItem(
        "user-desktop", tr("Desktop"),
        Fm::FilePath::fromLocalPath(
            QStandardPaths::writableLocation(QStandardPaths::DesktopLocation).toLocal8Bit().constData()));
    ...
    {  // Applications
        const char* application_icon_names[] = {"system-software-install", "applications-accessories",
                                                "application-x-executable"};
        // NOTE: g_themed_icon_new_from_names() accepts char**, but actually const char** is OK.
        Fm::GIconPtr gicon{
            g_themed_icon_new_from_names((char**)application_icon_names, G_N_ELEMENTS(application_icon_names)), false};
        auto fmicon = Fm::IconInfo::fromGIcon(std::move(gicon));
        applicationsItem =
            new PlacesModelItem(fmicon, tr("Applications"), Fm::FilePath::fromUri("menu:///applications/"));
    ...
    {  // Network
        ...
        // NOTE: g_themed_icon_new_from_names() accepts char**, but actually const char** is OK.
        Fm::GIconPtr gicon{g_themed_icon_new_from_names((char**)network_icon_names, G_N_ELEMENTS(network_icon_names)),
                           false};
        auto fmicon = Fm::IconInfo::fromGIcon(std::move(gicon));
...
// static
void PlacesModel::onTrashChanged(GFileMonitor* /*monitor*/,
                                 GFile* /*gf*/,
                                 GFile* /*other*/,
...
void PlacesModel::updateTrash() {
    struct UpdateTrashData {
        ...
        GFile* gf;
        UpdateTrashData(PlacesModel* _model) : model(_model) { gf = g_file_new_for_uri("trash:///"); }
        ~UpdateTrashData() { g_object_unref(gf); }
    };

    if (trashItem_) {
        UpdateTrashData* data = new UpdateTrashData(this);
        g_file_query_info_async(
            data->gf, G_FILE_ATTRIBUTE_TRASH_ITEM_COUNT, G_FILE_QUERY_INFO_NONE, G_PRIORITY_LOW, nullptr,
            [](GObject* /*source_object*/, GAsyncResult* res, gpointer user_data) {
                ...
                if (_this != nullptr) {  // ensure that our model object is not deleted yet
                    ...
                    if (inf) {
                        if (_this->trashItem_ !=
                            nullptr) {  // it's possible that when we finish, the trash item is removed
...
bool PlacesModel::dropMimeData(const QMimeData* data,
    ...
    if (data->hasFormat(QStringLiteral("application/x-bookmark-row"))) {  // the data being dopped is a bookmark row
        ...
        if (item == bookmarksRoot
            ...
            for (auto& path : paths) {
                ...
                if (g_file_query_file_type(path.gfile().get(), G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS, nullptr) ==
...
bool PlacesModel::canDropMimeData(const QMimeData* data,
                                  Qt::DropAction action,
                                  int row,
                                  int column,
