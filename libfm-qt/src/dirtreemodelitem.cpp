...
DirTreeModelItem::DirTreeModelItem(std::shared_ptr<const Fm::FileInfo> info,
                                   DirTreeModel* model,
                                   DirTreeModelItem* parent)
...
void DirTreeModelItem::loadFolder() {
    if (!expanded_) {
        ...
        onFolderFinishLoadingConn_ =
            QObject::connect(folder_.get(), &Fm::Folder::finishLoading, model_, [this]() { onFolderFinishLoading(); });
        onFolderFilesAddedConn_ = QObject::connect(folder_.get(), &Fm::Folder::filesAdded, model_,
                                                   [this](Fm::FileInfoList files) { onFolderFilesAdded(files); });
        onFolderFilesRemovedConn_ = QObject::connect(folder_.get(), &Fm::Folder::filesRemoved, model_,
                                                     [this](Fm::FileInfoList files) { onFolderFilesRemoved(files); });
        onFolderFilesChangedConn_ =
            QObject::connect(folder_.get(), &Fm::Folder::filesChanged, model_,
                             [this](std::vector<Fm::FileInfoPair>& changes) { onFolderFilesChanged(changes); });
...
void DirTreeModelItem::insertFiles(Fm::FileInfoList files) {
    if (children_.size() == 1 && placeHolderChild_) {
        ...
        std::sort(files.begin(), files.end(),
                  [](const std::shared_ptr<const Fm::FileInfo>& a, const std::shared_ptr<const Fm::FileInfo>& b) {
                      return QString::localeAwareCompare(a->displayName(), b->displayName()) < 0;
                  });
        // insert the files into the visible children list at once
        model_->beginInsertRows(index(), 1,
                                files.size() + 1);  // the first item is the placeholder item, so we start from row 1
        for (auto& file : files) {
            if (file->isDir()) {
...
int DirTreeModelItem::insertItem(DirTreeModelItem* newItem) {
    ...
    if (model_->showHidden() || !newItem->fileInfo_ || !newItem->fileInfo_->isHidden()) {
        auto it = std::lower_bound(
            children_.cbegin(), children_.cend(), newItem, [=](const DirTreeModelItem* a, const DirTreeModelItem* b) {
