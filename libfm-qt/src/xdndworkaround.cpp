...
// static
void XdndWorkaround::setWindowProperty(xcb_window_t window,
                                       xcb_atom_t propAtom,
                                       xcb_atom_t typeAtom,
                                       void* data,
                                       int len,
...
bool XdndWorkaround::selectionRequest(xcb_selection_request_event_t* event) {
    ...
    if (type_name == "text/x-moz-url" || type_name.startsWith("text/uri-list")) {
        ...
        if (mime != nullptr && mime->hasUrls()) {
            ...
            xcb_change_property(conn, XCB_PROP_MODE_REPLACE, event->requestor, event->property, atomFormat, 8,
                                data.size(), (const void*)data.constData());
