...
FileDialog::FileDialog(QWidget* parent, FilePath path)
    ...
    // path bar
    connect(ui->location, &PathBar::chdir, [this](const FilePath& path) { setDirectoryPath(path); });

    // side pane
    ui->sidePane->setMode(Fm::SidePane::ModePlaces);
    connect(ui->sidePane, &SidePane::chdirRequested,
            [this](int /*type*/, const FilePath& path) { setDirectoryPath(path); });
    ...
    connect(ui->fileName, &QLineEdit::textEdited, [this](const QString& /*text*/) {
        disconnect(ui->folderView->selectionModel(), &QItemSelectionModel::selectionChanged, this,
                   &FileDialog::onSelectionChanged);
        ui->folderView->selectionModel()->clearSelection();
    ...
    // file type
    connect(ui->fileTypeCombo, &QComboBox::currentTextChanged, [this](const QString& text) { selectNameFilter(text); });
    ...
    connect(reloadAction, &QAction::triggered, [this]() {
        if (folder_ && folder_->isLoaded()) {
            ...
            if (!selFiles.empty() && selFiles.size() <= 50) {  // otherwise senseless and CPU-intensive
                lambdaConnection_ = QObject::connect(folder_.get(), &Fm::Folder::finishLoading, this,
                                                     [this, selFiles]() { selectFilesOnReload(selFiles); });
            }
            folder_->reload();
        }
    });
    ...
    connect(shortcut, &QShortcut::activated, [this]() { ui->folderView->selectionModel()->clearSelection(); });
    ...
    iconViewAction_ = menu->addAction(
        QIcon::fromTheme(u"view-list-icons"_s, style()->standardIcon(QStyle::SP_FileDialogContentsView)),
        tr("Icon View"));
    ...
    thumbnailViewAction_ =
        menu->addAction(QIcon::fromTheme(u"view-preview"_s, style()->standardIcon(QStyle::SP_FileDialogInfoView)),
                        tr("Thumbnail View"));
    ...
    compactViewAction_ =
        menu->addAction(QIcon::fromTheme(u"view-list-text"_s, style()->standardIcon(QStyle::SP_FileDialogListView)),
                        tr("Compact View"));
    ...
    detailedViewAction_ = menu->addAction(
        QIcon::fromTheme(u"view-list-details"_s, style()->standardIcon(QStyle::SP_FileDialogDetailedView)),
        tr("Detailed List View"));
    ...
    connect(bigSizeAction, &FileDialogIconSizeAction::editingFinished,
            [this, bigSizeAction]() { setBigIconSize(bigSizeAction->value()); });
    ...
    connect(smallSizeAction, &FileDialogIconSizeAction::editingFinished,
            [this, smallSizeAction]() { setSmallIconSize(smallSizeAction->value()); });
    ...
    connect(thumbnailSizeAction, &FileDialogIconSizeAction::editingFinished,
            [this, thumbnailSizeAction]() { setThumbnailIconSize(thumbnailSizeAction->value()); });
    ...
    connect(showHiddenAction, &QAction::triggered, [this](bool checked) { proxyModel_->setShowHidden(checked); });
    // also add a separate shortcut for it
    shortcut = new QShortcut(QKeySequence(Qt::CTRL | Qt::Key_H), this);
    connect(shortcut, &QShortcut::activated, [this]() { proxyModel_->setShowHidden(!proxyModel_->showHidden()); });

    // thumbnail and tooltip actions
    auto thumbnailsAction = menu->addAction(tr("Show Thumbnails"));
    thumbnailsAction->setCheckable(true);
    connect(thumbnailsAction, &QAction::triggered, [this](bool checked) { setShowThumbnails(checked); });
    auto tooltipsAction = menu->addAction(tr("Show File Tooltips"));
    tooltipsAction->setCheckable(true);
    connect(tooltipsAction, &QAction::triggered, [this](bool checked) { setNoItemTooltip(!checked); });

    // per-pixel action
    auto perPixelAction = menu->addAction(tr("Smooth Scrolling"));
    perPixelAction->setCheckable(true);
    connect(perPixelAction, &QAction::triggered, [this](bool checked) { setScrollPerPixel(checked); });
    ...
    auto optionsAction = toolbar->addAction(
        QIcon::fromTheme(u"preferences-system"_s, QIcon::fromTheme(u"document-properties"_s)), tr("Options"));
...
bool FileDialog::eventFilter(QObject* watched, QEvent* event) {
    ...
    if (event->type() == QEvent::KeyPress) {
        if (QKeyEvent* ke = static_cast<QKeyEvent*>(event)) {
            ...
            else if (watched == ui->fileName && ui->fileName->hasFocus() && ke->key() == Qt::Key_Backtab) {
    ...
    if (parsedNames.isEmpty()) {
        ...
        if (fileMode_ != QFileDialog::Directory) {
            ...
            if (exists) {
                ...
                if (fileMode_ == QFileDialog::AnyFile && acceptMode_ != QFileDialog::AcceptOpen && confirmOverwrite_) {
                    if (QMessageBox::warning(this, windowTitle(),
                                             tr("%1 already exists.\nDo you want to replace it?").arg(firstName),
...
void FileDialog::setDirectoryPath(FilePath directory, FilePath selectedPath, bool addHistory) {
    ...
    if (selectedPath.isValid() && folder_) {
        if (folder_->isLoaded()) {
            ...
            lambdaConnection_ = QObject::connect(folder_.get(), &Fm::Folder::finishLoading, this,
                                                 [this, selectedPath]() { selectFilePathWithDelay(selectedPath); });
        }
    }
    else {
        if (folder_ == nullptr || folder_->isLoaded()) {
            ...
            lambdaConnection_ =
                QObject::connect(folder_.get(), &Fm::Folder::finishLoading, this, [this, selectedPath]() {
...
void FileDialog::updateSelectionMode() {
    ...
    ui->folderView->childView()->setSelectionMode(fileMode_ == QFileDialog::ExistingFiles
    ...
    if (job->isCancelled()) {
        ...
        for (size_t i = 0; i < paths.size(); ++i) {
            ...
            if (fileMode_ == QFileDialog::Directory) {
                ...
                if (error.isEmpty()) {
                    // we're selecting files, but the selected file path refers to a dir or shortcut (such as
                    // computer:///)
...
void FileDialog::setViewMode(FolderView::ViewMode mode) {
    ...
    disconnect(ui->folderView->selectionModel(), &QItemSelectionModel::currentRowChanged, this,
               &FileDialog::onCurrentRowChanged);
    disconnect(ui->folderView->selectionModel(), &QItemSelectionModel::selectionChanged, this,
               &FileDialog::onSelectionChanged);
    ...
    // selection changes
    connect(ui->folderView->selectionModel(), &QItemSelectionModel::currentRowChanged, this,
            &FileDialog::onCurrentRowChanged);
    connect(ui->folderView->selectionModel(), &QItemSelectionModel::selectionChanged, this,
            &FileDialog::onSelectionChanged);
    // update selection mode for the view
...
void FileDialog::updateSaveButtonText(bool saveOnFolder) {
    if (fileMode_ != QFileDialog::Directory && acceptMode_ == QFileDialog::AcceptSave) {
        ...
        if (!saveOnFolder) {
            ...
            if (!parsedNames.isEmpty()) {
                if (parsedNames.at(0).contains(QLatin1Char('/'))) {
                    ...
                    auto info = proxyModel_->fileInfoFromPath(
                        directoryPath_.child(parsedNames.at(0).toLocal8Bit().constData()));
                    if (info && info->isDir()) {
                        saveOnFolder = true;
                    }
                }
...
void FileDialog::updateAcceptButtonState() {
    ...
    if (fileMode_ != QFileDialog::Directory) {
        if (acceptMode_ == QFileDialog::AcceptOpen) {
            if (firstSelectedDir() || (fileMode_ == QFileDialog::AnyFile && !ui->fileName->text().isEmpty())) {
                ...
                for (auto& name : parsedNames) {
                    if (name.contains(QLatin1Char('/'))) {
                        ...
                        if (proxyModel_->indexFromPath(directoryPath_.child(name.toLocal8Bit().constData()))
                                .isValid()) {
                            enable = true;
                            break;
                        }
    ...
    else if (fileMode_ == QFileDialog::Directory && acceptMode_ != QFileDialog::AcceptSave) {
        QStringList parsedNames = parseNames();
        if (parsedNames.isEmpty()) {
...
bool FileDialog::FileDialogFilter::filterAcceptsRow(const ProxyFolderModel* /*model*/,
