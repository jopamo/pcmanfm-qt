...
bool changeFileName(const Fm::FilePath& filePath, const QString& newName, QWidget* parent, bool showMessage) {
    ...
    Fm::GErrorPtr err;
    GFilePtr gfile{g_file_set_display_name(filePath.gfile().get(), newName.toLocal8Bit().constData(),
...
void setDefaultAppForType(const Fm::GAppInfoPtr app, std::shared_ptr<const Fm::MimeType> mimeType) {
    ...
    auto mimeappsListPath =
        CStrPtr(g_build_filename(configDir.toUtf8().constData(), mimeappsList.constData(), nullptr));
...
void createFileOrFolder(CreateFileType type, FilePath parentDir, const TemplateItem* templ, QWidget* parent) {
    ...
    QString prompt;
    QString dialogTitle = type == CreateNewFolder ? QObject::tr("Create Folder") : QObject::tr("Create File");
