...
FmXmlFileTag fm_xml_file_set_handler(FmXmlFile* file,
    ...
    for (i = 1; i < file->n_tags; i++)
        if (strcmp(file->tags[i].name, tag) == 0) {
            g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_UNKNOWN_ELEMENT, _("Duplicate handler for tag <%s>"),
                        tag);
            return i;
        }
            ...
            if (*from == '#') {
                ...
                *to = '\'';
                ...
                if (*from == ';')
                    ...
                    if (end)
                        g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_PARSE, _("Entity name '%-.*s' is not known"),
                                    (int)(end - from), from);
                    else
                        g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_PARSE,
...
static inline void _update_file_ptr_part(FmXmlFile* file, const char* start, const char* end) {
    while (start < end) {
        if (*start == '\n') {
            file->line++;
            file->pos = 0;
 ...
 * Since: 1.2.0
 */
gboolean fm_xml_file_parse_data(FmXmlFile* file, const char* text, gsize size, GError** error, gpointer user_data) {
    gsize ptr, len;
...
_restart:
    ...
    if (file->data && file->data->len && file->data->str[0] == '<') {
        ...
        if (file->data->len >= 11 /* <![CDATA[]] */ && strncmp(file->data->str, "<![CDATA[", 9) == 0) {
            ...
            if (file->current_item == NULL) /* CDATA at top level! */
                g_warning("FmXmlFile: line %u: junk CDATA in XML file ignored", file->line);
            else {
                item = fm_xml_file_item_new(FM_XML_FILE_TEXT);
                item->text = item->comment = g_strndup(&file->data->str[9], file->data->len - 11);
        ...
        /* check for comment */
        if (file->data->len >= 7 /* <!-- -- */ && strncmp(file->data->str, "<!--", 4) == 0) {
            end = file->data->str + file->data->len;
            ...
            if (_is_space(end[-3]))
                file->comment_pre = g_strndup(&file->data->str[5], file->data->len - 8);
            else /* FIXME: check: XML spec says it should be not '-' */
                file->comment_pre = g_strndup(&file->data->str[5], file->data->len - 7);
            _update_file_ptr(file, 1);
            g_string_truncate(file->data, 0);
            goto _restart;
        }
        /* check for DTD - it may be only at top level */
        if (file->current_item == NULL && file->data->len >= 10 && strncmp(file->data->str, "<!DOCTYPE", 9) == 0 &&
            _is_space(file->data->str[9])) {
            /* FIXME: can DTD contain any tags? count '<' and '>' pairs */
            if (file->tags[0].name) /* duplicate DTD! */
                g_warning("FmXmlFile: line %u: duplicate DTD, ignored", file->line);
            else
                file->tags[0].name = g_strndup(&file->data->str[10], file->data->len - 10);
            _update_file_ptr(file, 1);
            g_string_truncate(file->data, 0);
            goto _restart;
        }
        /* support directives such as <?xml ..... ?> */
        if (file->data->len >= 4 /* <?x? */ && file->data->str[1] == '?' &&
        ...
        *dst = '\0'; /* terminate the tag */
        if (closing) {
            ...
            {
                g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_PARSE,
                            _("Element '%s' was closed but no element was opened"), tag);
                return FALSE;
            }
            else {
                char* tagname;
            ...
            _close_the_tag:
                ...
                g_string_truncate(file->data, 0);
                if (item->tag != FM_XML_FILE_TAG_NOT_HANDLED) {
                    if (!file->tags[item->tag].handler(item, item->children, item->attribute_names,
                                                       item->attribute_values,
                                                       item->attribute_names ? g_strv_length(item->attribute_names) : 0,
                                                       file->line, file->pos, error, user_data))
                        return FALSE;
                }
                file->pos++; /* '>' */
                goto _restart;
        ...
        else /* opening tag */
        {
            ...
            while (dst < end) {
                ...
                if (value + 3 <= end && *value == '=') /* minimum is ="" */
                {
                    ...
                    if (quote != '\'' && quote != '"') {
                        g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_PARSE,
                                    _("Invalid char '%c' at start of attribute value"), quote);
                        goto _attr_error;
                    }
                    file->pos++; /* quote char */
                    ...
                    if (!unescape_gstring_inplace(buff, &file->line, &file->pos, TRUE, error)) {
    ...
    /* otherwise we stopped at some data somewhere */
        ...
        {
            g_warning("FmXmlFile: line %u: junk data in XML file ignored", file->line);
            _update_file_ptr(file, 0);
        }
        else if (unescape_gstring_inplace(file->data, &file->line, &file->pos, FALSE, error)) {
            item = fm_xml_file_item_new(FM_XML_FILE_TEXT);
...
GList* fm_xml_file_finish_parse(FmXmlFile* file, GError** error) {
    ...
    if (file->current_item) {
        if (file->current_item->tag == FM_XML_FILE_TEXT && file->current_item->parent == NULL)
            g_warning("FmXmlFile: junk at end of XML");
        else {
            g_set_error_literal(error, G_MARKUP_ERROR, G_MARKUP_ERROR_PARSE, _("Document ended unexpectedly"));
 ...
 * Since: 1.2.0
 */
void fm_xml_file_item_append_text(FmXmlFileItem* item, const char* text, gssize text_size, gboolean cdata) {
...
gboolean fm_xml_file_item_append_child(FmXmlFileItem* item, FmXmlFileItem* child) {
    ...
    {
        /* g_debug("moving item %p(%d) from parser %p into %p as child of %p", child, (int)child->tag, child->file,
         * item->file, item); */
 ...
 * Since: 1.2.0
 */
gboolean fm_xml_file_item_set_attribute(FmXmlFileItem* item, const char* name, const char* value) {
    int i, n_attr;
...
static gboolean _parser_item_to_gstring(FmXmlFile* file,
                                        GString* string,
                                        FmXmlFileItem* item,
                                        GString* prefix,
                                        gboolean* has_nl,
    ...
    switch (item->tag) {
        ...
        _do_tag:
            ...
            if (item->comment != NULL)
                g_string_append_printf(string, "%s<!-- %s -->", prefix->str, item->comment);
            else if (item->attribute_names == NULL && item->children == NULL && file->tags[item->tag].in_line) {
    ...
    *has_nl = FALSE; /* to collect data from nested elements */
    ...
    if (l != NULL) /* failed */
        return FALSE;
    /* close the tag */
    g_string_append_printf(string, "%s</%s>", (*has_nl) ? prefix->str : "", tag_name);
