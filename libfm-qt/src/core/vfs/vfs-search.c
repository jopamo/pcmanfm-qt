...
#define FM_SEARCH_VFILE(o) (G_TYPE_CHECK_INSTANCE_CAST((o), FM_TYPE_SEARCH_VFILE, FmSearchVFile))
...
static inline FmSearchIntIter* _search_iter_new(FmSearchIntIter* parent,
    ...
    enu = g_file_enumerate_children(folder_path, attributes, flags, cancellable, error);
    if (enu == NULL)
        return NULL;
    ...
    while (!g_cancellable_set_error_if_cancelled(cancellable, error)) {
        ...
        if (iter == NULL) /* ended with folder */
        {
            if (enu->target_folders == NULL)
                break;
            iter = _search_iter_new(NULL, enu->attributes, enu->flags, enu->target_folders->data, cancellable, error);
            if (iter == NULL)
                break;
            /* data is moved into iter now so free link itself */
            enu->target_folders = g_slist_delete_link(enu->target_folders, enu->target_folders);
        ...
        if (file_info && g_file_info_get_name(file_info)) {
            ...
            if (fm_search_job_match_file(enu, file_info, iter->folder_path, cancellable, &err)) {
...
static void parse_search_uri(FmVfsSearchEnumerator* priv, const char* uri_str) {
    ...
    if (g_ascii_strncasecmp(uri_str, scheme, sizeof(scheme) - 1) == 0) {
        ...
        while (p) {
            ...
            priv->target_folders = g_slist_prepend(priv->target_folders, g_file_new_for_commandline_arg(path));
            g_free(path);
        ...
        if (params) {
                ...
                if (strcmp(name, "show_hidden") == 0)
                    ...
                    if (priv->mime_types) {
                        ...
                        if (!g_strstr_len(priv->attributes, -1, G_FILE_ATTRIBUTE_STANDARD_CONTENT_TYPE)) {
                            gchar* attributes =
                                g_strconcat(priv->attributes, ",", G_FILE_ATTRIBUTE_STANDARD_CONTENT_TYPE, NULL);
                            g_free(priv->attributes);
            ...
            if (name_regex) {
                ...
                priv->name_regex_utf8 =
                    g_regex_new(name_regex, priv->name_case_insensitive ? G_REGEX_CASELESS : 0, 0, NULL);
                g_free(name_regex);
            }
            ...
            if (content_regex) {
                ...
                priv->content_regex_utf8 =
                    g_regex_new(content_regex, priv->content_case_insensitive ? G_REGEX_CASELESS : 0, 0, NULL);
                g_free(content_regex);
            }
...
static void fm_search_job_match_folder(FmVfsSearchEnumerator* priv,
    ...
    iter = _search_iter_new(priv->iter, priv->attributes, priv->flags, folder_path, cancellable, error);
    if (iter == NULL) /* error */
        return;
...
static gboolean fm_search_job_match_content(FmVfsSearchEnumerator* priv,
    ...
    if (priv->content_pattern || priv->content_regex) {
        ...
        if (g_file_info_get_file_type(info) == G_FILE_TYPE_REGULAR &&
            g_file_info_get_attribute_uint64(info, G_FILE_ATTRIBUTE_STANDARD_SIZE) > 0) {
            ...
            if (file) {
                ...
                if (stream) {
                    if (priv->content_pattern && !priv->content_case_insensitive) {
                         ...
                         * exact match. */
                        ret = fm_search_job_match_content_exact(priv, info, G_INPUT_STREAM(stream), cancellable, error);
                    }
                    else {
                        /* grep-like regexp search and case insensitive search
                         * are line-based. */
                        ret = fm_search_job_match_content_line_based(priv, info, G_INPUT_STREAM(stream), cancellable,
                                                                     error);
                    }
...
G_DEFINE_TYPE_WITH_CODE(FmSearchVFile,
                        fm_vfs_search_file,
                        G_TYPE_OBJECT,
...
static GFile* _fm_vfs_search_get_child_for_display_name(GFile* file, const char* display_name, GError** error) {
...
static GMount* _fm_vfs_search_find_enclosing_mount(GFile* file, GCancellable* cancellable, GError** error) {
    ERROR_UNSUPPORTED(error);
    return NULL;
}
...
static GFileInputStream* _fm_vfs_search_read_fn(GFile* file, GCancellable* cancellable, GError** error) {
    ERROR_UNSUPPORTED(error);
    return NULL;
}
...
static gboolean _fm_vfs_search_delete_file(GFile* file, GCancellable* cancellable, GError** error) {
    ERROR_UNSUPPORTED(error);
    return FALSE;
}

static gboolean _fm_vfs_search_trash(GFile* file, GCancellable* cancellable, GError** error) {
    ERROR_UNSUPPORTED(error);
    return FALSE;
}

static gboolean _fm_vfs_search_make_directory(GFile* file, GCancellable* cancellable, GError** error) {
    ERROR_UNSUPPORTED(error);
    return FALSE;
}
...
static GFileIOStream* _fm_vfs_search_open_readwrite(GFile* file, GCancellable* cancellable, GError** error) {
    ERROR_UNSUPPORTED(error);
    return NULL;
}
