...
BasicFileLauncher::BasicFileLauncher() : quickExec_{false} {}
        ...
        */
        if (fileInfo->isMountable()) {
            if (fileInfo->target().empty()) {
                ...
                GErrorPtr err{G_IO_ERROR, G_IO_ERROR_NOT_MOUNTED, QObject::tr("The path is not mounted.")};
...
bool BasicFileLauncher::launchPaths(FilePathList paths, GAppLaunchContext* ctx) {
    ...
    QObject::connect(
        job, &FileInfoJob::error, &eventLoop,
        [this, job, ctx](const GErrorPtr& err, Job::ErrorSeverity /* severity */, Job::ErrorAction& act) {
        ...
        Qt::BlockingQueuedConnection);  // BlockingQueuedConnection is required here to pause the job and wait for user
                                        // response

    QObject::connect(job, &FileInfoJob::finished, [&eventLoop]() {
...
GAppInfoPtr BasicFileLauncher::chooseApp(const FileInfoList& /* fileInfos */,
                                         const char* /*mimeType*/,
                                         GErrorPtr& /* err */) {
    return GAppInfoPtr{};
}
...
bool BasicFileLauncher::showError(GAppLaunchContext* /* ctx */,
                                  const GErrorPtr& /* err */,
                                  const FilePath& /* path */,
                                  const FileInfoPtr& /* info */) {
    return false;
}
...
bool BasicFileLauncher::launchDesktopEntry(const FileInfoPtr& fileInfo,
    ...
    else if (fileInfo->isNative() /* an exception */ || fileInfo->path().hasUriScheme("menu")) {
        if (target.empty()) {
            filename = fileInfo->path().localPath();
            desktopEntryName = filename.get();
        }
...
bool BasicFileLauncher::launchDesktopEntry(const char* desktopEntryName,
                                           const FilePathList& paths,
                                           GAppLaunchContext* ctx) {
    bool ret = false;
    GAppInfo* app;
...
FilePath BasicFileLauncher::handleShortcut(const FileInfoPtr& fileInfo, GAppLaunchContext* ctx) {
    ...
    if (scheme) {
        ...
        if (strcmp(scheme.get(), "file") == 0 || strcmp(scheme.get(), "trash") == 0 ||
            strcmp(scheme.get(), "network") == 0 || strcmp(scheme.get(), "computer") == 0 ||
            ...
            if (GAppInfoPtr app{g_app_info_get_default_for_uri_scheme(scheme.get()), false}) {
                ...
                GErrorPtr err{
                    ...
                    QObject::tr("No default application is set to launch '%1'").arg(QString::fromUtf8(target.c_str()))};
    ...
    if (g_file_test(filename.get(), G_FILE_TEST_IS_EXECUTABLE)) {
        ...
        switch (act) {
            ...
            case ExecAction::DIRECT_EXEC: {
                ...
                GAppInfoPtr app{
                    fm_app_info_create_from_commandline(quoted.get(), nullptr, GAppInfoCreateFlags(flags), nullptr)};
                if (app) {
                    ...
                    if (run_path && strcmp(run_path.get(), ".")) {
                        ...
                        if (chdir(run_path.get()) != 0) {
                            ...
                            // show errors
                            QString msg =
                                QObject::tr("Cannot set working directory to '%1': %2")
                                    .arg(QString::fromUtf8(run_path.get()), QString::fromUtf8(g_strerror(errno)));
