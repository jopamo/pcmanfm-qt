...
BookmarkItem::BookmarkItem(const FilePath& path, const QString name) : path_{path}, name_{name} {
...
Bookmarks::Bookmarks(QObject* parent) : QObject(parent), idle_handler{false} {
    ...
    mon = GObjectPtr<GFileMonitor>{g_file_monitor_file(file_.gfile().get(), G_FILE_MONITOR_NONE, nullptr, nullptr),
                                   false};
    if (mon) {
...
const std::shared_ptr<const BookmarkItem>& Bookmarks::insert(const FilePath& path, const QString& name, int pos) {
    const auto insert_pos =
        (pos < 0 || static_cast<size_t>(pos) > items_.size()) ? items_.cend() : items_.cbegin() + pos;
...
void Bookmarks::save() {
    ...
    // G_UNLOCK(bookmarks);
    GError* err = nullptr;
    if (!g_file_replace_contents(file_.gfile().get(), buf.c_str(), buf.length(), nullptr, FALSE, G_FILE_CREATE_NONE,
                                 nullptr, nullptr, &err)) {
