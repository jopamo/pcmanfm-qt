...
bool launchTerminal(const char* programName, const FilePath& workingDir, Fm::GErrorPtr& error) {
    ...
    bool ret = g_spawn_async(dir.get(), argv, envp, G_SPAWN_SEARCH_PATH, child_setup,
                             (gpointer)(gsize)getpgid(getppid()), nullptr, &error);
    g_strfreev(argv);
    g_strfreev(envp);
    return ret;
}
...
extern "C" char* expand_terminal(char* cmd, gboolean keep_open, GError** error) {
    ...
    char* ret = nullptr;
    if (custom_args && g_strcmp0(custom_args.get(), "") != 0)
        ret = g_strdup_printf("%s %s %s %s", program, custom_args.get(), opts, cmd);
    else
