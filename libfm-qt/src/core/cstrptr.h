...
struct CStrDeleter {
    void operator()(char* ptr) const { g_free(ptr); }
};
...
struct CStrHash {
    std::size_t operator()(const char* str) const { return g_str_hash(str); }
};

struct CStrEqual {
    bool operator()(const char* str1, const char* str2) const { return g_str_equal(str1, str2); }
};

struct CStrVDeleter {
    void operator()(char** ptr) const { g_strfreev(ptr); }
};
