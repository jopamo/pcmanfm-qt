...
class LIBFM_QT_API TemplateItem {
   public:
    ...
    QString displayName() const { return fileInfo_->displayName(); }

    const std::string& name() const { return fileInfo_->name(); }

    std::shared_ptr<const IconInfo> icon() const { return fileInfo_->icon(); }

    std::shared_ptr<const FileInfo> fileInfo() const { return fileInfo_; }

    std::shared_ptr<const MimeType> mimeType() const { return fileInfo_->mimeType(); }
...
class LIBFM_QT_API Templates : public QObject {
   ...
   public:
    ...
    bool hasTemplates() const { return !items_.empty(); }

   Q_SIGNALS:
    void itemAdded(const std::shared_ptr<const TemplateItem>& item);

    void itemChanged(const std::shared_ptr<const TemplateItem>& oldItem,
