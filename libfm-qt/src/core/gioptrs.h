...
class GErrorPtr {
   public:
    ...
    GErrorPtr(GError*&& err) noexcept : err_{err} { err = nullptr; }

    GErrorPtr(const GErrorPtr& other) = delete;

    GErrorPtr(GErrorPtr&& other) noexcept : err_{other.err_} { other.err_ = nullptr; }
    ...
    GError** operator&() { return &err_; }

    GError* operator->() { return err_; }

    const GError* operator->() const { return err_; }

    bool operator==(const GErrorPtr& other) const { return err_ == other.err_; }

    bool operator==(GError* err) const { return err_ == err; }

    bool operator!=(std::nullptr_t) const { return err_ != nullptr; }

    operator bool() const { return err_ != nullptr; }
