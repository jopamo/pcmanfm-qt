...
class LIBFM_QT_API ThumbnailJob : public Job {
   ...
   public:
    ...
    static bool localFilesOnly() { return localFilesOnly_; }

    static int maxThumbnailFileSize() { return maxThumbnailFileSize_; }

    static void setMaxThumbnailFileSize(int size);

    static int maxExternalThumbnailFileSize() { return maxExternalThumbnailFileSize_; }

    static void setMaxExternalThumbnailFileSize(int size);

    const std::vector<QImage>& results() const { return results_; }
   ...
   private:
    ...
    QImage generateThumbnail(const std::shared_ptr<const FileInfo>& file,
