...
static const char* fallbackIconNames[] = {"unknown", "application-octet-stream", "application-x-generic",
                                          "text-x-generic", nullptr};
...
IconInfo::IconInfo(const char* name) : gicon_{g_themed_icon_new(name), false} {}

IconInfo::IconInfo(const GIconPtr gicon) : gicon_{std::move(gicon)} {}
    ...
    if (G_LIKELY(name)) {
        ...
        if (g_path_is_absolute(name)) {
            ...
            g_object_unref(gicon_file);
                            ...
                            (g_ascii_strcasecmp(&dot[1], "png") == 0 || g_ascii_strcasecmp(&dot[1], "svg") == 0 ||
