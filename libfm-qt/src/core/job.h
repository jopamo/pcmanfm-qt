...
class LIBFM_QT_API Job : public QObject, public QRunnable {
    Q_OBJECT
   public:
    enum class ErrorAction { CONTINUE, RETRY, ABORT };

    enum class ErrorSeverity { UNKNOWN, WARNING, MILD, MODERATE, SEVERE, CRITICAL };

    explicit Job();

    ~Job() override;

    bool isCancelled() const { return g_cancellable_is_cancelled(cancellable_.get()); }
    ...
    const GCancellablePtr& cancellable() const { return cancellable_; }
   ...
   private:
    ...
    void onCancellableCancelled(GCancellable* /*cancellable*/) { Q_EMIT cancelled(); }
