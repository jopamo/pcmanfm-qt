...
PathBar::PathBar(QWidget* parent) : QWidget(parent), tempPathEdit_(nullptr), toggledBtn_(nullptr) {
    ...
    buttonsLayout_->setSizeConstraint(
        QLayout::SetFixedSize);              // required when added to scroll area according to QScrollArea doc.
...
void PathBar::setScrollButtonVisibility() {
    ...
    if (showScrollers) {
        ...
        scrollToStart_->setMaximumHeight(
            std::max(buttonsWidget_->height(), scrollToStart_->minimumSizeHint().height()));
