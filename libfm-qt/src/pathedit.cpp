...
void PathEditJob::runJob() {
    GError* err = nullptr;
    GFileEnumerator* enu =
        g_file_enumerate_children(dirName,
...
PathEdit::PathEdit(QWidget* parent)
    : QLineEdit(parent), completer_(new QCompleter()), model_(new QStringListModel()), cancellable_(nullptr) {
...
bool PathEdit::event(QEvent* e) {
    if (e->type() == QEvent::KeyPress) {
        ...
        if ((key == Qt::Key_Tab && keyEvent->modifiers() == Qt::NoModifier) || key == Qt::Key_Backtab) {
            e->accept();
