...
enum { ACCESS_NO_CHANGE = 0, ACCESS_READ_ONLY, ACCESS_READ_WRITE, ACCESS_FORBID };
...
void FilePropsDialog::initGeneralPage() {
    ...
    connect(totalSizeJob, &Fm::TotalSizeJob::finished, this, &FilePropsDialog::onDeepCountJobFinished,
        ...
        if (folder != nullptr) {
            ...
            if (folder->getFilesystemInfo(&total, &free)) {
                ...
                ui->progressBar->setValue(
                    std::round(static_cast<qreal>((total - free) * 100) / static_cast<qreal>(total)));
                ui->progressBar->setFormat(tr("%p% used"));
                ui->spaceLabel->setText(
                    tr("%1 Free of %2").arg(formatFileSize(free, false), formatFileSize(total, false)));
            }
        }
    }
...
void FilePropsDialog::onIconButtonclicked() {
    ...
    QStringList icons =
        QStandardPaths::locateAll(QStandardPaths::GenericDataLocation, u"icons"_s, QStandardPaths::LocateDirectory);
    ...
    if (iconDir.isEmpty()) {
        iconDir =
            QStandardPaths::locate(QStandardPaths::GenericDataLocation, u"icons"_s, QStandardPaths::LocateDirectory);
        if (iconDir.isEmpty()) {
            return;
        }
    }
    const QString iconPath =
        QFileDialog::getOpenFileName(this, tr("Select an icon"), iconDir, tr("Images (*.png *.xpm *.svg *.svgz )"));
    if (!iconPath.isEmpty()) {
...
void FilePropsDialog::accept() {
    ...
    bool needChmod = ((newOwnerPermSel != ownerPermSel) || (newGroupPermSel != groupPermSel) ||
                      (newOtherPermSel != otherPermSel) || (newExecCheckState != execCheckState));
        ...
        if (QString::fromStdString(fileInfo->name()) != new_name) {
            ...
            if (parent_path) {
                ...
                if (!g_file_move(path.gfile().get(), dest.gfile().get(),
                                 GFileCopyFlags(G_FILE_COPY_ALL_METADATA | G_FILE_COPY_NO_FALLBACK_FOR_MOVE |
