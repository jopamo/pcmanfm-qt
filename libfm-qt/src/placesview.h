...
class LIBFM_QT_API PlacesProxyModel : public QSortFilterProxyModel {
   ...
   public:
    ...
    bool isShowingAll() const { return showAll_; }

    bool isHidden(const QString& str) const { return hidden_.contains(str); }

    bool hasHidden() const { return !hidden_.isEmpty(); }
...
class LIBFM_QT_API PlacesView : public QTreeView {
   ...
   public:
    ...
    const Fm::FilePath& currentPath() const { return currentPath_; }

    void chdir(Fm::FilePath path) { setCurrentPath(std::move(path)); }
