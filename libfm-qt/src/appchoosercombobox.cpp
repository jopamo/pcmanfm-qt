...
AppChooserComboBox::AppChooserComboBox(QWidget* parent)
    : QComboBox(parent), defaultAppIndex_(-1), prevIndex_(0), blockOnCurrentIndexChanged_(false) {
    ...
    connect((QComboBox*)this, static_cast<void (QComboBox::*)(int)>(&QComboBox::currentIndexChanged), this,
            &AppChooserComboBox::onCurrentIndexChanged);
}
...
void AppChooserComboBox::setMimeType(std::shared_ptr<const Fm::MimeType> mimeType) {
    ...
    if (mimeType_) {
        ...
        for (GList* l = appInfos_glist; l; l = l->next, ++i) {
            ...
            addItem(gicon ? Fm::IconInfo::fromGIcon(gicon)->qicon() : QIcon(),
                    QString::fromUtf8(g_app_info_get_name(app.get())));
        ...
        if (dlg.exec() == QDialog::Accepted) {
            ...
            if (app) {
                ...
                if (found != appInfos_.cend()) {
                    ...
                    insertItem(0, Fm::IconInfo::fromGIcon(gicon)->qicon(),
                               QString::fromUtf8(g_app_info_get_name(it->get())));
                    setCurrentIndex(0);
                }
