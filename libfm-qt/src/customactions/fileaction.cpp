...
static unordered_map<const char*, shared_ptr<FileActionObject>, CStrHash, CStrEqual>
    all_actions;  // cache all loaded actions
...
std::string FileActionObject::expand_str(const char* templ,
                                         const FileInfoList& files,
                                         bool for_display,
                                         std::shared_ptr<const FileInfo> first_file) {
    if (!templ) {
        return string{};
    }
    string result;
        ...
        if (ch == '%') {
            ...
            switch (ch) {
                ...
                case 'n':  // username of the (first) URI
                    ...
                    break;
                case 'o':  // no-op operator which forces a singular form of execution when specified as first
                           // parameter,
...
void FileActionMenu::cache_children(const FileInfoList& files, const char** items_list) {
    for (; *items_list; ++items_list) {
        ...
        if (item_id_prefix[0] == '[' && item_id_prefix[len - 1] == ']') {
            ...
            if (g_spawn_command_line_sync(command.c_str(), &output, nullptr, &exit_status, nullptr) &&
                exit_status == 0) {
...
std::shared_ptr<FileActionItem> FileActionItem::fromActionObject(std::shared_ptr<FileActionObject> action_obj,
                                                                 const FileInfoList& files) {
...
FileActionItem::FileActionItem(std::shared_ptr<FileAction> _action,
                               std::shared_ptr<FileActionProfile> _profile,
                               const FileInfoList& files)
            ...
            if (g_file_test(full_path.get(), G_FILE_TEST_IS_DIR)) {
                ...
                if (all_actions.find(id) == all_actions.cend()) {
                    ...
                    if (g_key_file_load_from_file(kf, full_path.get(), G_KEY_FILE_NONE, nullptr)) {
                        ...
                        action->setId(id);
                        all_actions.insert(
                            make_pair(action->id.get(), action));  // add the id/action pair to hash table
...
bool FileActionItem::compare_items(std::shared_ptr<const FileActionItem> a, std::shared_ptr<const FileActionItem> b) {
    ...
    if (!level_zero_checked) {
        ...
        auto level_zero =
            CStrPtr{g_build_filename(g_get_user_data_dir(), "file-manager/actions/level-zero.directory", nullptr)};
        if (g_file_test(level_zero.get(), G_FILE_TEST_IS_REGULAR)) {
            ...
            if (g_key_file_load_from_file(kf, level_zero.get(), G_KEY_FILE_NONE, nullptr)) {
                auto itemsList =
                    CStrArrayPtr{g_key_file_get_string_list(kf, "Desktop Entry", "ItemsList", nullptr, nullptr)};
                if (itemsList) {
                    for (uint i = 0; i < g_strv_length(itemsList.get()); ++i) {
                        CStrPtr desktop_file_name{g_strconcat(itemsList.get()[i], ".desktop", nullptr)};
                        auto desktop_file = CStrPtr{g_build_filename(g_get_user_data_dir(), "file-manager/actions",
                        ...
                        if (g_key_file_load_from_file(desktop_file_key, desktop_file.get(), G_KEY_FILE_NONE, nullptr)) {
                            auto actionName = CStrPtr{g_key_file_get_locale_string(desktop_file_key, "Desktop Entry",
