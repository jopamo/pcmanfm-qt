...
DirTreeView::DirTreeView(QWidget* parent) : QTreeView(parent), currentExpandingItem_(nullptr) {
    ...
    connect(this, &DirTreeView::customContextMenuRequested, this, &DirTreeView::onCustomContextMenuRequested);
...
void DirTreeView::mousePressEvent(QMouseEvent* event) {
    if (event && event->button() == Qt::RightButton && event->type() == QEvent::MouseButtonPress) {
            ...
            if (event->mimeData()->hasUrls()) {  // files uris are dropped
                ...
                if (!srcPaths.empty()) {
                    ...
                    QTimer::singleShot(0, this, [this, curPos, srcPaths, destPath] {
                        Qt::DropAction action = DndActionMenu::askUser(Qt::CopyAction | Qt::MoveAction | Qt::LinkAction,
                                                                       curPos, viewport());
                        switch (action) {
                            case Qt::CopyAction:
