...
SidePane::SidePane(QWidget* parent)
    : QWidget(parent), view_(nullptr), combo_(nullptr), iconSize_(24, 24), mode_(ModeNone), showHidden_(false) {
    ...
    connect(combo_, static_cast<void (QComboBox::*)(int)>(&QComboBox::currentIndexChanged), this,
            &SidePane::onComboCurrentIndexChanged);
...
bool SidePane::eventFilter(QObject* watched, QEvent* event) {
    ...
    if (view_ != nullptr && watched == view_ &&
        (event->type() == QEvent::PaletteChange || event->type() == QEvent::StyleChange) && mode_ == ModePlaces) {
...
void SidePane::setMode(Mode mode) {
    ...
    switch (mode) {
        ...
        case ModeDirTree: {
            ...
            connect(dirTreeView, &DirTreeView::openFolderInNewWindowRequested, this,
                    &SidePane::openFolderInNewWindowRequested);
            connect(dirTreeView, &DirTreeView::openFolderInNewTabRequested, this,
                    &SidePane::openFolderInNewTabRequested);
            connect(dirTreeView, &DirTreeView::openFolderInTerminalRequested, this,
                    &SidePane::openFolderInTerminalRequested);
            connect(dirTreeView, &DirTreeView::createNewFolderRequested, this, &SidePane::createNewFolderRequested);
            connect(dirTreeView, &DirTreeView::prepareFileMenu, this, &SidePane::prepareFileMenu);
            break;
        }
        default:;
    }
