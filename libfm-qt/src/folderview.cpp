...
FolderViewListView::FolderViewListView(QWidget* parent)
    : QListView(parent), activationAllowed_(true), cursorOnSelectionCorner_(false), mouseLeftPressed_(false) {
...
QItemSelectionModel::SelectionFlags FolderViewListView::selectionCommand(const QModelIndex& index,
                                                                         const QEvent* event) const {
    // select/deselect the item by pressing its selection corner with the left mouse button
    if (cursorOnSelectionCorner_ && event != nullptr && event->type() == QEvent::MouseButtonPress &&
...
void FolderViewListView::mousePressEvent(QMouseEvent* event) {
    ...
    if (selectionMode() == QAbstractItemView::ExtendedSelection && (event->modifiers() & Qt::ShiftModifier)) {
        ...
        if (!sel.isEmpty()) {
            std::sort(sel.begin(), sel.end(),
                      [](QItemSelectionRange a, QItemSelectionRange b) { return a.top() < b.top(); });
        ...
        || ((event->buttons() & ~(Qt::BackButton | Qt::ForwardButton)) &&
            !(event->buttons() == Qt::LeftButton
              ...
              && (!mouseLeftPressed_
                  ...
                  || (globalItemPressPoint_ - event->globalPosition().toPoint()).manhattanLength() <=
...
QModelIndex FolderViewListView::indexAt(const QPoint& point) const {
    ...
    if (viewMode() == QListView::IconMode && index.isValid()) {
        ...
        FolderItemDelegate* delegate =
            static_cast<FolderItemDelegate*>(itemDelegateForColumn(FolderModel::ColumnFileName));
        ...
        if (isCursorPos && _iconSize.width() >= 48 &&
            ...
            if (point.x() >= icnLeft && point.x() <= icnLeft + s && point.y() >= icnTop && point.y() <= icnTop + s) {
        ...
        if (point.y() < iconTop) {  // above icon
            ...
            if (point.y() > visRect.top() + margins.height() + _iconSize.height() +
                                _textSize.height()  // below text
...
FolderViewTreeView::FolderViewTreeView(QWidget* parent)
    : QTreeView(parent), doingLayout_(false), layoutTimer_(nullptr), activationAllowed_(true) {
...
void FolderViewTreeView::headerContextMenu(const QPoint& p) {
    ...
    if (model()) {
        ...
        QLabel* label =
            new QLabel(QStringLiteral("<center><b>") + tr("Visible Columns") + QStringLiteral("</b></center>"));
...
void FolderViewTreeView::paintEvent(QPaintEvent* event) {
    ...
    if (rubberBandRect_.isValid()) {  // draw rubberband
        ...
        opt.opaque = false;
        QRect r =
            rubberBandRect_.adjusted(-horizontalOffset(), -verticalOffset(), -horizontalOffset(), -verticalOffset())
...
void FolderViewTreeView::mouseMoveEvent(QMouseEvent* event) {
    ...
    if (event->buttons() == Qt::NoButton || (event->buttons() & ~(Qt::BackButton | Qt::ForwardButton))) {
        // handle rubberband
        if (selectionMode() == QAbstractItemView::ExtendedSelection && (event->buttons() & Qt::LeftButton) &&
            ...
            viewport()->update(
                r.adjusted(-horizontalOffset(), -verticalOffset(), -horizontalOffset(), -verticalOffset()));
...
void FolderViewTreeView::setSelection(const QRect& rect, QItemSelectionModel::SelectionFlags command) {
    if (selectionMode() == QAbstractItemView::ExtendedSelection && model() &&
        state() == QAbstractItemView::DragSelectingState && !rubberBandRect_.isNull()) {  // rubberband selection
        QRect r =
            rubberBandRect_.adjusted(-horizontalOffset(), -verticalOffset(), -horizontalOffset(), -verticalOffset());
...
void FolderViewTreeView::layoutColumns() {
    ...
    if (numCols > 0) {
        ...
        for (int column = 0; column < numCols; ++column) {
            ...
            else if (hiddenColumns_.contains(columnId) && columnId != filenameColumn) {  // never hide the name column
                headerView->setSectionHidden(columnId, true);
                widths.append(headerView->minimumSectionSize());
                continue;
            }
        ...
        if (customColumnWidths_.size() <= filenameColumn) {  // practically means no custom width
            ...
            if (desiredWidth > availWidth) {
                ...
                int filenameMinWidth = std::min(iconSize().width() + 30 * opt.fontMetrics.averageCharWidth(),
...
void FolderViewTreeView::dataChanged(const QModelIndex& topLeft,
...
void FolderView::updateGridSize() {
    ...
    FolderItemDelegate* delegate =
        static_cast<FolderItemDelegate*>(listView->itemDelegateForColumn(FolderModel::ColumnFileName));
...
void FolderView::emitClickedAt(ClickType type, const QPoint& pos) {
    ...
    if (index.isValid() && (!ctrlRightClick_ || QApplication::keyboardModifiers() != Qt::ControlModifier)) {
...
void FolderView::childDropEvent(QDropEvent* e) {
    ...
    if (QGuiApplication::platformName() == QStringLiteral("xcb") &&
        e->mimeData()->hasFormat(QStringLiteral("XdndDirectSave0"))) {
        ...
        if (dndSource != 0) {
            ...
            XdndWorkaround::setWindowProperty(dndSource, XdndDirectSaveAtom, textAtom, (void*)fileUri.constData(),
                                              fileUri.length());
        ...
        switch (event->type()) {
            ...
            case QEvent::Wheel: {
                ...
                if (mode == DetailedListMode) {
                    FolderViewTreeView* treeView = static_cast<FolderViewTreeView*>(view);
                    delegate =
                        static_cast<FolderItemDelegate*>(treeView->itemDelegateForColumn(FolderModel::ColumnFileName));
                }
                else {
                    FolderViewListView* listView = static_cast<FolderViewListView*>(view);
                ...
                if (event->spontaneous() &&
                    ...
                    if (sbar && sbar->isVisible()) {
                        ...
                        if (QApplication::wheelScrollLines() > 1) {
                            ...
                            if (mode == CompactMode || (we->modifiers() & Qt::ShiftModifier) || std::abs(delta) < 120 ||
                            ...
                            else if (iconSize(mode).height() >= 64 && QApplication::wheelScrollLines() > 2 &&
                        ...
                        if (!scrollPerPixel_) {
                            ...
                            QWheelEvent e(we->position(), we->globalPosition(), we->pixelDelta(), QPoint(0, delta),
                                          we->buttons(), Qt::NoModifier, we->phase(), false, we->source());
...
void FolderView::scrollSmoothly() {
    ...
    if (totalDelta != 0) {
        ...
        if ((mode == IconMode || mode == ThumbnailMode) && (QApplication::mouseButtons() & Qt::LeftButton)) {
            const QPoint globalPos = QCursor::pos();
            QPoint pos = view->viewport()->mapFromGlobal(globalPos);
            QMouseEvent ev(QEvent::MouseMove, pos, view->viewport()->mapTo(view->viewport()->topLevelWidget(), pos),
                           globalPos, Qt::LeftButton, Qt::LeftButton, QApplication::keyboardModifiers());
...
void FolderView::onAutoSelectionTimeout() {
    ...
    QItemSelectionModel::SelectionFlags flags =
        (mode == DetailedListMode ? QItemSelectionModel::Rows : QItemSelectionModel::NoUpdate);
    QItemSelectionModel* selModel = view->selectionModel();
