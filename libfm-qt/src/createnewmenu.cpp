...
class TemplateAction : public QAction {
   public:
    TemplateAction(std::shared_ptr<const TemplateItem> item, QObject* parent) : QAction(parent) {
        setTemplateItem(std::move(item));
    }

    const std::shared_ptr<const TemplateItem> templateItem() const { return templateItem_; }
    ...
    void setTemplateItem(std::shared_ptr<const TemplateItem> item) {
        ...
        setText(QStringLiteral("%1 (%2)").arg(templateItem_->displayName(), QString::fromUtf8(mimeType->desc())));
...
CreateNewMenu::CreateNewMenu(QWidget* dialogParent, Fm::FilePath dirPath, QWidget* parent)
    ...
    templates_->forEachItem([this](const std::shared_ptr<const TemplateItem>& item) { addTemplateItem(item); });
}
...
void CreateNewMenu::updateTemplateItem(const std::shared_ptr<const TemplateItem>& oldItem,
                                       const std::shared_ptr<const TemplateItem>& newItem) {
