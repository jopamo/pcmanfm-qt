...
class LIBFM_QT_API AppChooserDialog : public QDialog {
    Q_OBJECT
   public:
    explicit AppChooserDialog(std::shared_ptr<const Fm::MimeType> mimeType,
    ...
    const std::shared_ptr<const Fm::MimeType>& mimeType() const { return mimeType_; }

    void setCanSetDefault(bool value);

    bool canSetDefault() const { return canSetDefault_; }

    const Fm::GAppInfoPtr& selectedApp() const { return selectedApp_; }
