...
ExecFileDialog::ExecFileDialog(const FileInfo& fileInfo, QWidget* parent, Qt::WindowFlags f)
    : QDialog(parent, f), ui(new Ui::ExecFileDialog()), result_(BasicFileLauncher::ExecAction::DIRECT_EXEC) {
    ui->setupUi(this);
    // show file icon
    ...
    if (fileInfo.isDesktopEntry()) {
        ...
        msg = tr("This file '%1' is executable. Do you want to execute it?").arg(fileInfo.displayName());
        ui->exec->setDefault(true);
        ui->open->hide();
    }
    ui->msg->setText(msg);
