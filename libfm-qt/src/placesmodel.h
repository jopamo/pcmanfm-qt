...
class LIBFM_QT_API PlacesModel : public QStandardItemModel {
   ...
   public:
    enum { FileInfoRole = Qt::UserRole, FmIconRole };
    ...
    class ItemAction : public QAction {
       public:
        explicit ItemAction(const QModelIndex& index, QString text, QObject* parent = nullptr)
            : QAction(text, parent), index_(index) {}

        QPersistentModelIndex& index() { return index_; }
   ...
   public:
    ...
    bool showTrash() { return trashItem_ != nullptr; }
    void setShowTrash(bool show);

    bool showApplications() { return showApplications_; }
    void setShowApplications(bool show);

    bool showDesktop() { return showDesktop_; }
