...
class LIBFM_QT_API BrowseHistoryItem {
   public:
    explicit BrowseHistoryItem() : scrollPos_(0) {}

    explicit BrowseHistoryItem(Fm::FilePath path, int scrollPos = 0) : path_(std::move(path)), scrollPos_(scrollPos) {}
    ...
    Fm::FilePath path() const { return path_; }

    int scrollPos() const { return scrollPos_; }

    void setScrollPos(int pos) { scrollPos_ = pos; }
...
class LIBFM_QT_API BrowseHistory {
   public:
    ...
    Fm::FilePath currentPath() const { return items_[currentIndex_].path(); }

    int currentScrollPos() const { return items_[currentIndex_].scrollPos(); }

    BrowseHistoryItem& currentItem() { return items_[currentIndex_]; }

    size_t size() const { return items_.size(); }

    BrowseHistoryItem& at(int index) { return items_[index]; }
