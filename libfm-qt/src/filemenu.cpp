...
FileMenu::FileMenu(Fm::FileInfoList files,
    ...
    menu->addSeparator();
    openWithAction_ =
        new QAction(QIcon::fromTheme(QStringLiteral("applications-other")), tr("Other Applications"), this);
    ...
    if (allTrash_) {  // all selected files are in trash:///
        ...
        if (!(sameType_ && info_->isDir() && (files_.size() > 1 ? isWritableDir : info_->isWritable()))) {
    ...
    if (!allVirtual_ && !allTrash_ && !(sameFilesystem_ && path.hasUriScheme("computer"))) {
...
void FileMenu::addTrustAction() {
    if (info_->isExecutableType()
        ...
        && (info_->isDesktopEntry() || g_file_test(info_->path().localPath().get(), G_FILE_TEST_IS_EXECUTABLE))) {
        QAction* trustAction =
            new QAction(files_.size() > 1 ? tr("Trust selected executables") : tr("Trust this executable"), this);
...
void FileMenu::onOpenTriggered() {
    if (files_.size() > 20) {
        QMessageBox::StandardButton r =
            QMessageBox::question(parentWidget() ? parentWidget()->window() : nullptr, tr("Many files"),
                                  tr("Do you want to open these %1 files?", nullptr, files_.size()).arg(files_.size()),
...
void FileMenu::setUseTrash(bool trash) {
    if (useTrash_ != trash) {
        ...
        if (deleteAction_ && !info_->path().hasUriScheme("trash")) {
            ...
            deleteAction_->setIcon(useTrash_ ? QIcon::fromTheme(QStringLiteral("user-trash"))
