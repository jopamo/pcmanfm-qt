...
PlacesProxyModel::PlacesProxyModel(QObject* parent)
    : QSortFilterProxyModel(parent), showAll_(false), hiddenItemsRestored_(false) {}
...
PlacesView::PlacesView(QWidget* parent) : QTreeView(parent) {
    ...
    connect(model_.get(), &QAbstractItemModel::rowsRemoved, this,
            [](const QModelIndex&, int, int) { proxyModel_->setHidden(QString()); });
            ...
            if (item->type() == PlacesModelItem::Volume) {
                ...
                if (!volumeItem->isMounted()) {
                    ...
                    QTimer::singleShot(0, op, [this, op, type, index]() {
                        if (op->wait()) {
                            if (PlacesModelItem* item = static_cast<PlacesModelItem*>(
                                    model_->itemFromIndex(proxyModel_->mapToSource(index)))) {
                                auto path = item->path();
    ...
    if (index.column() == 0) {
        ...
        if (index.parent() == proxyModel_->mapFromSource(model_->devicesRoot->index())) {  // this is a mounted device
            ...
            PlacesModelItem* item =
                static_cast<PlacesModelItem*>(model_->itemFromIndex(proxyModel_->mapToSource(itemIndex)));
            if (item) {
...
void PlacesView::setCurrentPath(Fm::FilePath path) {
    ...
    if (currentPath_) {
        ...
        if (item) {
            selectionModel()->select(proxyModel_->mapFromSource(item->index()),
...
void PlacesView::dropEvent(QDropEvent* event) {
    ...
    if (!event->mimeData()->hasFormat(
            QStringLiteral("application/x-bookmark-row"))  // dropped data is not a bookmark row
        && event->mimeData()->hasUrls()) {                 // file uris are dropped
        QModelIndex index = indexAt(event->position().toPoint());
        if (index.isValid() && index.column() == 0  // the real item is at column 0
            ...
            if (item && item->type() != PlacesModelItem::Mount &&
                (item->type() != PlacesModelItem::Volume || static_cast<PlacesModelVolumeItem*>(item)->isMounted())) {
                auto destPath = item->path();
                if (destPath && strcmp(destPath.toString().get(), "menu://applications/") != 0 &&
                    ...
                    if (!srcPaths.empty()) {
                        ...
                        QTimer::singleShot(0, this, [this, curPos, srcPaths, destPath] {
                            if (strcmp(destPath.toString().get(), "trash:///") == 0) {
                                ...
                                Qt::DropAction action = DndActionMenu::askUser(
                                    Qt::CopyAction | Qt::MoveAction | Qt::LinkAction, curPos, viewport());
                                switch (action) {
                                    case Qt::CopyAction:
...
void PlacesView::commitData(QWidget* editor) {
    ...
    PlacesModelBookmarkItem* item =
        static_cast<PlacesModelBookmarkItem*>(model_->itemFromIndex(proxyModel_->mapToSource(currentIndex())));
    auto bookmarkItem = item->bookmark();
    // rename bookmark
...
void PlacesView::onMountVolume() {
    ...
    QTimer::singleShot(0, op, [op] { op->wait(); });
}
...
void PlacesView::contextMenuEvent(QContextMenuEvent* event) {
    ...
    if (index.isValid()) {
        ...
        if (index.parent().isValid() && item->type() != PlacesModelItem::Mount &&
            (item->type() != PlacesModelItem::Volume || static_cast<PlacesModelVolumeItem*>(item)->isMounted())) {
        ...
        if (proxyModel_->hasHidden()) {
            ...
            connect(action, &QAction::triggered, [this](bool checked) { showAll(checked); });
            menu->addAction(action);
        }
