...
class AppMenuViewItem : public QStandardItem {
   public:
    explicit AppMenuViewItem(MenuCacheItem* item) : item_(menu_cache_item_ref(item)) {
    ...
    ~AppMenuViewItem() override { menu_cache_item_unref(item_); }

    MenuCacheItem* item() { return item_; }

    int type() const override { return menu_cache_item_get_type(item_); }

    bool isApp() { return type() == MENU_CACHE_TYPE_APP; }

    bool isDir() { return type() == MENU_CACHE_TYPE_DIR; }
