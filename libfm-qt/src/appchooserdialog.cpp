...
GAppInfo* AppChooserDialog::customCommandToApp() {
    ...
    if (!cmdline.isEmpty()) {
        ...
        app = app_info_create_from_commandline(cmdline.constData(), app_name.constData(), bin1,
                                               mimeType_ ? mimeType_->name() : nullptr, ui->useTerminal->isChecked(),
                                               ui->keepTermOpen->isChecked());
    _out:
        g_free(bin1);
    }
    return app;
...
void AppChooserDialog::setMimeType(std::shared_ptr<const Fm::MimeType> mimeType) {
    ...
    if (mimeType_) {
        QString text = tr("Select an application to open \"%1\" files").arg(QString::fromUtf8(mimeType_->desc()));
