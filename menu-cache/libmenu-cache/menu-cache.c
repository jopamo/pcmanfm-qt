...
#define SET_CACHE_READY(_cache_)          \
    do {                                  \
...
struct _MenuCache {
    ...
    guint version;
    guint reload_id;
    guint ready : 1; /* used for sync access */
};
...
static MenuCacheItem* read_item(GDataInputStream* f,
                                MenuCache* cache,
...
static void read_dir(GDataInputStream* f,
                     MenuCacheDir* dir,
                     MenuCache* cache,
                     MenuCacheFileDir** all_used_files,
                     int n_all_used_files) {
...
static gint read_all_used_files(GDataInputStream* f, MenuCache* cache, MenuCacheFileDir*** all_used_files) {
    char* line;
    gsize len;
    int i, n;
...
MenuCacheNotifyId menu_cache_add_reload_notify(MenuCache* cache, MenuCacheReloadNotify func, gpointer user_data) {
    ...
    if (is_first && cache->reload_id == 0)
        cache->reload_id = g_idle_add_full(G_PRIORITY_HIGH_IDLE, menu_cache_reload_idle, menu_cache_ref(cache),
...
gboolean menu_cache_reload(MenuCache* cache) {
    ...
    g_idle_add_full(G_PRIORITY_HIGH_IDLE, reload_notify, menu_cache_ref(cache), (GDestroyNotify)menu_cache_unref);
    MENU_CACHE_UNLOCK;
...
GSList* menu_cache_dir_get_children(MenuCacheDir* dir) {
    ...
    g_timeout_add_seconds(10, (GSourceFunc)menu_cache_item_unref, menu_cache_item_ref(MENU_CACHE_ITEM(dir)));
    return dir->children;
}
...
gboolean menu_cache_app_get_is_visible(MenuCacheApp* app, guint32 de_flags) {
    if (app->flags & FLAG_IS_NODISPLAY)
        return FALSE;
    return (!app->show_in_flags || (app->show_in_flags & de_flags)) && _can_be_exec(app);
}
...
static void get_socket_name(char* buf, int len) {
    ...
    g_snprintf(buf, len, "%s/menu-cached-%s", g_get_user_runtime_dir(), dpy ? dpy : ":0");
#else
...
static MenuCache* menu_cache_create(const char* menu_name) {
    ...
    buf = g_strdup_printf("REG:%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t" CACHE_VERSION "\t00000000000000000000000000000000\n",
                          menu_name, langs_list, xdg_cache_home, xdg_cfg, xdg_prefix, xdg_data, xdg_cfg_home,
                          xdg_data_home);
...
static gpointer menu_cache_loader_thread(gpointer data) {
    ...
    if ((cache->cancellable && g_cancellable_is_cancelled(cache->cancellable)) || !register_menu_to_server(cache))
        SET_CACHE_READY(cache);
    return NULL;
}
