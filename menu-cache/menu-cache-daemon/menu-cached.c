...
static void on_file_changed(GFileMonitor* mon, GFile* gf, GFile* other, GFileMonitorEvent evt, Cache* cache);
...
static void do_reload(Cache* cache) {
    ...
    if (!regenerate_cache(cache->menu_name, cache->lang_name, cache->cache_file, cache->env, &new_n_files,
                          &new_files)) {
    ...
    for (i = 0; i < cache->n_files; ++i) {
        g_file_monitor_cancel(cache->mons[i]);
        g_signal_handlers_disconnect_by_func(cache->mons[i], (gpointer)on_file_changed, cache);
        g_object_unref(cache->mons[i]);
    }
    /*
        g_file_monitor_cancel(cache->cache_mon);
...
void on_file_changed(GFileMonitor* mon, GFile* gf, GFile* other, GFileMonitorEvent evt, Cache* cache) {
    (void)other;
    (void)evt;
#ifdef G_ENABLE_DEBUG
    ...
    cache->delayed_reload_handler =
        g_timeout_add_seconds_full(G_PRIORITY_LOW, 3, (GSourceFunc)delayed_reload, cache, NULL);
}
    ...
    if (memcmp(line, "REG:", 4) == 0) {
        ...
        if (!cache) {
            ...
            */
            ...
            if (!regenerate_cache(cache->menu_name, cache->lang_name, cache->cache_file, cache->env, &cache->n_files,
                                  &cache->files)) {
                DEBUG("regeneration of cache failed.");
            }
        }
...
static gboolean on_new_conn_incoming(GIOChannel* ch, GIOCondition cond, gpointer user_data) {
    ...
    client_io->source_id =
        g_io_add_watch(child, G_IO_PRI | G_IO_IN | G_IO_HUP | G_IO_ERR, on_client_data_in, client_io);
...
int main(int argc, char** argv) {
    ...
    if (!freopen("/dev/null", "r", stdin)) {
        g_warning("failed to reopen stdin to /dev/null");
    }
    if (!freopen("/dev/null", "w", stdout)) {
        g_warning("failed to reopen stdout to /dev/null");
    }
    if (!freopen("/dev/null", "w", stderr)) {
        g_warning("failed to reopen stderr to /dev/null");
    }
#endif
