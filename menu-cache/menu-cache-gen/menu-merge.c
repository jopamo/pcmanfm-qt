...
#define RETURN_TRUE_AND_DESTROY_IF_QUIET(a) \
    do {                                    \
        if (verbose == 0) {                 \
...
static gboolean _fail_if_in_layout(FmXmlFileItem* item, GError** error) {
    ...
    if (parent && (fm_xml_file_item_get_tag(parent) == menuTag_Layout ||
        ...
        g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, _("Tag <%s> is invalid below <%s>"),
                    fm_xml_file_item_get_tag_name(item), fm_xml_file_item_get_tag_name(parent));
        return TRUE;
    }
    return FALSE;
}

#define RETURN_IF_IN_LAYOUT(i, e)        \
    do {                                 \
...
/* checks the tag */
static gboolean _menu_xml_handler_Name(FmXmlFileItem* item,
                                       GList* children,
                                       char* const* attribute_names,
                                       char* const* attribute_values,
                                       guint n_attributes,
    ...
    if (name_item == NULL || (name = fm_xml_file_item_get_data(name_item, NULL)) == NULL ||
        strchr(name, '/') != NULL) /* empty or invalid tag */
    {
        RETURN_TRUE_AND_DESTROY_IF_QUIET(item);
        g_set_error_literal(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, _("Invalid <Name> tag"));
        return FALSE;
    }
    return TRUE;
}

static gboolean _menu_xml_handler_Not(FmXmlFileItem* item,
                                      GList* children,
                                      char* const* attribute_names,
                                      char* const* attribute_values,
                                      guint n_attributes,
    ...
    if (verbose > 0)
        for (child = children; child; child = child->next) {
            ...
            if (tag != menuTag_And && tag == menuTag_Or && tag == menuTag_Filename && tag != menuTag_Category &&
                tag != menuTag_All) {
...
static gboolean _menu_xml_handler_AppDir(FmXmlFileItem* item,
                                         GList* children,
                                         char* const* attribute_names,
                                         char* const* attribute_values,
                                         guint n_attributes,
    ...
    if (children == NULL || fm_xml_file_item_get_tag((name = children->data)) != FM_XML_FILE_TEXT ||
        (path = fm_xml_file_item_get_data(name, NULL)) == NULL) {
        RETURN_TRUE_AND_DESTROY_IF_QUIET(item);
        g_set_error_literal(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, _("Invalid <AppDir> tag"));
        return FALSE;
    }
...
static gboolean _menu_xml_handler_DefaultAppDirs(FmXmlFileItem* item,
                                                 GList* children,
                                                 char* const* attribute_names,
                                                 char* const* attribute_values,
                                                 guint n_attributes,
...
static gboolean _menu_xml_handler_DirectoryDir(FmXmlFileItem* item,
                                               GList* children,
                                               char* const* attribute_names,
                                               char* const* attribute_values,
                                               guint n_attributes,
    ...
    if (children == NULL || fm_xml_file_item_get_tag((name = children->data)) != FM_XML_FILE_TEXT ||
        (path = fm_xml_file_item_get_data(name, NULL)) == NULL) {
        RETURN_TRUE_AND_DESTROY_IF_QUIET(item);
        g_set_error_literal(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, _("Invalid <DirectoryDir> tag"));
        return FALSE;
    }
    if (g_path_is_absolute(path))
        _add_dir_dir(path);
...
static gboolean _menu_xml_handler_DefaultDirectoryDirs(FmXmlFileItem* item,
                                                       GList* children,
                                                       char* const* attribute_names,
                                                       char* const* attribute_values,
                                                       guint n_attributes,
...
static gboolean _menu_xml_handler_MergeFile(FmXmlFileItem* item,
                                            GList* children,
                                            char* const* attribute_names,
                                            char* const* attribute_values,
                                            guint n_attributes,
    ...
    if (children == NULL || fm_xml_file_item_get_tag((name = children->data)) != FM_XML_FILE_TEXT ||
        (path = fm_xml_file_item_get_data(name, NULL)) == NULL) {
        RETURN_TRUE_AND_DESTROY_IF_QUIET(item);
        g_set_error_literal(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, _("Invalid <MergeFile> tag"));
        return FALSE;
    }
    if (attribute_names)
...
static gboolean _menu_xml_handler_MergeDir(FmXmlFileItem* item,
                                           GList* children,
                                           char* const* attribute_names,
                                           char* const* attribute_values,
                                           guint n_attributes,
    ...
    if (children == NULL || fm_xml_file_item_get_tag((name = children->data)) != FM_XML_FILE_TEXT ||
        (path = fm_xml_file_item_get_data(name, NULL)) == NULL) {
        RETURN_TRUE_AND_DESTROY_IF_QUIET(item);
        g_set_error_literal(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, _("Invalid <MergeDir> tag"));
        return FALSE;
    }
    if (!g_path_is_absolute(path)) {
        char* _dir = g_path_get_dirname(data->file_path);
...
static gboolean _menu_xml_handler_DefaultMergeDirs(FmXmlFileItem* item,
                                                   GList* children,
                                                   char* const* attribute_names,
                                                   char* const* attribute_values,
                                                   guint n_attributes,
    ...
    if (parent == NULL || fm_xml_file_item_get_tag(parent) != menuTag_Menu) {
        RETURN_TRUE_AND_DESTROY_IF_QUIET(item);
        g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, _("Tag <%s> can appear only below <Menu>"),
...
static gboolean _menu_xml_handler_LegacyDir(FmXmlFileItem* item,
                                            GList* children,
                                            char* const* attribute_names,
                                            char* const* attribute_values,
                                            guint n_attributes,
    ...
    if (children == NULL || fm_xml_file_item_get_tag((name = children->data)) != FM_XML_FILE_TEXT ||
        (path = fm_xml_file_item_get_data(name, NULL)) == NULL) {
        RETURN_TRUE_AND_DESTROY_IF_QUIET(item);
        g_set_error_literal(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, _("Invalid <LegacyDir> tag"));
        return FALSE;
    }
    if (!g_path_is_absolute(path)) {
        char* _dir = g_path_get_dirname(data->file_path);
...
static gboolean _menu_xml_handler_Filename(FmXmlFileItem* item,
                                           GList* children,
                                           char* const* attribute_names,
                                           char* const* attribute_values,
                                           guint n_attributes,
    ...
    if (children == NULL || fm_xml_file_item_get_tag(children->data) != FM_XML_FILE_TEXT ||
        (id = fm_xml_file_item_get_data(children->data, NULL)) == NULL) {
        RETURN_TRUE_AND_DESTROY_IF_QUIET(item);
        g_set_error_literal(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, _("Empty <Filename> tag"));
        return FALSE;
    }
    parent = fm_xml_file_item_get_parent(item);
    if (parent)
...
static gboolean _menu_xml_handler_Menuname(FmXmlFileItem* item,
                                           GList* children,
                                           char* const* attribute_names,
                                           char* const* attribute_values,
                                           guint n_attributes,
    ...
    if (children == NULL || fm_xml_file_item_get_tag(children->data) != FM_XML_FILE_TEXT ||
        ...
        RETURN_TRUE_AND_DESTROY_IF_QUIET(item);
        g_set_error_literal(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, _("Empty <Menuname> tag"));
        return FALSE;
    }
    parent = fm_xml_file_item_get_parent(item);
    if (parent)
    ...
    if (attribute_names)
        while (attribute_names[0]) {
            if (strcmp(attribute_names[0], "show_empty") == 0) {
                menu->layout.show_empty = (g_ascii_strcasecmp(attribute_values[0], "true") == 0);
                menu->layout.only_unallocated = TRUE;
            }
            else if (strcmp(attribute_names[0], "inline") == 0) {
                menu->layout.allow_inline = (g_ascii_strcasecmp(attribute_values[0], "true") == 0);
                menu->layout.is_set = TRUE;
            }
            else if (strcmp(attribute_names[0], "inline_header") == 0) {
                menu->layout.inline_header = (g_ascii_strcasecmp(attribute_values[0], "true") == 0);
                menu->layout.inline_header_is_set = TRUE;
            }
            else if (strcmp(attribute_names[0], "inline_alias") == 0) {
                menu->layout.inline_alias = (g_ascii_strcasecmp(attribute_values[0], "true") == 0);
...
static gboolean _menu_xml_handler_Separator(FmXmlFileItem* item,
                                            GList* children,
                                            char* const* attribute_names,
                                            char* const* attribute_values,
                                            guint n_attributes,
...
static gboolean _menu_xml_handler_Merge(FmXmlFileItem* item,
                                        GList* children,
                                        char* const* attribute_names,
                                        char* const* attribute_values,
                                        guint n_attributes,
...
static gboolean _menu_xml_handler_Layout(FmXmlFileItem* item,
                                         GList* children,
                                         char* const* attribute_names,
                                         char* const* attribute_values,
                                         guint n_attributes,
...
static gboolean _menu_xml_handler_DefaultLayout(FmXmlFileItem* item,
                                                GList* children,
                                                char* const* attribute_names,
                                                char* const* attribute_values,
                                                guint n_attributes,
    ...
    if (attribute_names)
        while (attribute_names[0]) {
            if (strcmp(attribute_names[0], "show_empty") == 0)
                layout->show_empty = (g_ascii_strcasecmp(attribute_values[0], "true") == 0);
            else if (strcmp(attribute_names[0], "inline") == 0)
                layout->allow_inline = (g_ascii_strcasecmp(attribute_values[0], "true") == 0);
            else if (strcmp(attribute_names[0], "inline_header") == 0)
                layout->inline_header = (g_ascii_strcasecmp(attribute_values[0], "true") == 0);
            else if (strcmp(attribute_names[0], "inline_alias") == 0)
                layout->inline_alias = (g_ascii_strcasecmp(attribute_values[0], "true") == 0);
            else if (strcmp(attribute_names[0], "inline_limit") == 0)
...
static gboolean _merge_xml_file(MenuTreeData* data,
                                FmXmlFileItem* item,
                                const char* path,
                                GList** m,
                                GError** error,
        ...
        if (fm_xml_file_item_get_tag(it->data) != menuTag_Menu) {
            ...
            g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT,
                        _("Merging file may contain only <Menu> top level tag,"
                          " got <%s>"),
                        fm_xml_file_item_get_tag_name(it->data));
...
static gboolean _merge_menu_directory(MenuTreeData* data,
                                      FmXmlFileItem* item,
                                      const char* path,
                                      GList** m,
                                      GError** error,
    ...
    if (dir) {
        while ((name = g_dir_read_name(dir))) {
            ...
            if (!ok) {
                ...
                */
                g_free(child);
                if (ignore_not_exist && err->domain == G_IO_ERROR && (err->code == G_IO_ERROR_NOT_FOUND)) {
                    g_clear_error(&err);
                    continue;
                }
                g_propagate_error(error, err);
    ...
    else if (ignore_not_exist && err->domain == G_FILE_ERROR && (err->code == G_FILE_ERROR_NOENT)) {
...
static FmXmlFileItem* _walk_path(GList* child, const char* path, FmXmlFileItem* parent, gboolean create) {
...
static FmXmlFileItem* _walk_children(GList* children, FmXmlFileItem* list, FmXmlFileTag tag, gboolean create) {
    GList *sub, *l;
    FmXmlFileItem* item = NULL;
...
static gboolean _activate_merges(MenuTreeData* data, FmXmlFileItem* item, GError** error) {
...
restart:
    ...
    if (sub != NULL) {
        ...
        while (i > 0) {
            ...
            if (!fm_xml_file_insert_before(sub, it_sub) && verbose > 0) {
                g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_UNKNOWN_ELEMENT,
                            _("Failed to insert tag <MergeDir>%s</MergeDir>"), merged);
                g_free(merged);
    ...
    for (l = children; l; l = l->next) {
        ...
        if (tag == menuTag_MergeFile || tag == menuTag_MergeDir) {
            path = fm_xml_file_item_get_data(fm_xml_file_item_find_child(l->data, FM_XML_FILE_TEXT), NULL);
            /* find duplicate - only last one should be used */
            for (l2 = l->next; l2; l2 = l2->next) {
                if (fm_xml_file_item_get_tag(l2->data) == tag) {
                    path2 = fm_xml_file_item_get_data(fm_xml_file_item_find_child(l2->data, FM_XML_FILE_TEXT), NULL);
    ...
    if (sub != NULL) {
        ...
        while (i > 0) {
            ...
            if (!fm_xml_file_insert_before(sub, it_sub) && verbose > 0) {
                g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_UNKNOWN_ELEMENT,
                            _("Failed to insert tag <AppDir>%s</AppDir>"), merged);
                g_free(merged);
                goto failed; /* failed to merge */
            }
            g_free(merged);
        ...
        for (l2 = l->next; l2; l2 = l2->next)
            if (l2->data != NULL && fm_xml_file_item_get_tag(l2->data) == menuTag_AppDir)
                if (strcmp(fm_xml_file_item_get_data(fm_xml_file_item_find_child(l2->data, FM_XML_FILE_TEXT), NULL),
                           fm_xml_file_item_get_data(fm_xml_file_item_find_child(l->data, FM_XML_FILE_TEXT), NULL)) ==
                    0)
                    break;
    ...
    if (sub != NULL) {
        ...
        while (i > 0) {
            ...
            if (!fm_xml_file_insert_before(sub, it_sub) && verbose > 0) {
                g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_UNKNOWN_ELEMENT,
                            _("Failed to insert tag <LegacyDir>%s</LegacyDir>"), merged);
                g_free(merged);
                goto failed; /* failed to merge */
            }
            g_free(merged);
    ...
    if (sub != NULL) {
        ...
        while (i > 0) {
            ...
            if (!fm_xml_file_insert_before(sub, it_sub) && verbose > 0) {
                g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_UNKNOWN_ELEMENT,
                            _("Failed to insert tag <DirectoryDir>%s</DirectoryDir>"), merged);
                g_free(merged);
                goto failed; /* failed to merge */
            }
            g_free(merged);
    ...
    for (l = children; l; l = l->next) {
        sub = l->data;
        if (fm_xml_file_item_get_tag(sub) == menuTag_Menu && !_activate_merges(data, sub, error))
            goto failed; /* failed to merge */
    }
    g_list_free(children);
    return TRUE;
...
static MenuMenu* _make_menu_node(FmXmlFileItem* node, MenuLayout* def) {
    ...
    for (l = children; l; l = l->next) {
        ...
        if (tag == menuTag_Menu) {
            ...
            if (item != NULL)
                menu->id = g_list_prepend(menu->id, g_strdup(fm_xml_file_item_get_data(item, NULL)));
        }
        else if (tag == menuTag_Name) {
            if (menu->name == NULL)
                menu->name =
                    g_strdup(fm_xml_file_item_get_data(fm_xml_file_item_find_child(l->data, FM_XML_FILE_TEXT), NULL));
        }
        else if (tag == menuTag_OnlyUnallocated)
            menu->layout.only_unallocated = TRUE;
        else if (tag == menuTag_NotOnlyUnallocated)
            menu->layout.only_unallocated = FALSE;
        else if (tag == menuTag_Include || tag == menuTag_Exclude || tag == menuTag_DirectoryDir ||
...
MenuMenu* get_merged_menu(const char* file, FmXmlFile** xmlfile, GError** error) {
    ...
    menuTag_Filename = fm_xml_file_set_handler(data.menu, "Filename", &_menu_xml_handler_Filename, FALSE, NULL);
    menuTag_Or = fm_xml_file_set_handler(data.menu, "Or", &_menu_xml_handler_pass, FALSE, NULL);
    menuTag_And = fm_xml_file_set_handler(data.menu, "And", &_menu_xml_handler_pass, FALSE, NULL);
    menuTag_Not = fm_xml_file_set_handler(data.menu, "Not", &_menu_xml_handler_Not, FALSE, NULL);
    menuTag_Category = fm_xml_file_set_handler(data.menu, "Category", &_menu_xml_handler_pass, FALSE, NULL);
    menuTag_MergeFile = fm_xml_file_set_handler(data.menu, "MergeFile", &_menu_xml_handler_MergeFile, FALSE, NULL);
    menuTag_MergeDir = fm_xml_file_set_handler(data.menu, "MergeDir", &_menu_xml_handler_MergeDir, FALSE, NULL);
    menuTag_DefaultMergeDirs =
        fm_xml_file_set_handler(data.menu, "DefaultMergeDirs", &_menu_xml_handler_DefaultMergeDirs, FALSE, NULL);
    menuTag_KDELegacyDirs =
        fm_xml_file_set_handler(data.menu, "KDELegacyDirs", &_menu_xml_handler_DefaultMergeDirs, FALSE, NULL);
    menuTag_Name = fm_xml_file_set_handler(data.menu, "Name", &_menu_xml_handler_Name, FALSE, NULL);
    ...
    menuTag_AppDir = fm_xml_file_set_handler(data.menu, "AppDir", &_menu_xml_handler_AppDir, FALSE, NULL);
    menuTag_DefaultAppDirs =
        fm_xml_file_set_handler(data.menu, "DefaultAppDirs", &_menu_xml_handler_DefaultAppDirs, FALSE, NULL);
    menuTag_DirectoryDir =
        fm_xml_file_set_handler(data.menu, "DirectoryDir", &_menu_xml_handler_DirectoryDir, FALSE, NULL);
    ...
    menuTag_OnlyUnallocated =
        fm_xml_file_set_handler(data.menu, "OnlyUnallocated", &_menu_xml_handler_pass, FALSE, NULL);
    menuTag_NotOnlyUnallocated =
        fm_xml_file_set_handler(data.menu, "NotOnlyUnallocated", &_menu_xml_handler_pass, FALSE, NULL);
    menuTag_All = fm_xml_file_set_handler(data.menu, "All", &_menu_xml_handler_pass, FALSE, NULL);
    menuTag_LegacyDir = fm_xml_file_set_handler(data.menu, "LegacyDir", &_menu_xml_handler_LegacyDir, FALSE, NULL);
    menuTag_Move = fm_xml_file_set_handler(data.menu, "Move", &_menu_xml_handler_pass, FALSE, NULL);
    menuTag_Old = fm_xml_file_set_handler(data.menu, "Old", &_menu_xml_handler_pass, FALSE, NULL);
    menuTag_New = fm_xml_file_set_handler(data.menu, "New", &_menu_xml_handler_pass, FALSE, NULL);
    menuTag_Layout = fm_xml_file_set_handler(data.menu, "Layout", &_menu_xml_handler_Layout, FALSE, NULL);
    menuTag_DefaultLayout =
        fm_xml_file_set_handler(data.menu, "DefaultLayout", &_menu_xml_handler_DefaultLayout, FALSE, NULL);
    menuTag_Menuname = fm_xml_file_set_handler(data.menu, "Menuname", &_menu_xml_handler_Menuname, FALSE, NULL);
    menuTag_Separator = fm_xml_file_set_handler(data.menu, "Separator", &_menu_xml_handler_Separator, FALSE, NULL);
    menuTag_Merge = fm_xml_file_set_handler(data.menu, "Merge", &_menu_xml_handler_Merge, FALSE, NULL);
    /* Do parsing */
