...
static char* _get_language_string(GKeyFile* kf, const char* key) {
    ...
    return _escape_lf(g_key_file_get_locale_string(kf, G_KEY_FILE_DESKTOP_GROUP, key, languages[0], NULL));
}
...
static char** _get_language_string_list(GKeyFile* kf, const char* key, gsize* lp) {
    ...
    str = g_key_file_get_locale_string_list(kf, G_KEY_FILE_DESKTOP_GROUP, key, languages[0], lp, NULL);
    if (str != NULL)
...
static void _fill_menu_from_file(MenuMenu* menu, const char* path) {
    ...
    menu->layout.nodisplay =
        g_key_file_get_boolean(kf, G_KEY_FILE_DESKTOP_GROUP, G_KEY_FILE_DESKTOP_KEY_NO_DISPLAY, NULL);
    menu->layout.is_set = TRUE;
exit:
    g_key_file_free(kf);
}

static const char** menu_app_intern_key_file_list(GKeyFile* kf,
                                                  const char* key,
                                                  gboolean localized,
                                                  gboolean add_to_des) {
    gsize len, i;
...
static void _fill_app_from_key_file(MenuApp* app, GKeyFile* kf) {
    ...
    app->categories = menu_app_intern_key_file_list(kf, G_KEY_FILE_DESKTOP_KEY_CATEGORIES, FALSE, FALSE);
    app->keywords = menu_app_intern_key_file_list(kf, "Keywords", TRUE, FALSE);
    app->show_in = menu_app_intern_key_file_list(kf, G_KEY_FILE_DESKTOP_KEY_ONLY_SHOW_IN, FALSE, TRUE);
    app->hide_in = menu_app_intern_key_file_list(kf, G_KEY_FILE_DESKTOP_KEY_NOT_SHOW_IN, FALSE, TRUE);
    app->use_terminal = g_key_file_get_boolean(kf, G_KEY_FILE_DESKTOP_GROUP, G_KEY_FILE_DESKTOP_KEY_TERMINAL, NULL);
    app->use_notification =
        g_key_file_get_boolean(kf, G_KEY_FILE_DESKTOP_GROUP, G_KEY_FILE_DESKTOP_KEY_STARTUP_NOTIFY, NULL);
    app->hidden = g_key_file_get_boolean(kf, G_KEY_FILE_DESKTOP_GROUP, G_KEY_FILE_DESKTOP_KEY_NO_DISPLAY, NULL);
}
...
static void _fill_apps_from_dir(MenuMenu* menu, GList* lptr, GString* prefix, gboolean is_legacy) {
    ...
    while ((name = g_dir_read_name(gd)) != NULL) {
        ...
        if (g_file_test(filename, G_FILE_TEST_IS_DIR)) {
        ...
        else if (!g_str_has_suffix(name, ".desktop") || !g_file_test(filename, G_FILE_TEST_IS_REGULAR) ||
                 !g_key_file_load_from_file(kf, filename, G_KEY_FILE_KEEP_TRANSLATIONS, NULL))
            ; /* ignore not key files */
        else if ((id = g_key_file_get_string(kf, G_KEY_FILE_DESKTOP_GROUP, G_KEY_FILE_DESKTOP_KEY_TYPE, NULL)) ==
                     NULL ||
            ...
            if (app == NULL) {
                if (!g_key_file_get_boolean(kf, G_KEY_FILE_DESKTOP_GROUP, G_KEY_FILE_DESKTOP_KEY_HIDDEN, NULL)) {
                ...
                g_warning(
                    "id '%s' already allocated for %s and requested to"
                    " change to %s, ignoring request",
                    name, (const char*)app->dirs->data, dir);
            else if (g_key_file_get_boolean(kf, G_KEY_FILE_DESKTOP_GROUP, G_KEY_FILE_DESKTOP_KEY_HIDDEN, NULL)) {
                VDBG("removing app id=%s", app->id), g_hash_table_remove(all_apps, name);
            }
            else {
                /* reset the data */
                menu_app_reset(app);
...
static gboolean menu_app_match(MenuApp* app, GList* rules, gboolean do_all) {
    ...
    for (; rules != NULL; rules = rules->next) {
        rule = rules->data;
        if (rule->type != MENU_CACHE_TYPE_NONE || fm_xml_file_item_get_tag(rule->rule) != menuTag_Include)
            continue;
...
static gboolean menu_app_match_excludes(MenuApp* app, GList* rules) {
    ...
    for (; rules != NULL; rules = rules->next) {
        rule = rules->data;
        if (rule->type != MENU_CACHE_TYPE_NONE || fm_xml_file_item_get_tag(rule->rule) != menuTag_Exclude)
            continue;
        children = fm_xml_file_item_get_children(rule->rule);
        for (child = children; child; child = child->next)
            if (menu_app_match_tag(app, child->data))
...
static void menu_menu_free(MenuMenu* menu) {
    g_free(menu->name);
    g_free(menu->key);
    g_list_free_full(menu->id, g_free);
...
static void _stage1(MenuMenu* menu, GList* dirs, GList* apps, GList* legacy, GList* p) {
    ...
    for (child = menu->children; child; child = child->next) {
        ...
        if (tag == menuTag_DirectoryDir) {
            id = g_intern_string(
                fm_xml_file_item_get_data(fm_xml_file_item_find_child(rule->rule, FM_XML_FILE_TEXT), NULL));
    ...
    for (l = _apps; l; l = l->next) {
        for (child = _apps; child; child = result) {
            ...
            if (child == l)
                continue;
            len = strlen(l->data);
            if (strncmp(l->data, child->data, len) == 0 && ((const char*)child->data)[len] == G_DIR_SEPARATOR)
                        ...
                        for (l = available; l; l = l->next) {
                            ...
                            if (app->key == NULL) {
                                if (app->title != NULL)
                                    ...
                                    g_warning("id %s has no Name", app->id), app->key = g_utf8_collate_key(app->id, -1);
                            }
                            app->menus = g_list_prepend(app->menus, menu);
                        }
                        next = available;
                        ...
                        if (tag != 1)
                            for (l = menu->children; l;) {
                                if (((MenuMenu*)l->data)->layout.type == MENU_CACHE_TYPE_DIR) {
                                    ...
                                    VVDBG("+++ composing menu %s (%s)", ((MenuMenu*)this->data)->name,
                                          ((MenuMenu*)this->data)->title);
                                    if (((MenuMenu*)this->data)->key == NULL) {
                                        if (((MenuMenu*)this->data)->title != NULL)
                                            ((MenuMenu*)this->data)->key =
                                                g_utf8_collate_key(((MenuMenu*)this->data)->title, -1);
                                        else
                                            ((MenuMenu*)this->data)->key =
                                                g_utf8_collate_key(((MenuMenu*)this->data)->name, -1);
                                    }
                                    l = this->next;
    ...
    for (child = menu->children; child;) {
        ...
        child = child->next;
        if (submenu->layout.type == MENU_CACHE_TYPE_DIR && submenu->layout.allow_inline &&
            ...
            if (submenu->layout.inline_alias && g_list_length(submenu->children) == 1) {
                ...
                VDBG("replacing title of single child of %s due to inline_alias", submenu->name);
            ...
            while (submenu->children != NULL) {
                menu->children = g_list_insert_before(menu->children, child, submenu->children->data);
...
static gint _stage2(MenuMenu* menu, gboolean with_hidden) {
    ...
    while (child) {
        ...
        switch (app->type) {
            ...
            default:
                /* separator */
                if (child == menu->children || next == NULL || (app = next->data)->type == MENU_CACHE_TYPE_SEP)
                    menu->children = g_list_delete_link(menu->children, child);
        }
        child = next;
    }
    ...
    if (count > 0 && with_hidden)
        ...
        while (to_delete) /* if no apps here then don't keep dirs as well */
        {
...
static gboolean write_app_extra(FILE* f, MenuApp* app) {
    ...
    ret = fprintf(f, "%s\n%s\n%s\n%s\n", NONULL(app->try_exec), NONULL(app->wd), cats, keywords) > 0;
    g_free(cats);
    g_free(keywords);
    return ret;
}
...
static gboolean write_app(FILE* f, MenuApp* app, gboolean with_hidden) {
    ...
    return fprintf(f, "-%s\n%s\n%s\n%s\n%s\n%d\n%s\n%s\n%u\n%d\n", app->id, NONULL(app->title), NONULL(app->comment),
                   NONULL(app->icon), NONULL(app->filename), index, NONULL(app->generic_name), NONULL(app->exec), flags,
                   show) > 0 &&
...
static gboolean write_menu(FILE* f, MenuMenu* menu, gboolean with_hidden) {
    ...
    if (fprintf(f, "+%s\n%s\n%s\n%s\n%s\n%d\n", menu->name, NONULL(menu->title), NONULL(menu->comment),
                NONULL(menu->icon), menu->id ? (const char*)menu->id->data : "", index) < 0)
        return FALSE;
    /* pass show_empty into file if format is v.1.2 */
    if (req_version >= 2 && fprintf(f, "%d\n", menu->layout.nodisplay ? FLAG_IS_NODISPLAY : 0) < 0)
        return FALSE;
...
gboolean save_menu_cache(MenuMenu* layout, const char* menuname, const char* file, gboolean with_hidden) {
    const char* de_names[N_KNOWN_DESKTOPS] = {"LXDE", "GNOME", "KDE", "XFCE", "ROX"};
    char* tmp;
    FILE* f = NULL;
    GSList* l;
    int i;
    ...
    fprintf(f, "1.%d\n%s%s\n%d\n", req_version, /* use CACHE_GEN_VERSION */
            ...
            g_slist_length(DirDirs) + g_slist_length(AppDirs) + g_slist_length(MenuDirs) + g_slist_length(MenuFiles));
    VDBG("%d %d %d %d", g_slist_length(DirDirs), g_slist_length(AppDirs), g_slist_length(MenuDirs),
